...
%><html><link href="Css/calendar-blue.css" rel="stylesheet"><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script src="Js/Calendar.js"></script><script language="javascript" src="inc/func.js" type="text/javascript"></script><style type="text/css"><!--
.style2 {
	color: #990066;
	font-size: 18px;
}
...
.style5 {color: #000000}
--></style><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><body><script language="javascript">
function chk()
{
if(!CheckIsNull(document.form1.shopid,"请选择连锁店.")) return false;
if(!CheckIsNull(document.form1.menshi,"请选择门市.")) return false;
if(!CheckIsNull(document.form1.CustomerLostType,"请选择拍照类型！")) return false;
if(!CheckIsDate(document.form1.PublishCardTime,"请输入正确的发卡日期,格式如:2006-10-1.")) return false;
if(!CheckIsDate(document.form1.WeddingDay,"请输入正确的结婚日期,格式如:2006-10-1.")) return false;
if(!CheckIsNull(document.form1.times,"请填写日期！")) return false;
...
function showph(el){
	tr = document.getElementById("tr_ph");
	if(el.checked){
		tr.style.display='';
	}
	else{
...
sqlsearch = "select * from city order by pxid"
...
response.write"subcat["&count&"] = new Array('"& trim(rssearch("czmc"))&"','"& trim(rssearch("sfid"))&"','"& trim(rssearch("sfmc"))&"');"
dim count
count = count + 1
rssearch.movenext
loop
...
response.write"{"
response.write"document.form1.city.length = 0;"
response.write"var locationid=locationid;"
response.write"var i;"
response.write"for (i=0;i < onecount; i++)"
response.write"{"
response.write"if (subcat[i][2] == locationid)"
response.write"{"
response.write"document.form1.city.options[document.form1.city.length] = new Option(subcat[i][0], subcat[i][0]);"
response.write"}"
response.write"}"
response.write"}"
response.write"</script>"
...
sqlsearch2 =  "select * from city order by pxid"
...
response.write"subcat2["&count2&"] = new Array('"& trim(rssearch2("czmc"))&"','"& trim(rssearch2("sfid"))&"','"& trim(rssearch2("sfmc"))&"');"
dim count2
count2 = count2 + 1
rssearch2.movenext
loop
...
response.write"{"
response.write"document.form1.city2.length = 0;"
response.write"var locationid=locationid;"
response.write"var i;"
response.write"for (i=0;i < onecount2; i++)"
response.write"{"
response.write"if (subcat2[i][2] == locationid)"
response.write"{"
response.write"document.form1.city2.options[document.form1.city2.length] = new Option(subcat[i][0], subcat[i][0]);"
response.write"}"
response.write"}"
response.write"}"
response.write"</script>"
...
beizhu=htmlencode2(request("beizhu"))
if request("action")="add" then
if request("lxpeple")="" then
response.Write "<script>alert('请填写客户姓名！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
count11=conn.execute("select count(*) from kehu where [number]='"&request("number")&"' and [number]<>''")(0)
if count11>0 then
response.Write "<script>alert('该卡号跟已有卡号重复，请检查！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
if request("menshi2")="" and request("menshi3")<>"" then
response.Write "<script>alert('如果合作门市只要一个，请在同事1里面选择！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
if request("menshi2")=request("menshi3") and request("menshi2")<>""  then
response.Write "<script>alert('请不要选择相同的两位合作门市！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
set rs=server.CreateObject("adodb.recordset")
rs.open "select top 1 * from kehu",conn,1,3
rs.addnew
...
rs("group")=conn.execute("select [group] from yuangong where username='"&session("userid")&"'")(0)
rs("lxpeple")=lxpeple
rs("address")=address
...
rs("telephone2")=request("telephone2")
end if
...
rs.update
idd=conn.execute("select max(id) from kehu ")(0)
if request("tianjia")="保存信息转到客户管理区" then
response.Write "<script>alert('添加成功，转到客户专区！');location='admin/ckkh.asp'</script>"
elseif request("tianjia")="保存客人信息并预约" then
response.Write "<script>alert('添加成功,进入预约状态！');location='xiadan.asp?id="&idd&"'</script>"
elseif request("tianjia")="保存信息继续添加客人" then
response.Write "<script>alert('添加成功！');location='tjkh.asp'</script>"
end if
rs.close
set rs=nothing
end if
%><table width="811"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td><div align="center"><img src="img/1101.JPG" width="806" height="43" border="0"></div></td></tr><tr><td height="539" valign="top" background="img/1102.JPG"><table width="100%"  border="0" cellpadding="0" cellspacing="0"><tr><td width="3%" height="34">&nbsp;</td><td colspan="2" align="right">接单门市两人接,就选择两人,业绩平分。如选片换人请找主管更换,后期归选片&nbsp;&nbsp;&nbsp; </td></tr><tr><td height="482" colspan="3" valign="top"><form action="tjkh.asp?action=add" method="post" name="form1"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC" class="xu_kuan"><tr align="left" valign="middle" bgcolor="#FFFFFF"><td height="22" colspan="3" bgcolor="#FFFFFF" class="font">&nbsp;
                  <%
dim rsshop
set rsshop=conn.execute("select * from MultipleShopList order by px")
if rsshop.eof and rsshop.bof then response.write "<span style='display:none'>"
%>&nbsp;连锁店:
                  <select name="shopid" id="shopid"><option value="">请选择</option><option value="0" <%
if rsshop.eof and rsshop.bof then response.write "selected"
%>>总店</option><%
while not rsshop.eof
...
rsshop.close
set rsshop=nothing
%><span class="style7">门市1:
        <select name="menshi" id="menshi"><option value="">请选择</option><%
...
set rs=nothing
%></select></span>                  &nbsp; 门市2:
                    <select name="menshi2" id="select2"><option value="">请选择</option><%
set rs=conn.execute("select username,peplename from yuangong where [level]=1 and username<>'"&session("userid")&"' and isdisabled=0")
while not rs.eof
...
%></select>  
                    &nbsp; 门市3:
                    <select name="menshi3" id="select3"><option value="">请选择</option><%
set rs=conn.execute("select username,peplename from yuangong where [level]=1 and username<>'"&session("userid")&"' and isdisabled=0")
while not rs.eof
%><option value="<%= rs("username") %>"><%= rs("peplename") %></option><%
rs.movenext
...
%></select>
                    &nbsp; 拍照类型:
        <select name="CustomerLostType" id="CustomerLostType"><option value="">请选择</option><%
dim rslosttype
...
%></select></td></tr><tr align="left" valign="middle" bgcolor="#FFFFFF"><td height="20" colspan="3" class="font">&nbsp;&nbsp;卡号：
                  <input name="number" type="text" id="number" size="12">                  
                  &nbsp;发卡日期:
        
        <input name="PublishCardTime" type="text" id="PublishCardTime" size="12" /><a onClick="return showCalendar('PublishCardTime', 'y-mm-dd');" href="javascript:void(0)"><img src="Image/Button.gif" width="25" height="17" border="0" align="absmiddle" id="IMG" /></a>&nbsp;介绍人:
                  <input name="js_name" type="text" id="js_name" size="15" onClick="javascript:openkhwidnow();" readonly /><input type="button" name="button" id="button" value="清空" style="width:30px; background-color:eee" onClick="javascript:$E('js_name').value='';$E('js_id').value='';"><input type="hidden" name="js_id" id="js_id">				  &nbsp;图片
                  <input name="c_pic" type="text" id="c_pic" size="18" /><a href="#" onClick="window.open('admin/upfile.asp?formname=form1&editname=c_pic&filelx=jpg','','status=no,scrollbars=no,top=20,left=110,width=420,height=165')">上传图片</a></td></tr><tr align="left" valign="middle" bgcolor="#FFFFFF"><td height="20" colspan="2" bgcolor="#FFFFFF" class="font">&nbsp; 日期：
                  <input name="times" type="text" id="times" value="<%= date() %>" size="12">
                  <a onClick="return showCalendar('times', 'y-mm-dd');" href="#"><img src="Image/Button.gif" width="25" height="17" border="0" align="absMiddle" id="IMG2" /></a>&nbsp;&nbsp; 结婚纪念：
                  <select name="JhDateType" id="JhDateType"><option value="0" selected>农历</option><option value="1">公历</option></select><input name="WeddingDay" type="text" id="WeddingDay" size="13" /><a onClick="return showCalendar('WeddingDay', 'y-mm-dd');" href="javascript:void(0)"><img src="Image/Button.gif" width="25" height="17" border="0" align="absmiddle" id="IMG" /></a></td><td height="20" bgcolor="#FFFFFF" class="font">&nbsp;<input name="islost" type="checkbox" id="islost" value="1">
流失客户</td></tr><tr align="left" valign="middle" bgcolor="#FFFFFF"><td height="20" colspan="3" bgcolor="#E9F1FE" class="font">&nbsp;客人信息1</td></tr><tr align="left" valign="middle" bgcolor="#FFFFFF"><td width="33%" height="20" class="font">&nbsp;&nbsp;姓名：
                  <input name="lxpeple" type="text" id="lxpeple" size="18"></td><td width="33%" class="font">&nbsp;性别：
                    <input name="sex" type="radio" value="男" checked>
                    男
                    <input type="radio" name="sex" value="女">
                  女</td><td width="34%" class="font">&nbsp;出生：
                    <select name="ShengriType" id="ShengriType"><option value="0" selected>农历</option><option value="1">公历</option></select><input name="chusheng" type="text" id="chusheng" size="12" class="inp1" onFocus="if (this.value == this.defaultValue) this.value='';" onBlur="if (this.value==''){this.value=this.defaultValue;}else{CheckIsShortDate(this,'请输入正确的生日格式,如:8-8或<%= year(date())-25 %>-8-8.\t')}" value="格式如:8-8"></td></tr><tr align="left" valign="middle" bgcolor="#FFFFFF"><td height="21" class="font"> &nbsp;&nbsp;手机：
                  <input name="telephone" type="text" id="telephone5" size="18" onKeyUp="value=value.replace(/[^\d]/g,'')   "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" class="inp1" onFocus="if (this.value == this.defaultValue) this.value='';" onBlur="if (this.value=='') this.value=this.defaultValue;" value="灵通号码前加区号"></td><td height="21" class="font">&nbsp;电话：
                  <input name="home_tel" type="text" id="home_tel" size="18" onKeyUp="value=value.replace(/[^\d]/g,'')   "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"></td><td height="21" class="font">&nbsp;客人QQ:&nbsp;&nbsp;
                    <input name="qq" type="text" id="qq" size="18" /></td></tr><tr align="left" valign="middle" bgcolor="#FFFFFF"><td height="21" colspan="2" class="font"><div align="left"><span class="style4">&nbsp;&nbsp;地址：
                  <select name="city" id="city"><option value="">请选择</option><%
set rs=server.CreateObject("adodb.recordset")
...
%></select>
  &nbsp;
  <input name="address" type="text" id="address" size="35"></span> &nbsp;&nbsp;&nbsp;</div></td><td height="21" class="font">&nbsp;邮政编码:
                  <input name="post" type="text" id="post5" size="18"></td></tr><tr align="left" valign="middle" bgcolor="#E9F1FE"><td height="20" colspan="3" class="font">&nbsp;客人信息2</td></tr><tr align="left" valign="middle" bgcolor="#FFFFFF"><td height="20" class="font">&nbsp;&nbsp;姓名：
                    <input name="lxpeple2" type="text" id="lxpeple24" size="18">
  &nbsp;                  <div align="right"></div></td><td height="20" class="font">&nbsp;
                    <input name="sex2" type="radio" value="男">
                    男
                    <input name="sex2" type="radio" value="女" checked>
                  女</td><td height="20" class="font"> &nbsp;出生：
                    <select name="ShengriType2" id="ShengriType2"><option value="0" selected>农历</option><option value="1">公历</option></select><input name="chusheng2" type="text" id="chusheng2" size="12" class="inp1" onFocus="if (this.value == this.defaultValue) this.value='';" onBlur="if (this.value==''){this.value=this.defaultValue;}else{CheckIsShortDate(this,'请输入正确的纪念日格式,如:8-8或<%= year(date())-25 %>-8-8.\t')}" value="格式如:8-8"></td></tr><tr align="left" valign="middle" bgcolor="#FFFFFF"><td height="20" class="font">&nbsp;&nbsp;手机：
                  <input name="telephone2" type="text" id="telephone26" size="18" onKeyUp="value=value.replace(/[^\d]/g,'')   "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" class="inp1" onFocus="if (this.value == this.defaultValue) this.value='';" onBlur="if (this.value=='') this.value=this.defaultValue;" value="灵通号码前加区号"></td><td height="20" class="font">&nbsp;电话：
                  <input name="home_tel2" type="text" id="home_tel25" size="18" onKeyUp="value=value.replace(/[^\d]/g,'')   "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"></td><td height="20" class="font">&nbsp;客人QQ:&nbsp;&nbsp;
                    <input name="qq2" type="text" id="qq2" size="18" /></td></tr><tr align="left" valign="middle" bgcolor="#FFFFFF"><td height="20" colspan="2" class="font"><div align="right" class="style4"><div align="left">&nbsp;&nbsp;地址：
                    <select name="city2" id="city2"><option value="">请选择</option><%
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from address",conn,1,1
while not rs.eof
%><option value="<%= rs("address") %>"><%= rs("address") %></option><%
...
%></select>
  &nbsp;
  <input name="address2" type="text" id="address22" size="35"></div><div align="left"></div></div></td><td height="20" class="font">&nbsp;邮政编码:
                  <input name="post2" type="text" id="post2" size="18"></td></tr><tr align="left" valign="middle" bgcolor="#FFFFFF"><td height="20" colspan="3" bgcolor="#E9F1FE" class="font">&nbsp;<input name="chk_showph" type="checkbox" id="chk_showph" value="yes" onClick="showph(this)">
                  客人偏好选择</td></tr><tr id="tr_ph" align="left" valign="top" bgcolor="#FFFFFF" style="display:none"><td height="63" colspan="3"><%
set rs3=server.CreateObject("adodb.recordset")
rs3.open "select * from pianhao where ishidden=0 order by px,id",conn,1,1
response.write "<table width='100%' border='0' cellpadding='0' cellspacing='0'>"
while not rs3.eof
response.write "<tr><td>"
response.Write "<div style='float:left; width:80px'>&nbsp;&nbsp;"&rs3("title")&":</div>"
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from pianhao_list where title_id="&rs3("id")&" and ishidden=0 order by px,id",conn,1,1
while not rs.eof
response.write "<div style='float:left; width:65px; white-space:nowrap; padding-right:10px'>"
%><input name="check" type="checkbox" id="check" value="<%= rs("id") %>" />
                  <%
...
rs3.close
set rs3=nothing
%></td></tr><tr align="left" valign="top" bgcolor="#FFFFFF"><td height="39" colspan="3"><table width="562" height="39" border="0" cellpadding="0" cellspacing="0"><tr><td width="56" height="39" align="center" valign="top" class="font"><p align="left"><span class="style3 style4 style5">&nbsp;备注：</span></p></td><td width="506" align="left" valign="top"><span class="style2 style3"><textarea name="beizhu" cols="67" rows="3" id="beizhu">套系:
后期:</textarea></span></td></tr></table></td></tr></table><p align="center"><input name="tianjia" type="submit" id="tianjia2" value="保存信息转到客户管理区" onClick="return chk();">
                &nbsp;&nbsp;
  <input name="tianjia" type="submit" id="tianjia" value="保存信息继续添加客人" onClick="return chk();"><br><br><a href="admin/ckkh.asp">老客户预约请点击</a></p><p>&nbsp;</p></form></td></tr><tr><td height="18">&nbsp;</td><td>&nbsp;</td><td width="26%">&nbsp;</td></tr></table></td></tr></table></body></html>