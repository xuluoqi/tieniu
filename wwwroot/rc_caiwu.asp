...
End If
End Sub
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>无标题文档</title><script src="Js/Calendar.js"></script><script language="javascript" src="inc/func.js" type="text/javascript"></script><link href="Css/calendar-blue.css" rel="stylesheet"><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><style type="text/css"><!--
.style3 {color: #FF0000}
--></style></head><body><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr bgcolor="#FFFFFF" class="df"><form action="rc_caiwu.asp?action=<%= request("action") %>" method="post" name="form1">
      <td width="54%" height="24"><div align="left">&nbsp;时间查询:
...
%>">
              ...
              <input name="chaxun" type="submit" id="chaxun" value="查询"></div></td><td width="22%">点击单号进入客户控制面版</td></form><td width="24%">&nbsp;[<a href="rc_caiwu.asp?chaxun=查询&fromtime=<%= date-2 %>&totime=<%= date-2 %>"><%= day(date-2) %></a>] [<a href="rc_caiwu.asp?chaxun=查询&fromtime=<%= date-1 %>&totime=<%= date-1 %>"><%= day(date-1) %></a>] [<a href="rc_caiwu.asp?chaxun=查询&fromtime=<%= date %>&totime=<%= date %>"><font color="#CC0033"><%= day(date) %></font></a>] [<a href="rc_caiwu.asp?chaxun=查询&fromtime=<%= date+1 %>&totime=<%= date+1 %>"><%= day(date+1) %></a>] [<a href="rc_caiwu.asp?chaxun=查询&fromtime=<%= date+2 %>&totime=<%= date+2 %>"><%= day(date+2) %></a>] </td></tr></table><%
set rs=server.CreateObject("adodb.recordset")
if request("fromtime")<>"" and request("totime")<>""then
rs.open "select * from save_money where times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"#+1 and userid='"&session("userid")&"' order by times desc",conn,1,1
else
rs.open "select * from save_money where userid='"&session("userid")&"' order by times desc",conn,1,1
end if
...
%><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td><form action="rc_caiwu.asp?action=edit" name="form1" method="post"><table width="100%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr bgcolor="#99FFFF"><td width="9%" height="17"><div align="center">单号</div></td><td width="31%"><div align="center">金额</div><div align="center"></div></td><td width="22%"><div align="center">日期</div></td><td width="38%"><div align="center">备注</div><div align="center"></div></td></tr><%
while not rs.eof
%><tr bgcolor="#FFFFFF"><td height="13"><div align="left">&nbsp;&nbsp;&nbsp;<a href="javascript:openEditScript('kehu_mianban.asp?id=<%= rs("xiangmu_id") %>',450,500)"><%= rs("xiangmu_id") %></a></div></td><td><div align="center"><font color="#FF0000"><%= rs("money") %></font>(元)</div><div align="center"></div></td><td><div align="center"><%= datevalue(rs("times")) %></div></td><td><div align="left">
                &nbsp;&nbsp;<%= rs("beizhu") %></div><div align="center"></div></td></tr><%
rs.movenext
wend
%></table><table width="100%" height="39"  border="0" cellpadding="0" cellspacing="0"><tr><td width="74%"><div align="center">共有记录<span class="style3"><%=  rs.recordcount %>&nbsp;</span>个,当前页<span class="style3"><%=  page %>/<%=  pgnm %></span>：
                  <%
If page>1 Then
%><a href="rc_caiwu.asp?page=<%=  page-1 %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="rc_caiwu.asp?page=<%= page+1 %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
%></div></td><td width="26%">&nbsp;
