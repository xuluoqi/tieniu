...
defshopvalue = GetMultipleShopListValue()
if not isnull(defshopvalue) then
sqlshop = " and k.shopid="&defshopvalue
shopname = GetMultipleShopName(defshopvalue)
end if
shopid = request.form("shopid")
if shopid<>"" and isnumeric(shopid) then
sqlshop = " and k.shopid="&shopid
shopname = GetMultipleShopName(shopid)
end if
...
%>
    ...
    <br><span class="STYLE5"><br></span><span class="STYLE4">结婚礼服出租表</span><br><br></center><table width="100%" height="25" border="1" cellspacing="0"><tr bgcolor="#66FFFF" align="center" height="25"><td height="25">单号</td><td><%= GetAppellation(3, null) %></td><td><%= GetAppellation(4, null) %></td><td><%= GetDutyName(5) %></td><td>结婚妆时间</td><td>预设返租日期</td><td>礼服项目</td></tr><%
set rs=server.CreateObject("adodb.recordset")
rs.open "select c.* from chuzhu_jilu c inner join kehu k on c.kehu_id=k.id where c.OutDate=#"&times&"#"&sqlshop&" and c.type=0 order by c.id",conn,1,1
while not rs.eof
set rsnon = conn.execute("select * from NonSuitWedOrder where wedorderid="&rs("id"))
if not (rsnon.eof and rsnon.bof) then
hzname = GetFieldDataBySQL("select peplename from yuangong where id="&rsnon("hz_userid"),"str","")
hztime = rsnon("hz_time")
end if
rsnon.close
set rsnon = nothing
set rsxxm = conn.execute("select yx_jhz_name,hz_time from shejixiadan where id="&rs("xiangmu_id"))
if not (rsxxm.eof and rsxxm.bof) then
hzname = rsxxm("yx_jhz_name")
hztime = rsxxm("hz_time")
end if
rsxxm.close
set rsxxm = nothing
%><tr align="center" bgcolor="#FFFFFF" onMouseOver="this.bgColor='#FF9966'" onMouseOut="this.bgColor='#FFFFFF'" <%
if len(rs("orderno"))<10 then
%> onClick="javascript:openEditScript('<%= pageurl %>?id=<%= rs("id") %>',450,500)"<%
end if
%> style="cursor:hand">
    <td  height="25"><%= rs("orderno") %></td><td>&nbsp;
        <%
...
%></td><td>&nbsp;<%= arr(1) %></td><td>&nbsp;<%= hzname %></td><td>&nbsp;<%= hztime %></td><td>&nbsp;<%= rs("InDate") %></td><td><table width="95%" border="0" cellspacing="0" cellpadding="0"><%
set rstype=server.CreateObject("adodb.recordset")
rstype.open "SELECT distinct h.type FROM huensha h inner join chuzhu_details d on h.id=d.AnnexWedID where d.orderid="&rs("id"),conn,1,1
do while not rstype.eof
%><tr><td><%
hstype = GetFieldDataBySQL("select hs_type from hs_type where id="&rstype("type"),"str","")
if hstype<>"" then response.write hstype&":&nbsp;"
set rshs = server.CreateObject("adodb.recordset")
'
sqlhs = "SELECT huensha.num, Chuzhu_Details.Volume FROM Chuzhu_Details INNER JOIN huensha ON Chuzhu_Details.AnnexWedID = huensha.id where Chuzhu_Details.OrderID="&rs("id")&" and huensha.type="&rstype("type")
rshs.open sqlhs,conn,1,1
do while not rshs.eof
'
'
response.write rshs("num")&"&nbsp;["&rshs("volume")&"]"
rshs.movenext
if not rshs.eof then response.write ",&nbsp;"
loop
rshs.close
set rshs = nothing
%></td></tr><%
...
rs.close
set rs=nothing
%></table><script language="javascript">loadingHidden();</script></body></html>