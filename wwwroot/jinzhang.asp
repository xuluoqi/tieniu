...
--></style><link href="zxcss.css" rel="stylesheet" type="text/css"><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><style type="text/css"><!--
.style1 {color: #000000}
.style3 {color: #FF0000}
...
</script></head><body><table width="100%" height="55" border="0" align="center" cellpadding="0" cellspacing="0" class="jh"><tr><td  height="24"><div align="center"><table width="100%" height="27" border="0" cellpadding="0" cellspacing="0"  class="jh"><form action="jinzhang.asp" method="post" name="form1"><tr bgcolor="#FFFFFF"><td width="80" height="22">时间查询：</td><td width="103"><input name="fromtime" type="text" id="fromtime" size="12" value="<%
...
%>"></td><td><input name="chaxun" type="submit" id="chaxun" value="查询"><div align="center"></div></td></tr></form></table></div></td></tr><tr><td height="28"><table width="100%" height="70" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFCCFF"><tr bgcolor="#FF9966"><td width="326" height="24"><div align="left">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;项目</div></td><td width="214"><div align="center">项目完成</div></td><td width="241"><div align="center">管理</div></td></tr><%
set rs2=server.CreateObject("adodb.recordset")
if request("chaxun")="查询"then
sql="select * from shejixiadan where  times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# and userid='"&session("userid")&"' order by shejiwancheng asc,times desc"
else
sql="select * from shejixiadan where userid='"&session("userid")&"' order by shejiwancheng asc,times desc"
end if
...
%><tr <%
if y mod 2=0 then
response.Write "bgcolor='#CC99FF'"
else
response.Write "bgcolor='#FFFFCC'"
end if
%>>
        <td height="22"><div align="left"><a href="javascript:openEditScript('ckkh.asp',750,350)"> 
		      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%= conn.execute("select jixiang from jixiang where id="&rs2("jixiang")&"")(0) %></a></div></td><td><div align="center"><%
if rs2("shejiwancheng")=1 then
response.Write "是"
else
response.Write"否"
end if
%></div></td><td><div align="center"><a href="jinzhang_look.asp?id=<%= rs2("id") %>">报表查看</a></div></td></tr><%
rs2.movenext
i=i+1
if i>=count then exit do
loop
%><tr bgcolor="#66CCFF"><td height="12" colspan="5" bgcolor="#9999FF"><div align="center">共有记录&nbsp;<font color="#FF0000"><%=  rs2.recordcount %></font>&nbsp;个　当前页<span class="style3"><%=  page %>/<%=  pgnm %></span>：
              <%
If page>1 Then
%><a href="jinzhang.asp?page=<%=  page-1 %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>&chaxun=<%= request("chaxun") %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="jinzhang.asp?page=<%= page+1 %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>&chaxun=<%= request("chaxun") %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
