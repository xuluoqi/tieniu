...
End If
End Sub
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>取件打印</title><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script language="javascript" src="inc/func.js" type="text/javascript"></script></head><body><%
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from shejixiadan where id="&request("id")&"",conn,1,1
%><TABLE height=20 cellSpacing=0 cellPadding=0 width="98%" align=center border=0><TBODY><TR><TD width="15%">&nbsp;&nbsp;单号:<%= rs("id") %></TD><TD width="16%">服务门市：<%= conn.execute("select peplename from yuangong where username='"&rs("kj_userid")&"'")(0) %>&nbsp;</TD><TD width="14%">客户：<%
response.write conn.execute("select lxpeple from kehu where id="&rs("kehu_id"))(0)
%></TD><TD width="17%"><%= year(rs("times")) %>年<%= month(rs("times")) %>月<%= day(rs("times")) %>日 </TD><TD width="38%"><%= getPrintString(rs("kehu_id")) %></TD></TR></TBODY></TABLE><table width="98%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr bgcolor="#efefef"><td height="21" bgcolor="#FFFFFF"><div align="left">&nbsp;&nbsp;加买服务&nbsp;&nbsp;加买总金额:
        <%
fujia_money=conn.execute("select sum(money) from fujia where xiangmu_id="&request("id")&" and datevalue(times)=#"&request("times")&"#")(0)
fujia2_money=conn.execute("select sum(money) from fujia2 where xiangmu_id="&request("id")&" and datevalue(times)=#"&request("times")&"#")(0)
if isnull(fujia_money) then fujia_money=0
if isnull(fujia2_money) then fujia2_money=0
response.Write fujia_money+fujia2_money
%>
        元&nbsp;&nbsp;&nbsp;
	截止&nbsp;<%= year(rs("times")) %>年<%= month(rs("times")) %>月<%= day(rs("times")) %>日&nbsp;&nbsp;&nbsp;&nbsp;
	<%
if not isnull(conn.execute("select sum(money) from save_money where xiangmu_id="&request("id")&"")(0)) then
...
if money5>=money1+money2+money3+money4 then
response.Write "已结清"
else
response.Write "总共未结款:"&money1+money2+money3+money4-money5&"元"
end if
%></div><div align="center"></div><div align="center"></div></td></tr><tr bgcolor="#FFFFFF"><td height="80" valign="top"><div align="left">&nbsp;
&nbsp;&nbsp;
<table width="96%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td><%
set rs2=conn.execute("select * from fujia where xiangmu_id="&request("id")&" and datevalue(times)=#"&request("times")&"#")
if not rs2.eof then
while not rs2.eof
response.Write conn.execute("select yunyong from yunyong where id="&rs2("jixiang")&"")(0)&" "
response.Write "金额:"&rs2("money")&"元&nbsp;&nbsp;备注："&encode(rs2("beizhu"))&"<br>"
rs2.movenext
wend
end if
rs2.close
set rs2=nothing
set rs2=conn.execute("select * from fujia2 where xiangmu_id="&request("id")&" and datevalue(times)=#"&request("times")&"#")
if not rs2.eof then
while not rs2.eof
response.Write conn.execute("select yunyong from yunyong where id="&rs2("jixiang")&"")(0)&" "
response.Write "金额:"&rs2("money")&"元&nbsp;&nbsp;备注："&encode(rs2("beizhu"))&"<br>"
...
set rs2=nothing
%>
      打单:<%= session("username") %><br></td></tr></table><br>
