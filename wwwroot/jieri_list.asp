...
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>无标题文档</title><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script language="javascript" src="inc/func.js" type="text/javascript"></script><style type="text/css"><!--
.style3 {color: #FF0000}
--></style></head><body><table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr bgcolor="#99FFFF" class="df"><form action="" method="post" name="form1"><td height="24"><div align="left">&nbsp;&nbsp;查询:
        <select name="month1" id="month1" onChange="check()"><option value="<%
if request.Form("month1")<>"" then
response.Write request.Form("month1")
else
response.Write month(now)-1
end if
%>">
          <%
if request.Form("month1")<>"" then
...
%></select>
        月
        <select name="day1" id="day1"><option value="<%
if request.Form("day1")<>"" then
response.Write request.Form("day1")
else
...
%></select>
        日---
        <select name="month2" id="month2" onChange="check()"><option value="<%
if request.Form("month2")<>"" then
response.Write request.Form("month2")
else
...
%></select>
月
<select name="day2" id="day2"><option value="<%
if request.Form("day2")<>"" then
response.Write request.Form("day2")
else
response.Write day(now)
end if
%>">
...
%></select>
日&nbsp;&nbsp;
<input name="chaxun" type="submit" id="chaxun" value="查询"></div></td></form><td>&nbsp;[<a href="jieri_list.asp?month1=<%= month(date) %>&day1=<%= day(date)-2 %>&month2=<%= month(date) %>&day2=<%= day(date)-2 %>"><%= day(date-2) %></a>] [<a href="jieri_list.asp?month1=<%= month(date) %>&day1=<%= day(date)-1 %>&month2=<%= month(date) %>&day2=<%= day(date)-1 %>"><%= day(date-1) %></a>] [<a href="jieri_list.asp?month1=<%= month(date) %>&day1=<%= day(date) %>&month2=<%= month(date) %>&day2=<%= day(date) %>"><font color="#CC0033"><%= day(date) %></font></a>] [<a href="jieri_list.asp?month1=<%= month(date) %>&day1=<%= day(date)+1 %>&month2=<%= month(date) %>&day2=<%= day(date)+1 %>"><%= day(date+1) %></a>] [<a href="jieri_list.asp?month1=<%= month(date) %>&day1=<%= day(date)+2 %>&month2=<%= month(date) %>&day2=<%= day(date)+2 %>"><%= day(date+2) %></a>] </td></tr></table><%
if request.Form("month1")<>"" then
...
conn.execute("delete from kehu_jieri where id="&request("id")&"")
end if
set rs=server.CreateObject("adodb.recordset")
if  request("month1")<>"" and request("day1")<>"" and request("menshi")="" then
rs.open "select * from kehu_jieri where month(times)>="&month1&" and day(times)>="&day1&" and month(times)<="&month2&" and day(times)<="&day2&" and kehu_id in (select id from kehu where userid='"&session("userid")&"') order by times asc",conn,1,1
else
rs.open "select * from kehu_jieri where kehu_id in (select id from kehu where userid='"&session("userid")&"') order by id asc",conn,1,1
end if
...
if not rs.eof then
%><table width="100%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr bgcolor="#99FFFF"><td width="5%" bgcolor="#99FFFF"><div align="center">客户ID</div></td><td width="8%" bgcolor="#99FFFF"><div align="center">客户姓名</div></td><td width="26%" bgcolor="#99FFFF"><div align="center">标题</div></td><td width="15%" bgcolor="#99FFFF"><div align="center">节日名称</div></td><td width="10%"><div align="center">节日日期</div></td><td width="8%"><div align="center">门市</div></td><td width="28%"><div align="center">管理</div></td></tr><%
do while not rs.eof
%><tr bgcolor="#FFFFFF"><td bgcolor="#FFFFFF"><div align="center"><%= rs("kehu_id") %></div></td><td bgcolor="#FFFFFF"><div align="center"><a href="javascript:openEditScript('ckkh2.asp?id=<%= rs("kehu_id") %>',800,350)"><%= conn.execute("select lxpeple from kehu where id="&rs("kehu_id")&"")(0) %></a></div></td><td bgcolor="#FFFFFF"><div align="center"><%= rs("title") %></div></td><td bgcolor="#FFFFFF"><div align="center"><%
Set J_Rs=conn.execute("select JName from JieRi_1 where id="&rs("jieri_id")&"")
if J_Rs.Eof then
response.write ""
else
response.write J_Rs("JName")
end if
J_Rs.close
set J_Rs=nothing
%></div></td><td><div align="center"><%= rs("times") %></div></td><td><div align="center"><%= conn.execute("select peplename from yuangong where username='"&conn.execute("select userid from kehu where id="&rs("kehu_id")&"")(0)&"'")(0) %></div></td><td><div align="center"><a href="javascript:openwindow('admin/heka_sj.asp?id=<%= rs("id") %>&id2=<%= rs("jieri_id") %>',800,800)">内容设计</a>&nbsp;<a href="javascript:openwindow('admin/xinfeng1_print.asp?id=<%= rs("kehu_id") %>',700,400)">封面打印</a><%
if not isnull(rs("content")) then
%><a href="javascript:openwindow('admin/jieri1_print.asp?id=<%= rs("id") %>',700,400)">贺词打印</a>1
	<a href="javascript:openwindow('admin/jieri2_print.asp?id=<%= rs("id") %>',700,400)">贺词打印</a>2
	<%
else
response.Write "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
end if
...
rs.movenext
ii=ii+1
if ii>=count then exit do
loop
%></table><%
end if
%><table width="96%" height="22"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td><div align="center">共有记录<span class="style3"><%=  rs.recordcount %>&nbsp;</span>个,当前页<span class="style3"><%=  page %>/<%=  pgnm %></span>：
          <%
If page>1 Then
%><a href="jieri_list.asp?page=<%=  page-1 %>&month1=<%= month1 %>&day1=<%= day1 %>&month2=<%= month2 %>&day2=<%= day2 %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="jieri_list.asp?page=<%= page+1 %>&month1=<%= month1 %>&day1=<%= day1 %>&month2=<%= month2 %>&day2=<%= day2 %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
%></div></td></tr></table><%
