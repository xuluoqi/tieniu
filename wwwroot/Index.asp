...
response.write "<script language=javascript>alert('出错！目前公历只支持1950-2-17至2051-2-10，错误会输出 1900-1-1')</script>"
GongToNong="1900-1-1"
Exit Function
End If
...
response.write "<script language=javascript>alert('出错！目前农历只支持1950-1-1至2050-12-29，错误会输出 1900-1-1')</script>"
NongToGong="1900-1-1"
Exit Function
End If
...
End Function
End Class
%><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"><html xmlns="http://www.w3.org/1999/xhtml"><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312" /><meta name="robots" content="noindex, nofollow"><title>博诚影楼管理系统  -- 婚庆通中国</title><style type="text/css"><!--
body { margin: 0px; background:url(images/v2010_bg.gif); }
.text1{ 
	font-size:16px; color:#333; width:130px; background-color:transparent; border:none; height:18px; line-height:16px; padding:0 3px 0 3px;margin-top:2px; height:23px
}
.STYLE1 { font-size: 12px; line-height:18px;}
.divAuth { float:left; position:absolute; margin:0px; width:100%; height:1024px; background-color:#FFFFFF; z-index:2; FILTER: alpha(opacity=60);}
.divContent { 
	float:left; position:absolute; margin-top:100px; margin-left:300px; width:400px;
	height:250px; z-index:3; background-color:#FFFFFF; border:dashed #333333 1px;
}
.divBody { width:100%; height:100%; }
div { font-size:12px; }
input { font-size:12px; border:solid #666666 1px; }
td{ font-size:12px }
area { anything:expression(this.onfocus=this.blur); }
--></style><script language="javascript" src="inc/func.js" type="text/javascript"></script><script language="javascript" src="inc/ajax.js" type="text/javascript"></script><script language="javascript" src="inc/jscript.js"></script><script language="javascript" src="inc/CheckAuthor.js" type="text/javascript"></script><SCRIPT LANGUAGE="VBScript"> 
...
newwin=window.open("chat/index.asp","new",
"width=600,height=500,fullscreen=0,toolbar=0,location=0,directories=0,status=1,menubar=0,scrollbars=0,resizable=1,left=0,top=0");
   newwin.moveTo(-3,-3);
   newwin.resizeTo(WidthValue+6,HeightValue-23);
}
function chkform(){
	if(!CheckIsNull(document.getElementById("username"),"请输入帐户名称！")) return false;
	if(document.getElementById("username").value=="帐户"){
		alert("请输入帐户名称！");
		return false;
	}
	if(!CheckIsNull(document.getElementById("password"),"请输入帐户密码！")) return false;
	return true;
}
function fullwindow(url)
{
	window.open(url,"","width="+(screen.availWidth-8)+",height="+(screen.availHeight-30)+",scrollbars=1,top=0,left=0")
}
function onKeyDown(){  
	if(document.getElementById("divAuthor").style.display=="none"){
		if(event.keyCode==13){
			if(!chkform()){
				return false;
			}
	...
	if(event.keyCode==113){
		//window.open('admin/ck08.asp','','fullscreen');
		//void(0);
		window.close();
	}
	if(event.keyCode==122){
		window.open('admin/CustomerLotto.asp','','fullscreen');
		void(0);
	}
	if(event.keyCode==123){
		ExecShell('D:\\接单系统\\公司简介.exe');
		void(0);
	}
} 
document.onkeydown=onKeyDown;
</script></head><body onLoad="InitializeAuthor()" scroll=no><div id="divAuthor" class="divBody" style="display:none"><div class="divAuth"></div><div class="divContent"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"><form id="formAuthor" name="formAuthor" method="post" action="" onsubmit="return false;"><tr><td height="30" colspan="2" bgcolor="#333333"><font color="white">&nbsp;* 此终端未经授权,请输入授权帐户资料</font></td></tr><tr><td height="30" align="right">&nbsp;</td><td height="30">&nbsp;</td></tr><tr><td width="27%" height="30" align="right">授权帐户:</td><td width="73%" height="30"><input name="txtUsername" type="text" id="txtUsername" size="30" /></td></tr></form></table></div></div><table border="0" align="center" cellpadding="0" cellspacing="0"><form action="chklogin.asp" method="post" name="form" id="form"><tr><td style="padding-top:60px"><img src="images/v2010_nav.gif" width="705" height="214" border="0" usemap="#Map" /></td></tr><tr><td height="110" style="background:url(images/v2010_logbg.gif) no-repeat; padding-left:90px"><table border="0" cellspacing="0" cellpadding="0"><tr><td width="143" height="28" style="background:url(images/v2010_un.gif) no-repeat; padding-left:85px"><input name="username" type="text" class="text1" id="username" /></td><td rowspan="3">&nbsp;</td><td rowspan="3"><input name="image" type="image" style="border:none" onclick="return chkform()" src="images/v2010_login.gif" width="71" height="64" /></td><td rowspan="3"><input name="action" type="hidden" id="action" value="login" /></td></tr><tr><td height="6"></td></tr><tr><td width="143" height="28" style="background:url(images/v2010_pw.gif) no-repeat; padding-left:85px"><input name="password" type="password"  class="text1" id="password" /></td></tr></table></td></tr><tr><td height="154" valign="top" style="background:url(images/v2010_bottom.gif) no-repeat"><table width="100%" border="0" cellspacing="0" cellpadding="3"><tr><td align="center" style="color:#FF0">&nbsp;</td></tr><tr><td align="center" style="color:#FF0"><%= year(date()) %>年积分第一名:&nbsp;暂无&nbsp;第二名:&nbsp;暂无&nbsp; &nbsp;今天员工寿星:&nbsp;<%
Dim StartTime, EndTime, rs, sql,DateType, counter
counter = 0
set rs = server.createobject("adodb.recordset")
StartTime = Date()
EndTime = Date()
DateType = 1
Dim ChinaDay
Set ChinaDay = New ClsDay
if DateType=0 then
nStartTime = StartTime
nEndTime = EndTime
gStartTime = ChinaDay.NongToGong(StartTime,"")
gEndTime = ChinaDay.NongToGong(EndTime,"")
else
nStartTime = ChinaDay.GongToNong(StartTime)
nEndTime = ChinaDay.GongToNong(EndTime)
gStartTime = StartTime
gEndTime = EndTime
end if
Set ChinaDay = nothing
if right(CStr(nStartTime),4)<>"2-29" and right(CStr(nStartTime),4)<>"2-30" then
sql="Select * From [yuangong] Where 1=1 and birthday<>'' and not isnull(birthday) and ((datevalue('"&year(date())&"-'+iif(len(birthday)>5,mid(birthday,6),birthday))>=#"&nStartTime&"# and datevalue('"&year(date())&"-'+iif(len(birthday)>5,mid(birthday,6),birthday))<=#"&nEndTime&"# and birthdaytype=0) or (datevalue('"&year(date())&"-'+iif(len(birthday)>5,mid(birthday,6),birthday))>=#"&gStartTime&"# and datevalue('"&year(date())&"-'+iif(len(birthday)>5,mid(birthday,6),birthday))<=#"&gEndTime&"# and birthdaytype=1)) and birthday<>'' and not isnull(birthday) Order by birthday desc "
rs.open sql,conn,1,1
if not (rs.eof and rs.bof) then
do while not rs.eof
response.write rs("peplename")
rs.movenext
...
%></td></tr><tr><td align="center" style="color:#FF0">今天客人寿星:&nbsp;<%
if right(CStr(nStartTime),4)<>"2-29" and right(CStr(nStartTime),4)<>"2-30" then
sql = "Select * From [kehu] Where 1=1 and (( chusheng<>'' and not isnull(chusheng) and ((datevalue('"&year(date())&"-'+iif(len(chusheng)>5,mid(chusheng,6),chusheng))>=#"&nStartTime&"# and datevalue('"&year(date())&"-'+iif(len(chusheng)>5,mid(chusheng,6),chusheng))<=#"&nEndTime&"# and ShengriType=0) or (datevalue('"&year(date())&"-'+iif(len(chusheng)>5,mid(chusheng,6),chusheng))>=#"&gStartTime&"# and datevalue('"&year(date())&"-'+iif(len(chusheng)>5,mid(chusheng,6),chusheng))<=#"&gEndTime&"# and ShengriType=1)) and chusheng<>'' and not isnull(chusheng)) or ( chusheng2<>'' and not isnull(chusheng2) and ((datevalue('"&year(date())&"-'+iif(len(chusheng2)>5,mid(chusheng2,6),chusheng2))>=#"&nStartTime&"# and datevalue('"&year(date())&"-'+iif(len(chusheng2)>5,mid(chusheng2,6),chusheng2))<=#"&nEndTime&"# and ShengriType2=0) or (datevalue('"&year(date())&"-'+iif(len(chusheng2)>5,mid(chusheng2,6),chusheng2))>=#"&gStartTime&"# and datevalue('"&year(date())&"-'+iif(len(chusheng2)>5,mid(chusheng2,6),chusheng2))<=#"&gEndTime&"# and ShengriType2=1)) and chusheng2<>'' and not isnull(chusheng2))) Order by id asc"
rs.open sql,conn,1,1
if not (rs.eof and rs.bof) then
do while not rs.eof
dim arr_kehu_bir,arr_datetype
counter = counter + 1
arr_kehu_bir = split("chusheng,chusheng2",",")
arr_datetype = split("ShengriType,ShengriType2",",")
if rs(arr_kehu_bir(0))<>"" and not isnull(arr_kehu_bir(0)) then
newdatetime=rs(arr_kehu_bir(0))
if len(newdatetime)>5 then newdatetime=mid(newdatetime,6)
if rs(arr_datetype(0))=0 then
if cdate(nStartTime)<=cdate(year(nStartTime)&"-"&newdatetime) and cdate(nEndTime)>=cdate(year(nEndTime)&"-"&newdatetime) then
cur_name = rs("lxpeple")
'
'
'
...
if nStartTime2<>"" then
if cdate(nStartTime2)<=cdate(year(nStartTime2)&"-"&newdatetime) and cdate(nEndTime2)>=cdate(year(nEndTime2)&"-"&newdatetime) then
cur_name = rs("lxpeple")
'
'
'
...
if cdate(gStartTime)<=cdate(year(gStartTime)&"-"&newdatetime) and cdate(gEndTime)>=cdate(year(gEndTime)&"-"&newdatetime) then
cur_name = rs("lxpeple")
'
'
'
...
if gStartTime2<>"" then
if cdate(gStartTime2)<=cdate(year(gStartTime2)&"-"&newdatetime) and cdate(gEndTime2)>=cdate(year(gEndTime2)&"-"&newdatetime) then
cur_name = rs("lxpeple")
'
'
'
...
if rs(arr_kehu_bir(1))<>"" and not isnull(rs(arr_kehu_bir(1))) then
newdatetime=rs(arr_kehu_bir(1))
if len(newdatetime)>5 then newdatetime=mid(newdatetime,6)
if rs(arr_datetype(1))=0 then
if cdate(nStartTime)<=cdate(year(nStartTime)&"-"&newdatetime) and cdate(nEndTime)>=cdate(year(nEndTime)&"-"&newdatetime) then
cur_name = rs("lxpeple2")
'
'
'
end if
...
rs.movenext
response.write cur_name
if counter>=6 then
if counter<rs.recordcount then response.write "&nbsp;..."
exit do
else
if not rs.eof then response.write ", "
end if
...
%></td></tr><tr>
  <td align="center">中国影楼联盟&nbsp;(品牌影楼联盟)&nbsp;&nbsp; <a href="http://www.ps321.net" title="中国影楼联盟" target="_blank" style="color:#000; text-decoration:none">http://www.ps321.net</a></td>
</tr><tr>
  <td align="center">电话:130 0817 8495&nbsp;/ 邮箱:amdx88@163.com</td>
</tr></table></td></tr></form></table><map name="Map" id="Map"><area shape="rect" coords="77,121,148,208" href="javascript:" onclick="javascript:popwindow('neworder', 'admin/CustomerLotto.asp', 900, 600);void(0);" /><area shape="rect" coords="173,121,245,208" alt="团队" href="javascript:" onClick="vbscript:ExecShell('D:\\接单系统\\公司简介.exe')" /><area shape="rect" coords="270,121,343,208" alt="查件" href="javascript:" onclick="javascript:fullwindow('admin/lc_look.asp');void(0);" /><area shape="rect" coords="368,121,440,208" href="javascript:" onclick="javascript:openwindow('lcimg.html',1024,600);void(0);" /></map></body></html>