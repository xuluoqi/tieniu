...
End If
End Sub
losttype=request("losttype")
if losttype<>"1" then
losttype=0
else
losttype=1
end if
dim DataAnalyzeInvis
DataAnalyzeInvis = GetDataAnalyzeState()
function check_sjs()
if session("level")=2 or session("level")=4 or session("level")=5 or session("level")=11 or session("level")=12 or session("level")=13 or session("level")=14 then
check_sjs=true
else
check_sjs=false
end if
end function
if request("dele_id")<>""then
if conn.execute("select times from kehu where id="&request("dele_id")&"")(0)<date then
response.write "<script>alert('对不起，权限不足，无法执行此操作!');location='xiugai2.asp'</script>"
On Error GoTo 0
Err.Raise 9999
end if
conn.execute("delete from kehu where id="&request("dele_id")&"")
response.Redirect "xiugai2.asp"
On Error GoTo 0
Err.Raise 9999
end if
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>客户管理</title><link href="images/hs.css" rel="stylesheet" type="text/css"><style type="text/css"><!--
.style3 {color: #FF0000}
...
--></style><script src="Js/Calendar.js"></script><script language="javascript" src="inc/func.js" type="text/javascript"></script><link href="Css/calendar-blue.css" rel="stylesheet"><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><style type="text/css"><!--
.style17 {color: #003399}
.style1 {color: #000000}
--></style></head><body><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"><tr bgcolor="#FFFFFF"><form action="xiugai2.asp" method="post" name="form1"><td width="49%" height="22">&nbsp;&nbsp;条件查询
        :
        <select name="tiaojian" id="tiaojian"><option value="<%= request("tiaojian") %>">
            <%
if request("tiaojian")<>"" then
response.Write request("tiaojian")
else
response.Write "请选择"
end  if
%></option><option value="联系人" <%
if check_sjs() then response.write "selected"
%>>联系人</option><option value="卡号">卡号</option><option value="系统单号" <%
if not check_sjs() then response.write "selected"
%>>系统单号</option><option value="预约单号">预约单号</option><option value="联系电话">联系电话</option><option value="地址">地址</option><option value="备注">备注</option></select><input name="content" type="text" id="content" size="12" value="<%= request("content") %>">
        
        <input name="chaxun" type="submit" id="chaxun" value="查询"><input type="hidden" name="hid" value="<%= request("chaxun") %>"></td></form><td width="30%">&nbsp;&nbsp;&nbsp;&nbsp;请点击<span class="style3">预约信息</span>进入客户面版操作&nbsp;&nbsp;&nbsp;&nbsp;<span class="style17"><a href="javascript:openwindow('admin/lc_look.asp',750,450)"></a></span></td><td width="11%"><a href="admin/sms_receive.asp">接收短信</a></td><td width="10%"><%
if not check_sjs() then
if losttype<>"" and losttype=1 then
response.write "<a href='?losttype=0'><b>客户列表</b></a>"
else
response.write "<a href='?losttype=1'><b>流失客户</b></a>"
end if
end if
%></td></tr><%
if not check_sjs() then
%><tr bgcolor="#FFFFFF"><form action="xiugai2.asp" method="post" name="form2"><td height="22" colspan="4">&nbsp;&nbsp;时间查询
        <input name="fromtime" type="text" id="fromtime2" size="10" value="<%
...
%>" readonly>
          <span class="font"><a onClick="return showCalendar('fromtime', 'y-mm-dd');" href="#"><img src="Image/Button.gif" id="IMG2" align="absMiddle" border="0" /></a></span>到：
        <input name="totime" type="text" id="totime2" size="10" value="<%
...
%>" readonly>
          <span class="font"><a onClick="return showCalendar('totime', 'y-mm-dd');" href="#"><img src="Image/Button.gif" id="IMG2" align="absMiddle" border="0" /></a></span>&nbsp;&nbsp;
          <select name="address" id="address"><option value="<%= request("address") %>">
            <%
...
if request("content")<>"" and request("tiaojian")="联系人"  then
sql="select * from kehu  where 1=1 "&SQL_user&" and instr(1,LCASE(lxpeple&lxpeple2),LCASE('"&request("content")&"'),0)<>0 order by times desc "
elseif request("content")<>"" and request("tiaojian")="卡号"  then
sql="select * from kehu where 1=1 "&SQL_user&" and number='"&request("content")&"' "
elseif request("content")<>"" and request("tiaojian")="系统单号"  then
if not isnumeric(request("content")) then
response.Write "<script>alert('对不起，单号填写错误！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
count1=conn.execute("select count(*) from shejixiadan where id="&request("content")&SQL_user)(0)
if count1=0 then
response.Write "<script>alert('对不起，没有查到该单号！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
else
kehu_id=conn.execute("select kehu_id from shejixiadan where id="&request("content")&"")(0)
sql="select * from kehu where id="&kehu_id
end if
elseif request("content")<>"" and request("tiaojian")="预约单号"  then
count1=conn.execute("select count(*) from shejixiadan where danhao='"&request("content")&"'"&SQL_user)(0)
if count1=0 then
response.Write "<script>alert('对不起，没有查到该单号！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
else
kehu_id=conn.execute("select kehu_id from shejixiadan where danhao='"&request("content")&"'")(0)
sql="select * from kehu where id="&kehu_id
end if
elseif request("tiaojian")="联系电话" then
sql="select * from kehu where telephone&telephone2 like '%"&request("content")&"%' order by id desc"
elseif request("tiaojian")="地址" then
sql="select * from kehu where 1=1 "&SQL_user&" and address&address2 like '%"&request("content")&"%' order by times desc"
elseif request("tiaojian")="备注" then
sql="select * from kehu where 1=1 "&SQL_user&" and shuoming like '%"&request("content")&"%' order by times desc"
elseif request("fromtime")<>"" and request("totime")<>"" and request("address")="" then
sql="select * from kehu where 1=1 "&SQL_user&" and times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# order by times desc"
elseif request("fromtime")<>"" and request("totime")<>"" and request("address")<>"" then
sql="select * from kehu where (city='"&request("address")&"' or city2='"&request("address")&"') and "&SQL_user&" and times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# order by times desc"
else
if losttype=1 then
sql="select * from kehu where 1=1 "&SQL_user&" and islost=1 order by times desc"
else
if not check_sjs() then
sql="select * from kehu where 1=1 "&SQL_user&" and islost=0 order by times desc"
end if
end if
end if
if sql<>"" then
...
if request("page1")<>"" then response.Redirect "xiugai2.asp?page="&request("page1")
DO While not rs.eof
y=y+1
%><table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bordercolor="#A4B6D7" bgcolor="#A4B6D7" class="df"><tr <%
if y mod 2=0 then
response.Write "bgcolor=#efefef"
else
response.Write "bgcolor=#66FFFF"
end if
%>>
      <td width="188" height="22">&nbsp;&nbsp;&nbsp;<%= rs("lxpeple") %>[<%= rs("sex") %>] &nbsp;
          <%
if rs("chusheng")<>"" then response.Write rs("chusheng")
%></td><td width="247" height="22">&nbsp; 电话:
        <%
if trim(rs("telephone"))="" then
response.Write "无"
else
response.Write GetTelNo(rs("telephone"))
end if
if trim(rs("home_tel"))<>"" then response.Write "&nbsp;&nbsp;"&GetTelNo(rs("home_tel"))
%></td><td width="342" height="22">&nbsp;&nbsp;地址:<%= rs("address") %></td></tr><%
if trim(rs("lxpeple2"))<>"" then
%><tr <%
if y mod 2=0 then
response.Write "bgcolor=#efefef"
else
...
%>>
      <td height="22">&nbsp;&nbsp;&nbsp;<%= rs("lxpeple2") %>[<%= rs("sex2") %>] &nbsp;
          <%
if rs("chusheng2")<>"" then response.Write rs("chusheng2")
%></td><td height="22">&nbsp;&nbsp;电话:
        <%
if trim(rs("telephone2"))="" then
response.Write "无"
else
response.Write GetTelNo(rs("telephone2"))
end if
if trim(rs("home_tel2"))<>"" then response.Write "&nbsp;&nbsp;"&GetTelNo(rs("home_tel2"))
%></td><td height="22">&nbsp;&nbsp;地址:<%= rs("address2") %></td></tr><%
end if
%><tr <%
if y mod 2=0 then
response.Write "bgcolor=#efefef"
...
%>
        &nbsp;&nbsp;&nbsp;
        <%
if conn.execute("select num from heka ")(0)=1 then
%><a href="admin/jilu.asp?id=<%= rs("id") %>">
          <%
elseif conn.execute("select num from heka ")(0)=2 then
%></a><a href="admin/jilu2.asp?id=<%= rs("id") %>">
            <%
end if
%></a>&nbsp;&nbsp;&nbsp;<a href="admin/hs_history.asp?khid=<%= rs("id") %>"><font color="#D200D2"><b>礼服预租</b></font></a>&nbsp;&nbsp;&nbsp;
        <%
if not check_sjs() then
%>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="xiadan.asp?id=<%= rs("id") %>"><strong><span class="style3">&gt;新预约</span></strong></a>&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp; <a href="javascript:openwindow('admin/send.asp?id=<%= rs("id") %>',650,450)">短信</a>&nbsp;&nbsp; <a href="javascript:openwindow('admin/xinfeng1_print.asp?id=<%= rs("id") %>',700,400)">信封</a>&nbsp;
        <%
if rs("times")>=date and conn.execute("select count(*) from shejixiadan where kehu_id="&rs("id")&"")(0)<=0 then
%><a href="xiugai2.asp?dele_id=<%= rs("id") %>" onClick="javascript:return confirm('确定要删除吗?')">删除</a><%
end if
end if
%>
        &nbsp;&nbsp;添加日期:<%= datevalue(rs("times")) %> &nbsp;&nbsp;
        <%
if rs("userid")<>session("userid") and rs("userid")<>"" then response.Write "[同事:<font color=blue>"&conn.execute("select peplename from yuangong where username='"&rs("userid")&"'")(0)&"</font>]"
if rs("userid2")<>session("userid") and rs("userid2")<>"" then response.Write "[同事:<font color=blue>"&conn.execute("select peplename from yuangong where username='"&rs("userid2")&"'")(0)&"</font>]"
if rs("userid3")<>session("userid") and rs("userid3")<>"" then response.Write "[同事:<font color=blue>"&conn.execute("select peplename from yuangong where username='"&rs("userid3")&"'")(0)&"</font>]"
%>
        &nbsp;&nbsp;
        <%
if not check_sjs() then
%><a href="kehu_xiugai.asp?id=<%= rs("id") %>&url3=xiugai2.asp&page=<%= page %>" ><strong>修改</strong></a><%
end if
%></td></tr><%
set rs2=server.CreateObject("adodb.recordset")
rs2.open "select * from shejixiadan where kehu_id="&rs("id")&"" ,conn,1,1
while not rs2.eof
%><tr bgcolor="#FFFFFF"><td height="22" colspan="4"><table width="100%"  border="0" cellspacing="0" cellpadding="0" <%
if y mod 2=0 then
response.Write "bgcolor=#efefef"
else
response.Write "bgcolor=#66FFFF"
...
%>>
          <tr><td>&nbsp;&nbsp;&nbsp;<a href="javascript:openEditScript('<%
if not check_sjs() then
response.write "kehu_mianban.asp"
else
response.write "shejishi/kehu_mianban.asp"
end if
%>?id=<%= rs2("id") %>',450,500)"><%= rs2("id") %><%
if rs2("danhao")<>"" then response.Write "/"&rs2("danhao")
%>
              [
              <%
response.Write  conn.execute("select jixiang from jixiang where id="&rs2("jixiang")&"")(0)
%>
              ]<span class="style3">预约信息</span></a>&nbsp;&nbsp;&nbsp;&nbsp; <span class="style1"><%
fujia2_money=conn.execute("select sum(money) from fujia2 where xiangmu_id="&rs2("id")&"")(0)
if isnull(fujia2_money) then fujia2_money=0
goumai_money=conn.execute("select sum(money) from goumai where xiangmu_id="&rs2("id")&"")(0)
if isnull(goumai_money) then goumai_money=0
if not isnull(conn.execute("select sum(money) from fujia where xiangmu_id="&rs2("id")&"")(0)) then
money1=conn.execute("select sum(money) from fujia where xiangmu_id="&rs2("id")&"")(0)
else
money1=0
end if
if not isnull(conn.execute("select sum(money) from fujia2 where xiangmu_id="&rs2("id")&"")(0)) then
money2=conn.execute("select sum(money) from fujia2 where xiangmu_id="&rs2("id")&"")(0)
else
money2=0
end if
if not isnull(conn.execute("select sum(money) from goumai where xiangmu_id="&rs2("id")&"")(0)) then
money3=conn.execute("select sum(money) from goumai where xiangmu_id="&rs2("id")&"")(0)
else
money3=0
end if
money4=rs2("jixiang_money")
money=money1+money2+money3+money4
savemoney=conn.execute("select sum(money) from save_money where xiangmu_id="&rs2("id"))(0)
if isnull(savemoney) then savemoney=0
no_save=money-savemoney
if DataAnalyzeInvis=0 then response.write "总消费："&money&"元"
%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 总未缴：<%= no_save %> 元&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 摄影日期：
                <%
if isnull(rs2("pz_time")) then
response.write "未设定"
else
response.write rs2("pz_time")
end if
%>
                &nbsp;&nbsp;&nbsp;&nbsp;
                <%
set rsvote = server.createobject("adodb.recordset")
rsvote.open "select kyflag from vote where xiangmu_id="&rs2("id"),conn,1,1
if not rsvote.eof then
if rsvote(0)=1 then response.write "是否客怨：是"
end if
rsvote.close
set rsvote=nothing
...
rs2.close
set rs2=nothing
%></table><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td background="Image/02.jpg" height="5"></td></tr></table><%
rs.movenext
i=i+1
if i>=count then exit do
loop
'
if DataAnalyzeInvis=0 then
%><table width="100%" border="0" cellpadding="0" cellspacing="0"><tr><form action="" method="post" name="form3"><td height="26"><div align="center">共有顾客&nbsp;<span class="style3"><%=  rs.recordcount %>&nbsp;</span>个,当前页<span class="style3"><%=  page %>/<%=  pgnm %></span>：
        <%
If page>1 Then
%><a href="xiugai2.asp?page=<%=  page-1 %>&hid=<%= request("hid") %>&tiaojian=<%= request("tiaojian") %>&content=<%= request("content") %>&losttype=<%= losttype %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="xiugai2.asp?page=<%= page+1 %>&hid=<%= request("hid") %>&tiaojian=<%= request("tiaojian") %>&content=<%= request("content") %>&losttype=<%= losttype %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
%><input name="page1" type="text" id="page1" size="2"><input type="submit" name="Submit" value="go"><input name="losttype" type="hidden" id="losttype" value="<%= losttype %>">
