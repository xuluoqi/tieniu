...
End If
End Sub
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>自动排单系统</title><%
...
%><link href="Css/calendar-blue.css" rel="stylesheet"><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script language="javascript" src="inc/func.js" type="text/javascript"></script><script src="Js/Calendar.js"></script><script language="javascript">
function changetype(type){
	switch (type) {
		case "yx":
			document.all.type.value="yx";
			document.all.form1.action="mxa_print.asp";
			document.all.sp_time1.style.display="";
			document.all.sp_time2.style.display="none";
			break;
		case "pd":
			document.all.type.value="pd";
			document.all.form1.action="mxb_print.asp";
			document.all.sp_time1.style.display="";
			document.all.sp_time2.style.display="none";
			break;
		case "lf":
			document.all.type.value="lf";
			document.all.form1.action="mxd_print.asp";
			document.all.sp_time1.style.display="";
			document.all.sp_time2.style.display="none";
			break;
		case "job":
			document.all.type.value="job";
			document.all.form1.action="mxc_print.asp";
			document.all.sp_time1.style.display="none";
			document.all.sp_time2.style.display="";
			break;  
	}
	document.all.form1.submit();
}
function changedate(val){
	var currdate = document.getElementById("fromtime");
	var date = new Date();
	if(val==0){
...
function ShowDatecolor(){
	var result = OW("admin/sys_config_datecolor.asp?sign="+Math.random(),"modal");
	if(result!=undefined && result!="" && result.indexOf('|')>0){
		location.reload();
	}
}
function PaixiuSettings(){
	var currdate = document.getElementById("fromtime").value;
	var result = OW("admin/sys_config_paixiu.asp?dates="+currdate+"&sign="+Math.random(),"modal");
	if(result!=undefined && result=="saved"){
		document.frames.main11.location.reload();
	}
}
</script></head><body leftmargin="0" topmargin="0" scroll=no style="height:100%"><table width="100%" height="100%"  border="0" cellpadding="0" cellspacing="0"><tr><td height="30"><table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><form action="mxb_print.asp" method="get" name="form1" target="main11" id="form1"><td width="73%" height="25">&nbsp;
        <%
Call ShowMultipleShopSelect(0,GetMultipleShopListValue(),true)
%>
        &nbsp;&nbsp;<span id="sp_time1">时间查询
          <input name="fromtime" type="text" id="fromtime" size="12" value="<%
fromtime=request("fromtime")
if fromtime="" then fromtime=date()
response.write fromtime
%>" /><a onClick="return showCalendar('fromtime', 'y-mm-dd');" href="#"><img src="Image/Button.gif" width="25" height="17" border="0" align="absmiddle" id="IMG2" /></a></span><span id="sp_time2" style="display:none">选择年份：
            <select name="year" id="year"><option value="<%
...
%></select>
            选择月份：
            <select name="month" id="month"><option value="<%
...
next
%></select></span>
        &nbsp;<input name="chaxun" type="submit" id="chaxun" value="确定" /><input name="type" type="hidden" id="type" />
        &nbsp; <a href="javascript:void(0);" onClick="javascript:changetype('pd')">自动排单系统</a>&nbsp;&nbsp;<%
if CheckUserPermission("730") then
%><a href="javascript:void(0);" onClick="javascript:changetype('yx')">预设人员安排</a> &nbsp;&nbsp;<%
end if
%><a href="javascript:void(0);" onClick="javascript:changetype('lf')">礼服出租表</a><%
if session("level")=6 or CheckUserPermission("725") then
%>
        &nbsp;&nbsp;<a href="javascript:void(0);" onClick="javascript:changetype('job')">工作月报表</a><%
end if
if session("level")=10 then
%>
        &nbsp; &nbsp;&nbsp; <a href="javascript:;" onClick="ShowDatecolor();">颜色管理</a><%
end if
if session("level")=10 or (session("zhuguan")=1 and (session("level")=7 or session("level")=6 or session("level")=1 or session("level")=4 or session("level")=5 or session("level")=2 or session("level")=13)) then
%>
        &nbsp;&nbsp;<a href="javascript:;" onClick="PaixiuSettings();">排休管理</a><%
end if
%></td><td width="22%"><input name="predate" type="button" id="predate" value="前一天" onClick="javascript:changedate(-1)" /><input name="today" type="button" id="today" value="今天" onClick="javascript:changedate(0)" /><input name="nextdate" type="button" id="nextdate" value="后一天" onClick="javascript:changedate(1)" /></td><td width="5%"><a href="#" onClick="document.main11.focus();document.main11.print();">打印</a></td></form></tr></table></td></tr><tr><td valign="top"><iframe name="main11" src="mxb_print.asp?fromtime=<%= fromtime %>" frameborder="0" width="100%" height="100%"></iframe></td></tr></table></body></html>