...
End If
End Sub
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>无标题文档</title><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script language="javascript" src="inc/func.js" type="text/javascript"></script></head><body><%
select case request("action")
case "sp_out"
if request("beizhu")="" then
response.Write "<script>alert('请填写备注说明！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
sp_id=request("sp_id")
...
sl31=split(sl3,",")
sp_id2=split(sp_id,",")
if ubound(sl2)<>ubound(id2) then
response.Write "<script>alert('填写数量错误！');history.go(-1)</script>"
else
for i=lbound(id2) to ubound(id2)
conn.execute("update kucuen set sl=sl+"&sl31(i)&" where id="&sp_id2(i)&"")
next
conn.execute("delete from cuenchu where xiangmu_id="&request("id2")&" and type="&"2"&"")
for i=lbound(id2) to ubound(id2)
if not isnumeric(sl2(i)) then
response.Write "<script>alert('填写数量错误!');history.go(-1)</script>"
else
if sl2(i)<>0 then
conn.execute("update kucuen set sl=sl-"&sl2(i)&" where id="&sp_id2(i)&"")
conn.execute("insert into cuenchu (xiangmu_id,sp_id,sl,beizhu,type,times) values ("&request("id2")&","&sp_id2(i)&","&sl2(i)&",'"&request("beizhu")&"',"&"2"&",#"&now&"#)")
end if
end if
next
end if
response.Write "<script>alert('添加成功！');location='kucuen.asp?id="&request("id2")&"'</script>"
case "add"
%><br><br><form action="kucuen.asp?action=sp_out" method="post" name="form1"><table width="73%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><%
set rs3=server.CreateObject("adodb.recordset")
rs3.open "select * from kucuen where type=1",conn,1,1
while not rs3.eof
%><tr bgcolor="#FFFFFF"><td width="57%"><div align="left">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;类别：<%= rs3("sp_name") %><input name="sp_id" type="hidden" id="sp_id" value="<%= rs3("id") %>">
    </div></td><td width="43%"> &nbsp;数量：
      <input name="sl" type="text" id="sl" size="9"value="<%
if conn.execute("select count(*) from cuenchu where xiangmu_id="&request("id")&" and sp_id="&rs3("id")&"")(0)>0 then
response.Write conn.execute("select sl from cuenchu where xiangmu_id="&request("id")&" and sp_id="&rs3("id")&"")(0)
else
response.Write "0"
end if
%>">
(件)
<input name="id" type="hidden" id="id" value="<%= rs3("id") %>">
<input name="sl3" type="hidden" id="sl3" value="<%
if conn.execute("select count(*) from cuenchu where xiangmu_id="&request("id")&" and sp_id="&rs3("id")&"")(0)>0 then
response.Write conn.execute("select sl from cuenchu where xiangmu_id="&request("id")&" and sp_id="&rs3("id")&"")(0)
else
response.Write "0"
...
rs3.close
set rs3=nothing
%><tr bgcolor="#FFFFFF"><td colspan="2"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="12%" valign="top"><div align="right">备注说明：</div></td><td width="88%"><textarea name="beizhu" cols="65" rows="8" id="beizhu"><%
if conn.execute("select count(*) from cuenchu where xiangmu_id="&request("id")&"")(0)>0 then
response.Write conn.execute("select beizhu from cuenchu where xiangmu_id="&request("id")&"")(0)
end if
%></textarea></td></tr></table></td></tr></table><table width="73%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td height="39"><div align="center"><input type="submit" name="Submit" value="提交">
      &nbsp;&nbsp;&nbsp;&nbsp;
      <input type="button" name="Submit" value="返回" onClick="history.go(-1)"><input name="id2" type="hidden" id="id2" value="<%= request("id") %>">
    </div></td></tr></table></form><%
case "list"
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from cuenchu where id="&request("id")&"",conn,1,1
%><table width="80%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr bgcolor="#FFFFFF"><td width="50%"><div align="left">&nbsp;&nbsp;物品：<%= conn.execute("select sp_name from kucuen where id="&rs("sp_id")&"")(0) %></div></td><td width="50%"><div align="center">数量：<%= rs("sl") %>(件)</div></td></tr><tr bgcolor="#FFFFFF"><td colspan="2"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="11%" valign="top"><div align="right">备注说明：</div></td><td width="89%">&nbsp;&nbsp;<%= encode(rs("beizhu")) %></td></tr></table></td></tr></table><table width="80%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td height="39"><div align="center"><a href="#" onClick="window.close()">关闭</a></div></td></tr></table><%
case else
%><br><table width="90%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr bgcolor="#99FFFF"><td width="75%"><div align="left">&nbsp;&nbsp;&nbsp;
            <%
if conn.execute("select shejiwancheng from shejixiadan where id="&request("id")&"")(0)=0 then
%><a href="kucuen.asp?action=add&id=<%= request("id") %>">添加物品</a><%
end if
%></div></td><td width="25%"><div align="right"><a href="caiwu.asp"> 返回</a>&nbsp;&nbsp;&nbsp;&nbsp;</div></td></tr></table><%
set rs=server.CreateObject("adodb.recordset")
rs.open"select * from cuenchu where xiangmu_id="&request("id")&"",conn,1,1
if not rs.eof then
%><table width="90%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr bgcolor="#99FFFF"><td width="43%"><div align="center">类别</div></td><td width="21%"><div align="center">数量</div></td><td width="26%"><div align="center">时间</div></td><td width="10%"><div align="center">详细查看</div></td></tr><%
while not rs.eof
%><tr bgcolor="#FFFFFF"><td><div align="center"><%= conn.execute("select sp_name from kucuen where id="&rs("sp_id")&"")(0) %></div></td><td><div align="center"><%= rs("sl") %></div></td><td><div align="left">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%= rs("times") %></div></td><td><div align="center"><a href="javascript:openEditScript('kucuen.asp?action=list&id=<%= rs("id") %>',750,350)">详细</a></div></td></tr><%
rs.movenext
wend
%></table><%
end if
