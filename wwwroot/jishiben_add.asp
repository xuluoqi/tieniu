...
set rs=server.CreateObject("adodb.recordset")
sql="select * from jishiben "
rs.open sql,connstr,2,3
rs.addnew
rs("title")=request("title")
rs("neirong")=neirong
rs("shuoming")=HTMLEncode2(request("shuoming"))
rs("yewuyuan")=session("username")
rs("times")=now()
rs.update
rs.close
set rs=nothing
response.Write "<script>alert('记事成功!'); location='jishiben.asp'</script>"
end if
%><p>&nbsp;</p><table width="712" height="386" border="0" align="center" cellpadding="0" cellspacing="0"><form action="" method="post" name="form1" onSubmit="return check()"><tr><td height="30" colspan="2"><div align="center" class="style1"></div><div align="center"><span class="style1">日记添加</span></div></td></tr><tr><td width="119" height="20"><div align="right">标题：</div></td><td width="560"><input name="title" type="text" id="title" size="56" value="<%= month(now) %>月<%= day(now) %>日工作日记"></td></tr><tr><td height="137"><div align="right">今日工作汇报: <BR></div></td><td><textarea name="neirong" cols="80" rows="8" id="neirong"></textarea></td></tr><tr><td height="79"><div align="right">公司配合说明:</div></td><td><textarea name="shuoming" cols="80" rows="8" id="shuoming"></textarea></td></tr><tr><td height="79">&nbsp;</td><td><div align="center"><input name="tijiao" type="submit" id="tijiao" value="提交">
  　　　　
    <input type="button" name="Submit" value="返回" onClick="javascript:history.go(-1)"></div></td></tr></form></table><blockquote><p></p></blockquote></body></html>