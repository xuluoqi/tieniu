...
%><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script language="javascript" src="inc/func.js" type="text/javascript"></script><style type="text/css"><!--
.hyx {
...
--></style><%
set rs=server.CreateObject("adodb.recordset")
rs.open"select wenzhang.title,wenzhang.times ,wenzhang.id from wenzhang left join wen_type on wen_type.id=wenzhang.type where wen_type.id="&request("type_id")&" order by wenzhang.times desc",conn,1,1
if isnull(rs("title")) then
response.Write "<br><font color=red>暂时没有数据!</font>"
else
...
adcolor="white"
%><table width="80%" height="27" border="0" align="center" cellpadding="0" cellspacing="0" ><tr><td width="83%" height="27"> 　<font color='#006699' face='wingdings'>2</font><font color="#FF0066"><%= conn.execute("select title from wen_type where id="&request("type_id")&"")(0) %></font></td><td width="17%"><div align="center" ><a href="bbxp.asp?action=addnews&type_id=<%= request("type_id") %>"></a></div></td></tr></table><table width="79%" height="32" border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC" ><%
DO While not rs.eof
%><tr bgcolor="#FFFFFF"><td width=492 height="21"><a href="javascript:openEditScript('new_list.asp?id=<%= rs("id") %>&type_id=<%= request("type_id") %>',800,600)"><font color='#006699' face='wingdings'>&nbsp;2</font>&nbsp;<%= rs("title") %></a></td></tr><%
rs.movenext
i=i+1
if i>=count then exit do
loop
%></table><br><table width="91%" border="0" align="center" cellpadding="0" cellspacing="0" ><tr><td><div align="center">共记录&nbsp;<span class="style3"><%=  rs.recordcount %>&nbsp;</span>个,当前页<span class="style3"><%=  page %>/<%=  pgnm %></span>：
        <%
If page>1 Then
%><a href="bbxp.asp?type_id=<%= request("type_id") %>&page=<%=  page-1 %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="bbxp.asp?type_id=<%= request("type_id") %>&page=<%=  page+1 %>">下一页</a><%
Else
%><%=  "下一页" %><%
End If
