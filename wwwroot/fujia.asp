...
End If
End Sub
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>其他消费列表--门市-><%= conn.execute("select peplename from yuangong where username='"&conn.execute("select userid from shejixiadan where id="&request("id")&"")(0)&"'")(0) %></title><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><style type="text/css"><!--
.STYLE2 {color: #999999}
--></style><%
...
%></head><script src="Js/Calendar.js"></script><script language="javascript" src="inc/func.js" type="text/javascript"></script><link href="Css/calendar-blue.css" rel="stylesheet"><script language="javascript">
...
function check1()
{	
	if(!CheckIsNull(document.getElementById("times"),"请选择添加时间！"))　return false;
	if(!CheckIsNull(document.form1.userid,"请选择添加门市！"))　return false;
	if(!CheckIsNumericOrNull(document.getElementById("save_money"),"请填写预收金额！","预收金额填写错误！"))　return false;
	
	var summoney = parseFloat(document.getElementById("lb_summoney").innerHTML);
	var save_money = parseFloat(document.getElementById("save_money").value)
	if(summoney<save_money){
		alert("收款金额不能大于实际金额.");
		return false;
	}
	...
	//if(!CheckIsNumericOrNull(document.form1.sl,"请填写数量！","数量填写错误！"))　return false;
	//if(!CheckIsNull(document.form1.jixiang,"请选择类型！"))　return false;
	//if(!CheckIsNumericOrNull(document.form1.money,"请填写费用！","费用填写错误！"))　return false;
}
function check2(){
	if(!CheckIsNull(document.getElementById("times"),"请选择添加时间！"))　return false;
	if(!CheckIsNull(document.form1.userid,"请选择经手人！"))　return false;
	if(!CheckIsNumericOrNull(document.getElementById("save_money"),"请填写预收金额！","预收金额填写错误！"))　return false;
		...
		if(slt1.value!=""){
			...
			for(var k=i+1;k<=10;k++){
				...
				if(slt2.value!=""){
					...
					if(!CheckIsNumericOrNull(document.getElementById("money"+k),"请填写费用！","费用填写错误！"))　return false;
					if(!CheckIsNumericOrNull(document.getElementById("sl"+k),"请填写数量！","数量填写错误！"))　return false;
				}
			}
		}
	}
...
/*{if(!CheckIsNull(document.form2.jixiang,"请选择类型！"))　return false;
if(!CheckIsNumericOrNull(document.form2.money,"请填写费用！","费用填写错误！"))　return false;
if(!CheckIsNumericOrNull(document.form2.sl,"请填写数量！","数量填写错误！"))　return false;*/
}
function check3()
{
if(!CheckIsNull(document.form2.jixiang,"请选择消费项目！"))　return false;
if(!CheckIsNull(document.form2.userid,"请选择员工！"))　return false;
if(!CheckIsNumericOrNull(document.getElementById("save_money"),"请填写预收金额！","预收金额填写错误！"))　return false;
if(!CheckIsNumericOrNull(document.form2.money,"请填写费用！","费用填写错误！"))　return false;
if(!CheckIsNumericOrNull(document.form2.sl,"请填写数量！","数量填写错误！"))　return false;
}
...
dim ver,CompanyType
ver = GetFieldDataBySQL("select [version] from sysconfig","int",0)
...
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from fujia ",conn,1,3
rs.addnew
if conn.execute("select [type] from yunyong where id="&request("jixiang"&i))(0)=1 then
rs("cuenchu_id")=cuenchu_id
end if
if ver="Standard" then
money=request("money"&i)
else
money=0
end if
hq_money = hq_money + request("money"&i)
rs("sl")=request("sl"&i)
rs("userid")=request("userid")
rs("userid2")=request("userid2")
...
if not yy.eof then
counts9=counts9+1
allmemo = allmemo & "<td>"&yy(0)&"/"&request("sl"&i)&"/"&money&"元&nbsp;&nbsp;"&htmlencode2(request("beizhu"&i))&"</td>"
...
set rs=nothing
'
Call updateXiangmuFlag(id,money)
end if
next
if ver<>"Standard" then
conn.execute("update shejixiadan set beizhu=beizhu+'<br>后期消费金额："&hq_money&"元' where id="&request("id"))
end if
if request("x_pz_name1")<>"y" then
if request("pz_name1")<>"" then
conn.execute("update shejixiadan set lc_cp=now,cp_name='"&conn.execute("select peplename from yuangong where username='"&request("pz_name1")&"'")(0)&"' where id="&request("id"))
if request("per_pz_name1")="" then
conn.execute("insert into xiadan (userid,beizhu,xiangmu_id,type,times) values ('"&request("pz_name1")&"','&nbsp;',"&request("id")&",4,#"&date&"#)")
elseif request("per_pz_name1")<>request("pz_name1") then
conn.execute("update xiadan set userid='"&request("pz_name1")&"' where xiangmu_id="&request("id")&" and type=4 and userid='"&request("per_pz_name1")&"'")
end if
else
conn.execute("update shejixiadan set cp_name=null,lc_cp=null,cp_wedvol=0,cp_memo=null where id="&request("id"))
conn.execute("delete from xiadan where userid='"&request("per_pz_name1")&"' and xiangmu_id="&request("id")&" and type=4")
end if
end if
factmoney=request("save_money")
if factmoney="" or not isnumeric(factmoney) then factmoney=0
if allmemo<>"" then
set rs=server.CreateObject("adodb.recordset")
...
response.Write "<script>alert('添加成功!');location.href='save_money_print.asp?id="&save_id&"'</script>"
On Error GoTo 0
Err.Raise 9999
end if
end if
response.Write "<script>alert('添加成功!');location.href='save_money.asp?id="&request("id")&"'</script>"
case "dele1"
...
if conn.execute("select [type] from yunyong where id="&conn.execute("select jixiang from fujia where id="&request("id2")&"")(0))(0)=1 then
conn.execute("update yunyong set sl=sl+"&conn.execute("select sl from fujia where id="&request("id2")&"")(0)&" where id="&conn.execute("select jixiang from fujia where id="&request("id2")&"")(0))
conn.execute("delete from cuenchu where id="&conn.execute("select cuenchu_id from fujia where id="&request("id2")&"")(0))
end if
conn.execute("delete from fujia where id="&request("id2"))
...
if conn.execute("select [type] from yunyong where id="&conn.execute("select jixiang from fujia2 where id="&request("id2")&"")(0))(0)=1 then
conn.execute("update yunyong set sl=sl+"&conn.execute("select sl from fujia2 where id="&request("id2")&"")(0)&" where id="&conn.execute("select jixiang from fujia2 where id="&request("id2")&"")(0)&"")
conn.execute("delete from cuenchu where id="&conn.execute("select cuenchu_id from fujia2 where id="&request("id2")&"")(0)&"")
end if
conn.execute("delete from fujia2 where id="&request("id2")&"")
response.Redirect "save_money.asp?id="&request("id")&""
On Error GoTo 0
Err.Raise 9999
case "dele3"
...
if conn.execute("select [type] from yunyong where id="&conn.execute("select jixiang from fujia where id="&request("id2")&"")(0))(0)=1 then
conn.execute("update yunyong set sl=sl+"&conn.execute("select sl from goumai where id="&request("id2")&"")(0)&" where id="&conn.execute("select jixiang from goumai where id="&request("id2")&"")(0)&"")
conn.execute("delete from cuenchu where id="&conn.execute("select cuenchu_id from goumai where id="&request("id2")&"")(0)&"")
end if
conn.execute("delete from goumai where id="&request("id2")&"")
response.Redirect "save_money.asp?id="&request("id")&""
On Error GoTo 0
Err.Raise 9999
case "deldg"
...
conn.execute("delete from sell_jilu where id="&request("id2"))
response.Redirect "save_money.asp?id="&request("id")
On Error GoTo 0
Err.Raise 9999
case "add2ed"
...
conn.execute("update yunyong set sl=sl-"&request("sl"&i)&" where id="&request("jixiang"&i))
conn.execute("insert into cuenchu (xiangmu_id,sp_id,sl,type,type2,type3,beizhu,times) values ("&id&","&request("jixiang"&i)&","&request("sl"&i)&",2,1,2,'"&htmlencode2(request("beizhu"&i))&"',#"&now&"#)")
cuenchu_id=conn.execute("select max(id) from cuenchu where xiangmu_id="&request("id")&"")(0)
end if
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from fujia2 ",conn,1,3
rs.addnew
rs("cuenchu_id")=cuenchu_id
rs("xiangmu_id")=id
if ver="Standard" then
...
qz_money = qz_money + request("money"&i)
rs("sl")=request("sl"&i)
rs("money")=money
rs("jixiang")=request("jixiang"&i)
rs("beizhu")=htmlencode2(request("beizhu"&i))
...
next
if ver<>"Standard" then
conn.execute("update shejixiadan set beizhu=beizhu+'<br>拍照化妆消费："&qz_money&"元' where id="&request("id"))
end if
factmoney=request("save_money")
if factmoney="" or not isnumeric(factmoney) then factmoney=0
if allmemo<>"" then
...
if request("from")="shejishi" then
response.Write "<script>alert('添加成功!');location='shejishi/fujia.asp?id="&id&"&action=fujia1'</script>"
else
response.Write "<script>alert('添加成功!');location='save_money.asp?id="&id&"'</script>"
end if
case "add3ed"
counts9=0
jh_money = 0
...
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from goumai ",conn,1,3
rs.addnew
rs("cuenchu_id")=cuenchu_id
rs("xiangmu_id")=id
if ver="Standard" then
...
jh_money = jh_money + request("money"&i)
rs("sl")=request("sl"&i)
rs("money")=money
rs("jixiang")=request("jixiang"&i)
rs("beizhu")=htmlencode2(request("beizhu"&i))
...
next
if ver<>"Standard" then
conn.execute("update shejixiadan set beizhu=beizhu+'<br>结婚化妆消费："&jh_money&"元' where id="&request("id"))
end if
factmoney=request("save_money")
if factmoney="" or not isnumeric(factmoney) then factmoney=0
if allmemo<>"" then
...
if request("from")="shejishi" then
response.Write "<script>alert('添加成功!');location='shejishi/fujia.asp?id="&id&"&action=fujia2'</script>"
else
response.Write "<script>alert('添加成功!');location='save_money.asp?id="&id&"'</script>"
end if
case "add"
set rsflag=conn.execute("select lc_wc,shejiwancheng from shejixiadan  where id="&request("id"))
if rsflag("lc_wc")<>"" and not isnull(rsflag("lc_wc")) then
response.Write "<script>alert('对不起,该项目已经取件不能再对其进行修改!');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
if rsflag("shejiwancheng")=1 then
response.Write "<script>alert('对不起,该项目已经完成不能再对其进行修改!');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
rsflag.close
set rsflag=nothing
if session("level")<>1 and session("level")<>10 and session("level")<>7 then
response.Write "<script>alert('对不起，你不能添加二次消费！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from yunyong where ishidden=0",conn,1,1
response.write"<script language = ""JavaScript"">"
...
count=1
while not rs.eof
response.write"subcat["&count&"] = new Array('"& rs("money")&"','"&rs("id")&"','"&rs("beizhu")&"');"
count = count + 1
rs.movenext
wend
rs.close
...
response.write"var ll='';"
response.write"var mm='';"
response.write"for (i=1;i < onecount; i++)"
response.write"{"
response.write"if (subcat[i][1] == month)"
response.write"{"
response.write"kk = subcat[i][0];"
response.write"jj = subcat[i][2];"
response.write"}"
response.write"}"
response.Write "document.form1.money.value=kk ;"
response.Write "document.form1.beizhu.value=jj ;"
response.write"}"
response.Write "</script>"
response.write "<script language='JavaScript'>"&vbcrlf
response.write "<!-- Begin"&vbcrlf
...
response.write "//  End -->"&vbcrlf
response.write "</script> "&vbcrlf
%><br><table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td height="20" colspan="2">&gt;<a href="#" onClick="history.go(-1)">返回</a>&nbsp;&nbsp;&nbsp; 
  (接单:
    <%
set rsms = conn.execute("select * from shejixiadan where id="&request("id"))
if not rsms.eof then
response.write conn.execute("select peplename from yuangong where username='"&rsms("userid")&"'")(0)
if rsms("userid2")<>"" and not isnull(rsms("userid2")) then
response.write " / "&conn.execute("select peplename from yuangong where username='"&rsms("userid2")&"'")(0)
end if
if rsms("userid3")<>"" and not isnull(rsms("userid3")) then
response.write " / "&conn.execute("select peplename from yuangong where username='"&rsms("userid3")&"'")(0)
end if
cp_name=rsms("cp_name")
else
response.write "无"
end if
%> 
 &nbsp;选片:<%
if rsms("ky_name")<>"" and not isnull(rsms("ky_name")) then
response.write rsms("ky_name")
else
response.write "无"
end if
if rsms("ky_name2")<>"" and not isnull(rsms("ky_name2")) then
response.write "/"&rsms("ky_name2")
end if
rsms.close
set rsms=nothing
%>)
  </tr></table><table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td height="14" colspan="2">&nbsp;添加后请客人柜台进行付款  换产品补差价，请经理帐号去掉原有套系产品即可    (可连续输入 10 个也可单个输入）    
    </tr></table><table width="98%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><form action="fujia.asp?action=added" method="post" name="form1" onSubmit="return check1()"><tr><td height="22" align="left" bgcolor="#FFFFFF" colspan="3">&nbsp;设计单号:&nbsp;<%= request("id") %>&nbsp;&nbsp;&nbsp; 客户:&nbsp;
              <%
khid = conn.execute("select kehu_id from shejixiadan where id="&request("id"))(0)
set rs4=server.CreateObject("adodb.recordset")
rs4.open "select * from kehu where id="&khid,conn,1,1
...
%>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span <%
if (session("level")<>1 or session("zhuguan")<>1) and session("level")<>5 and session("level")<>10 and session("level")<>7 then response.write "style='display:none'"
%>>预收金额:
              <input name="save_money" type="text" id="save_money" onKeyUp="value=value.replace(/[^\d]/g,'');" size="10" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"<%
if ver<>"Standard" then response.write " disabled"
%>>
              元&nbsp;&nbsp; 刷卡收款：    
              <input type="checkbox" name="wzsk" value="1">
              &nbsp; 小计&nbsp;<span id="lb_summoney">0</span> 元 &nbsp;&nbsp; <br>
              &nbsp;添加门市1
              <select name="userid" id="userid"><option value="" selected>请选择</option><%
set prs = Conn.Execute("Select username,peplename,[level] from yuangong where [level]=1")
do while not prs.eof
response.write "<option value='"&prs("username")&"'>"&prs("peplename")&"</option>"
prs.movenext
loop
prs.close
set prs = nothing
%></select>
              &nbsp;&nbsp;&nbsp; 门市2
              <select name="userid2" id="userid2"><option value="" selected>请选择</option><%
set prs = Conn.Execute("Select username,peplename,[level] from yuangong where [level]=1")
do while not prs.eof
response.write "<option value='"&prs("username")&"'>"&prs("peplename")&"</option>"
prs.movenext
...
%></select>
              &nbsp;&nbsp; 摄影师
              <%
if session("level")<>10 and cp_name<>"" and not isnull(cp_name) then
response.Write "<input type='hidden' name='x_pz_name1' id='x_pz_name1' value='y'>"
response.write "<select name='pz_name1' id='pz_name1' disabled>"
else
%><select name="pz_name1" id="pz_name1"><%
end if
%><option value="">请选择...</option><%
set rss = server.CreateObject("adodb.recordset")
rss.open "select * from yuangong where level=4",conn,1,1
do while not rss.eof
%><option value="<%= rss("username") %>" <%
if cp_name=rss("peplename") then response.write " selected"
%>><%= rss("peplename") %></option><%
rss.movenext
loop
rss.close
%></select><%
if cp_name<>"" and not isnull(cp_name) then
set sjrs = conn.execute("select username from yuangong where peplename='"&cp_name&"'")
if not sjrs.eof then
uid=sjrs("username")
else
uid=""
end if
...
%><input name="per_pz_name1" type="hidden" id="per_pz_name1" value="<%= uid %>">
              &nbsp; <%
if CheckOldMoneyControl() then
%>添加时间
          <input name="times" type="text" id="times" size="13" value="<%= now %>" readonly>
...
%></td></tr><%
for i = 1 to 10
%><tr bgcolor="#FFFFFF"><td height="22" style="meizz:expression(this.noWrap=true);"><table width="100%" border="0" cellspacing="0" cellpadding="0"><tr bgcolor="#FFFFFF"><td width="540" height="22" style="meizz:expression(this.noWrap=true);">&nbsp;后期项目
...
rs2.close
set rs2=nothing
%></select><select name="jixiang<%= i %>" id="jixiang<%= i %>" style="width:150px" onChange="javascript:changePageShow(document.getElementById('pagevol<%= i %>'),this.options[this.selectedIndex].value,<%= i %>)">
              <option value="">请选择</option></select>
            &nbsp; 总费用
            <input name="money<%= i %>" type="text" id="money<%= i %>" size="4" onChange="getSumMoney()" onKeyDown="getSumMoney()" onKeyUp="value=value.replace(/[^\d]/g,'');getSumMoney()" value="0" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))">
            元 &nbsp;数量
            <input name="sl<%= i %>" type="text" id="sl<%= i %>" size="2" onKeyUp="value=value.replace(/[^\d]/g,'')   "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"></td><td id="td_page<%= i %>"><input name="pagevol<%= i %>" type="hidden" id="pagevol<%= i %>" size="2" onKeyUp="value=value.replace(/[^\d]/g,'')   "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"></td><td>&nbsp; 备注
          <input name="beizhu<%= i %>" type="text" id="beizhu<%= i %>" size="20"></td></tr></table></td></tr><%
next
%><tr><td height="35" align="center" bgcolor="#FFFFFF" colspan="3"><input name="btn_save" type="submit" id="btn_save" value="仅提交">
          &nbsp; 
            <input name="btn_save" type="submit" id="btn_save" value="提交并转到打印">
            &nbsp; 
            <input type="button" name="Submit" value="返回" onClick="javascript:history.go(-1)"><input type="hidden" name="id" value="<%= request("id") %>">          </td></tr></form></table><br><br><br><table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td width="19%" valign="top"><div align="right">常用语：</div></td><td width="81%"><textarea name="yongyu" cols="90" rows="7" id="textarea2"><%
if not isnull(conn.execute("select yongyu from two_yongyu where userid='"&"admin"&"'")(0)) then
response.Write conn.execute("select yongyu from two_yongyu where userid='"&"admin"&"'")(0)
end if
%></textarea></td></tr></table><br><table width="83%"  border="0" align="center" cellpadding="0" cellspacing="0" style="display:none"><tr><td width="19%" valign="top"><div align="right">常用语：</div></td><td width="81%"><textarea name="yongyu" cols="65" rows="7" id="textarea2"><%
if not isnull(conn.execute("select yongyu from two_yongyu where userid='"&"admin"&"'")(0)) then
response.Write conn.execute("select yongyu from two_yongyu where userid='"&"admin"&"'")(0)
end if
...
response.write "<br>"
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from yunyong where type2=1 and ishidden=0",conn,1,1
response.write"<script language = ""JavaScript"">"
response.write"var onecount;"
response.write"onecount=0;"
response.write"subcat = new Array();"
count=1
while not rs.eof
response.write"subcat["&count&"] = new Array('"& rs("choucheng")&"','"&rs("id")&"');"
count = count + 1
rs.movenext
wend
rs.close
...
response.write"}"
response.write"}"
response.Write "document.form2.money.value=kk ;"
response.write"}"
response.Write "</script>"
%><br><table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td height="20" colspan="2">&gt; 添加拍照消费
  </tr></table><table width="98%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><form action="fujia.asp?action=add2ed" method="post" name="form1" onSubmit="return check2()"><tr><td height="22" align="left" bgcolor="#FFFFFF">&nbsp;&nbsp; 设计单号:&nbsp;<%= request("id") %>&nbsp;&nbsp;&nbsp;&nbsp;客户:&nbsp;
          <%
khid = conn.execute("select kehu_id from shejixiadan where id="&request("id"))(0)
set rs4=server.CreateObject("adodb.recordset")
rs4.open "select * from kehu where id="&khid,conn,1,1
...
%>
        &nbsp;&nbsp;<%= GetDutyName(5) %>:
        <%
set rshz = conn.execute("select hz_name from shejixiadan where id="&request("id"))
if rshz("hz_name")<>"" and not isnull(rshz("hz_name")) then
response.write rshz("hz_name")
else
response.write "无"
end if
rshz.close
%>
&nbsp;&nbsp; <span <%
if (session("level")<>1 or session("zhuguan")<>1) and session("level")<>5 and session("level")<>10 and session("level")<>7 and session("level")<>14 then response.write "style='display:none'"
%>>预收金额:
          <input name="save_money" type="text" id="save_money" onKeyUp="value=value.replace(/[^\d]/g,'')   " size="10"onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))"<%
if ver<>"Standard" then response.write " disabled"
%>>
元&nbsp;&nbsp; 刷卡收款：
<input type="checkbox" name="wzsk" value="1">
&nbsp;&nbsp;&nbsp; 小计&nbsp;<span id="lb_summoney">0</span> 元</span></td></tr><tr><td height="22" bgcolor="#FFFFFF">&nbsp;&nbsp; 经手人1: <%
Call ShowUserSelect("userid", "1, 4, 5, 11, 12, 14", "username", "请选择...", "", 0, false)
%>
        &nbsp;&nbsp;经手人2: <%
Call ShowUserSelect("userid2", "1, 4, 5, 11, 12, 14", "username", "请选择...", "", 0, false)
%>
&nbsp;&nbsp;
经手人3: <%
Call ShowUserSelect("userid3", "1, 4, 5, 11, 12, 14", "username", "请选择...", "", 0, false)
%>&nbsp;&nbsp;
		<%
if CheckOldMoneyControl() then
%>
...
%></td></tr><%
for i = 1 to 10
%><tr><td height="22" bgcolor="#FFFFFF">&nbsp;&nbsp; 消费项目
        <select name="jixiang<%= i %>" id="jixiang<%= i %>">
          <option value="">请选择</option><%
set rs=server.CreateObject("adodb.recordset")
rs.open  "select * from yunyong where type2=1 and ishidden=0",conn,1,1
while not rs.eof
%><option value="<%= rs("id") %>"><%= rs("yunyong") %>&nbsp;&nbsp;单价:<%= rs("money") %></option><%
rs.movenext
wend
rs.close
set rs=nothing
%></select>        &nbsp; 总费用
        <input name="money<%= i %>" type="text" id="money<%= i %>" onChange="getSumMoney()" onKeyDown="getSumMoney()" onKeyUp="value=value.replace(/[^\d]/g,'');getSumMoney()" value="0" size="5"onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))">
        元 &nbsp;数量
        <input name="sl<%= i %>" type="text" id="sl<%= i %>" size="2" onKeyUp="value=value.replace(/[^\d]/g,'')   "onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))">
        &nbsp;&nbsp;备注
      <input name="beizhu<%= i %>" type="text" id="beizhu<%= i %>" size="40"></td></tr><%
next
%><tr><td height="35" align="center" bgcolor="#FFFFFF"><input name="btn_save" type="submit" id="btn_save" value="仅提交">
          &nbsp; 
            <input name="btn_save" type="submit" id="btn_save" value="提交并转到打印" <%
if request("from")="shejishi" then response.write "style='display:none'"
%>>
            &nbsp; 
            <input type="button" name="Submit" value="返回" onClick="javascript:history.go(-1)"><input type="hidden" name="id" value="<%= request("id") %>">      <input name="from" type="hidden" id="from" value="<%= request("from") %>"></td></tr></form></table><br><br><br><table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td width="10%" valign="top"><div align="right">常用语：</div></td><td><textarea name="textarea3" cols="78" rows="7" id="textarea4"><%
if not isnull(conn.execute("select yongyu from save3_yongyu")(0)) then
response.Write conn.execute("select yongyu from save3_yongyu")(0)
end if
%></textarea></td></tr></table><%
case "add3"
'
...
'
'
%><br><table width="98%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td height="20" colspan="2">&gt; 添加结婚妆消费
  </tr></table><table width="98%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><form action="fujia.asp?action=add3ed" method="post" name="form1" onSubmit="return check2()"><tr><td height="22" align="left" bgcolor="#FFFFFF"> &nbsp;设计单号:&nbsp;<%= request("id") %>&nbsp;&nbsp;&nbsp;&nbsp;客户:&nbsp;
          <%
khid = conn.execute("select kehu_id from shejixiadan where id="&request("id"))(0)
set rs4=server.CreateObject("adodb.recordset")
rs4.open "select * from kehu where id="&khid,conn,1,1
...
%>
        &nbsp;&nbsp;<%= GetDutyName(5) %>:
        <%
set rshz = conn.execute("select hz_userid from shejixiadan where id="&request("id"))
if rshz("hz_userid")<>"" and not isnull(rshz("hz_userid")) then
response.write conn.execute("select peplename from yuangong where username='"&rshz("hz_userid")&"'")(0)
else
response.write "无"
end if
rshz.close
...
&nbsp; 
&nbsp; 小计&nbsp;<span id="lb_summoney">0</span> 元</span></td></tr><tr><td height="22" bgcolor="#FFFFFF">&nbsp;经手人1:
        <select name="userid" id="userid"><option value="" selected>请选择员工</option><%
set prs = Conn.Execute("Select username,peplename,[level] from yuangong where [level] in (5, 11, 14) and isdisabled=0 order by [level]")
cur_level=0
do while not prs.eof
if cur_level<>prs("level") then
if prs("level")=5 then
response.write "<option value='' style='background-color:#f3f3f3'>==="&GetDutyName(5)&"===</option>"
elseif prs("level")=11 or prs("level")=14 then
response.write "<option value='' style='background-color:#f3f3f3'>==服&nbsp;&nbsp;&nbsp;&nbsp;装==</option>"
elseif prs("level")=14 then
response.write "<option value='' style='background-color:#f3f3f3'>=="&GetDutyName(14)&"==</option>"
end if
cur_level=prs("level")
end if
response.write "<option value='"&prs("username")&"'>&nbsp;&nbsp;"&prs("peplename")&"</option>"
...
%></select>
        &nbsp;&nbsp;经手人2:
        <select name="userid2" id="userid2"><option value="" selected>请选择员工</option><%
set prs = Conn.Execute("Select username,peplename,[level] from yuangong where [level] in (5, 11, 14) and isdisabled=0 order by [level]")
cur_level=0
do while not prs.eof
if cur_level<>prs("level") then
...
%></select>
&nbsp;&nbsp;经手人3:
<select name="userid3" id="userid3"><option value="" selected>请选择员工</option><%
set prs = Conn.Execute("Select username,peplename,[level] from yuangong where [level] in (5, 11, 14) and isdisabled=0 order by [level]")
cur_level=0
do while not prs.eof
if cur_level<>prs("level") then
...
%>>
            &nbsp; 
            <input type="button" name="Submit" value="返回" onClick="javascript:history.go(-1)"><input type="hidden" name="id" value="<%= request("id") %>">
        <input name="from" type="hidden" id="from" value="<%= request("from") %>"></td></tr></form></table><br><br><br><table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td width="10%" valign="top"><div align="right">常用语：</div></td><td><textarea name="textarea" cols="65" rows="7" id="textarea"><%
if not isnull(conn.execute("select yongyu from save3_yongyu")(0)) then
response.Write conn.execute("select yongyu from save3_yongyu")(0)
end if
%></textarea></td></tr></table><%
case else
%><table width="80%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td width="80%"><%
set rs=conn.execute("select distinct(dated) from (select datevalue(times) as dated from fujia where  xiangmu_id="&request("id")&" union  select datevalue(times) as dated from fujia2  where xiangmu_id="&request("id")&")")
i=0
while not rs.eof
i=i+1
...
rs.close
set rs=nothing
%></td><td width="20%">&nbsp;</td></tr></table><table width="88%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr bordercolor="#66FF33" bgcolor="#66FF33"><td width="66%">&nbsp;&nbsp;<span class="STYLE2">添加数据后柜台便清楚这个客人余款，请客人柜台进行收款</span></td><td width="18%" height="24"><a href="admin/two_yongyu.asp">常用语设置</a></td><td width="16%"><div align="center"><a href="fujia.asp?action=add&id=<%= request("id") %>">添加后期消费</a></div></td></tr></table><div align="center"><%
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from fujia where xiangmu_id="&request("id")&"",conn,1,1
if rs.eof then
response.Write "<font color=red size=2>没有数据 请点添加后期消费</font>"
else
%></div><table width="88%" height="41"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><%
while not rs.eof
%><tr bgcolor="#FFFFFF"><td width="22%" height="20">&nbsp;&nbsp;<%= conn.execute("select yunyong from yunyong where id="&rs("jixiang")&"")(0) %></td><td width="15%">&nbsp;员工：<%
...
%></td><td width="13%">&nbsp;数量：<%= rs("sl") %></td><td width="15%">&nbsp;费用：<%= rs("money") %>(元)</td><td width="35%"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="88%" height="20"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="69%">&nbsp;<%= rs("times") %></td><td width="31%">&nbsp;
                  <%
if rs("userid")<> conn.execute("select userid from shejixiadan where id="&request("id")&"")(0) and session("level")=1 then
response.Write "["&conn.execute("select peplename from yuangong where username='"&rs("userid")&"'")(0)&"]"
end if
%></td></tr></table></td><td width="12%"><%
if session("level")=10 or session("level")=3 then
%><a href="fujia.asp?action=dele1&id2=<%= rs("id") %>&id=<%= request("id") %>" onClick="return confirm('确认要删除吗？')">删除</a><%
end if
%></td></tr></table></td></tr><tr bgcolor="#FFFFFF"><td height="18" colspan="5"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="7%" valign="top"><div align="right">备注:</div></td><td width="93%"><%= encode(rs("beizhu")) %></td></tr></table></td></tr><%
rs.movenext
wend
rs.close
set rs=nothing
...
%><table width="88%"  border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#efefef"><tr><td width="74%" height="24" bgcolor="#66FF33"><div align="right"><a href="admin/save3_yongyu.asp">化妆常用语</a></div></td><td width="26%" bgcolor="#66FF33"><div align="center"><a href="fujia.asp?action=add2&id=<%= request("id") %>">添加拍照消费</a></div></td></tr></table><div align="center"><%
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from fujia2 where xiangmu_id="&request("id")&"",conn,1,1
if rs.eof then
response.Write "<font color=red size=2>还没有三次消费记录</font>"
else
%></div><table width="88%" height="43"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><%
while not rs.eof
%><tr bgcolor="#FFFFFF"><td height="20">&nbsp;&nbsp;<%= conn.execute("select yunyong from yunyong where id="&rs("jixiang")&"")(0) %></td><td>&nbsp;员工：
    <%
...
%></td><td>&nbsp;费用：<%= rs("money") %>元</td><td><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="63%">&nbsp;<%= rs("times") %></td><td width="27%">数量:<%= rs("sl") %></td><td width="10%"><%
if session("level")=10 or session("level")=3 then
%><a href="fujia.asp?action=dele2&id2=<%= rs("id") %>&id=<%= request("id") %>" onClick="return confirm('确认要删除吗？')">删除</a><%
end if
%></td></tr></table></td></tr><tr bgcolor="#FFFFFF"><td height="18" colspan="4"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="7%" valign="top"><div align="right">备注:</div></td><td width="93%"><%= encode(rs("beizhu")) %></td></tr></table></td></tr><%
rs.movenext
wend
rs.close
set rs=nothing
...
if CompanyType=0 then
%><br><table width="88%"  border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#efefef"><tr bgcolor="#66FF33"><td width="74%" height="24"><div align="right"><a href="hz_beizhu.asp?id=<%= request("id") %>">修改结婚化妆时间与要求</a></div></td><td width="26%" bgcolor="#66FF33"><div align="center"><a href="fujia.asp?action=add3&id=<%= request("id") %>">添加结婚妆消费</a></div></td></tr></table><table width="88%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td>&nbsp;&nbsp;结婚化妆具体时间:<%
...
%></td></tr><tr><td>&nbsp;&nbsp;化妆要求:<%= conn.execute("select hz_beizhu from shejixiadan where id="&request("id")&"")(0) %></td></tr></table><div align="center"><%
set rs=server.CreateObject("adodb.recordset")
...
if rs("userid2")<>"" and not isnull(rs("userid2")) then response.write "/"&conn.execute("select peplename from yuangong where username='"&rs("userid2")&"'")(0)
if rs("userid3")<>"" and not isnull(rs("userid3")) then response.write "/"&conn.execute("select peplename from yuangong where username='"&rs("userid3")&"'")(0)
%></td><td>&nbsp;费用：<%= rs("money") %>元</td><td>&nbsp;数量：<%= rs("sl") %></td><td><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="80%">&nbsp;<%= rs("times") %></td><td width="20%"><%
if session("level")=10 or session("level")=3 then
...
if dg_invis then
%><table width="88%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#efefef"><tr bgcolor="#FFFFFF"><td width="60%" height="18" bgcolor="#66FF33"><div align="right"><%
if session("level")=10 then
%><a href="sell.asp?action=type&id2=<%= request("id") %>"><strong>类别管理</strong><%
end if
%></a><a href="admin/two_yongyu.asp">&nbsp;&nbsp;&nbsp;</a></div></td><td width="20%" bgcolor="#66FF33" style="display:none"><div align="center"><a href="sell.asp?action=add&id=<%= request("id") %>">添加摄影师导购记录</a></div></td><td width="20%" bgcolor="#66FF33"><div align="center"><a href="sells.asp?action=add&id=<%= request("id") %>">添加助理导购记录</a></div></td></tr></table><%
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from sell_jilu where xiangmu_id="&request("id")&" order by times desc",conn,1,1
if rs.eof then
%><table width="88%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td><div align="center">还没有数据</div></td></tr></table><%
else
%><table width="88%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#999999"><%
do while not rs.eof
...
%><tr bgcolor="#FFFFFF"><td colspan="4"><table width="100%" border="0" cellspacing="0" cellpadding="0"><tr><td><font color="#FF0000">&nbsp;技术员：<%= peplename %></font></td><td align="right"><%
if session("level")=10 then response.write "<a href=?action=deldg&id2="&rs("id")&"&id="&request("id")&" onClick=""return confirm('确认要删除吗？')"">删除</a>"
%>&nbsp; </td></tr></table></td></tr><tr bgcolor="#FFFFFF"><td>&nbsp;物品:<%= rs("name") %></td><td width="15%">&nbsp;单价:<%= rs("dj") %>元</td><td width="15%">&nbsp;&nbsp;数量:<%= rs("sl") %></td><td width="25%">&nbsp;时间:<%= rs("times") %></td></tr><tr bgcolor="#FFFFFF"><td colspan="4">&nbsp;备注:<%= encode(rs("beizhu")) %></td></tr><%
rs.movenext
i=i+1
