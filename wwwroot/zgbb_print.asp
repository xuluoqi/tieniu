...
monthd=request("month")
else
monthd=month(now)
end if
dim str_lc,arr_lc
dim sl_new,sl_wsy,sl_ysy
str_lc = "lc_hz,lc_cp,lc_xp,lc_ky,lc_sj,xg_sj,lc_xp2,lc_wc"
arr_lc = split(str_lc,",")
sl_new = conn.execute("select count(*) from shejixiadan where not isnull(times) and year(times)="&yeard&" and month(times)="&monthd&" and not isnull(times)")(0)
if isnull(sl_new) then sl_new=0
sl_ysy = conn.execute("select count(*) from shejixiadan where not isnull(times) and year(times)="&yeard&" and month(times)="&monthd&" and not isnull(times) and not isnull(lc_cp)")(0)
if isnull(sl_ysy) then sl_ysy=0
sl_wsy=sl_new-sl_ysy
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><link href="zxcss.css" rel="stylesheet" type="text/css"><script language="javascript" src="inc/func.js" type="text/javascript"></script><style type="text/css"><!--
body {
...
--></style><title>部门主管跟踪表</title></head><body topmargin="0" leftmargin="0"><center><%= yeard %>年<%= monthd %>月跟踪表<br><br></center><table width="100%" height="25" border="1" cellspacing="0"><tr bgcolor="#66FFFF" align="center" height="25"><td width="13%" height="25">单号</td><td width="12%"><%= GetAppellation(3, null) %></td><td width="12%"><%= GetAppellation(4, null) %></td><td width="12%">取件时间</td><td width="11%">&nbsp;总未缴</td><td width="13%">接单时间</td><td width="14%">&nbsp;门市/设计师</td><td width="13%">当前流程</td></tr><%
set rs=server.CreateObject("adodb.recordset")
sql = "select * from shejixiadan where year(times)="&yeard&" and month(times)="&monthd
for i = 0 to ubound(arr_lc)
sql = sql & " and isnull("&arr_lc(i)&")"
next
sql = sql & " order by id"
rs.open sql,conn,1,1
'
while not rs.eof
%><tr align="center" bgcolor="#FFFFFF" onMouseOver="this.bgColor='#FF9966'" onMouseOut="this.bgColor='#FFFFFF'"><td height="25">&nbsp;<%= rs("id") %></td><td>&nbsp;<%= GetFieldDataBySQL("select lxpeple from kehu where id="&rs("kehu_id"),"str","") %><br><%= GetTelNo(GetFieldDataBySQL("select telephone from kehu where id="&rs("kehu_id"),"str","")) %></td><td>&nbsp;<%= GetFieldDataBySQL("select lxpeple2 from kehu where id="&rs("kehu_id"),"str","") %><br><%= GetTelNo(GetFieldDataBySQL("select telephone2 from kehu where id="&rs("kehu_id"),"str","")) %></td><td>&nbsp;<%
if rs("qj_time")<>"" then
%><%= rs("qj_time") %>.<%= rs("qj") %><%
end if
%></td><td>&nbsp;<%= FinalMoneySum(rs("id"),false) %></td><td><%= datevalue(rs("times")) %></td><td>&nbsp;<%= GetFieldDataBySQL("select peplename from yuangong where username='"&rs("userid")&"'","str","") %>/<%= rs("sj_name") %></td><td>未摄影</td></tr><%
rs.movenext
wend
rs.close
for i=0 to ubound(arr_lc)
sql = "select * from shejixiadan where year(times)="&yeard&" and month(times)="&monthd&" and not isnull("&arr_lc(i)&")"
if i < ubound(arr_lc) then
for k = i+1 to ubound(arr_lc)
sql = sql & " and isnull("&arr_lc(k)&")"
next
end if
sql = sql & " order by id"
rs.open sql,conn,1,1
'
while not rs.eof
%><tr align="center" bgcolor="#FFFFFF" onMouseOver="this.bgColor='#FF9966'" onMouseOut="this.bgColor='#FFFFFF'"><td height="25">&nbsp;<%= rs("id") %></td><td>&nbsp;<%= GetFieldDataBySQL("select lxpeple from kehu where id="&rs("kehu_id"),"str","") %><br><%= GetTelNo(GetFieldDataBySQL("select telephone from kehu where id="&rs("kehu_id"),"str","")) %></td><td>&nbsp;<%= GetFieldDataBySQL("select lxpeple2 from kehu where id="&rs("kehu_id"),"str","") %>/<%= GetTelNo(GetFieldDataBySQL("select telephone2 from kehu where id="&rs("kehu_id"),"str","")) %></td><td>&nbsp;<%
if rs("qj_time")<>"" then
%><%= rs("qj_time") %>.<%= rs("qj") %><%
end if
%></td><td>&nbsp;<%= FinalMoneySum(rs("id"),false) %></td><td><%= datevalue(rs("times")) %></td><td>&nbsp;<%= GetFieldDataBySQL("select peplename from yuangong where username='"&rs("userid")&"'","str","") %>/<%= rs("sj_name") %></td><td>&nbsp;<%
if not isnull(rs("lc_wc")) then
response.write "全部完成"
elseif not isnull(rs("lc_xp2")) then
response.write "修片完成"
elseif not isnull(rs("xg_sj")) then
response.write "看版完成"
elseif not isnull(rs("lc_sj")) then
response.write "设计完成"
elseif not isnull(rs("lc_ky")) then
response.write "选片完成"
elseif not isnull(rs("lc_xp")) then
response.write "调色完成"
elseif not isnull(rs("lc_cp")) then
response.write "摄影完成"
elseif not isnull(rs("lc_hz")) then
response.write "化妆完成"
end if
%></td></tr><%
rs.movenext
...
next
set rs=nothing
%></table><div style="padding-top:15px;"> 接单量 <%= sl_new %> 单&nbsp; 未摄影 <%= sl_wsy %> 单&nbsp; 已摄影 <%= sl_ysy %> 单</div><br><br></body></html>