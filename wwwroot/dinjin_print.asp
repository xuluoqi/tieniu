...
End If
End Sub
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>定金收款打印</title><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script language="javascript" src="inc/func.js" type="text/javascript"></script><style type="text/css">
<!--
.style4 {color: #FF0000}
...
<%
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from save_money where id="&request("id")&"" ,conn,1,3
if not rs.eof and ischeck=0 then
if session("level")=10 or session("level")=7 then
rs("ischeck")=1
rs("checkuserid")=session("adminid")
rs("checkdate")=now()
rs.update
end if
else
response.write "<script language='javascript'>alert('数据不存在或已被删除.');history.back();</script>"
On Error GoTo 0
Err.Raise 9999
end if
set rs3=server.CreateObject("adodb.recordset")
rs3.open "select * from shejixiadan where id="&rs("xiangmu_id")&"",conn,1,1
%>
<table width="78%"  border="0" cellpadding="0" cellspacing="1" style="margin-left:10px">
  <tr><td>&nbsp;&nbsp;单号：<%= rs("xiangmu_id") %><%
if rs3("danhao")<>"" and not isnull(rs3("danhao")) then response.write "&nbsp;/&nbsp;"&rs3("danhao")
%>
&nbsp;&nbsp;<%= datevalue(rs("times")) %>&nbsp;
<%
set rs4=server.CreateObject("adodb.recordset")
rs4.open "select * from kehu where id="&rs3("kehu_id")&"",conn,1,1
...
%>
            &nbsp;&nbsp;门市：<%
dim ms1,ms2,ms3
ms1=GetFieldDataBySQL("select peplename from yuangong where username='"&rs3("userid")&"'","str","")
if ms1<>"" then
response.write ms1
ms2=GetFieldDataBySQL("select peplename from yuangong where username='"&rs3("userid2")&"'","str","")
if ms2<>"" then
response.write "/"&ms2
ms3=GetFieldDataBySQL("select peplename from yuangong where username='"&rs3("userid3")&"'","str","")
if ms3<>"" then response.write "/"&ms3
end if
end if
%>&nbsp;&nbsp;&nbsp;<%= getPrintString(rs3("kehu_id")) %></td>
  </tr></table>
...
<table width="88%"  border="1" cellpadding="0" cellspacing="0" bordercolor="#dedede" bgcolor="#CCCCCC" class="tdbg_main" style="margin-left:10px;border-collapse:collapse ">
  <tr bgcolor="#efefef"><td height="33" bgcolor="#FFFFFF"><div align="left">&nbsp; <%= conn.execute("select jixiang from jixiang where id="&conn.execute("select jixiang from shejixiadan where id="&request("xiangmu_id")&"")(0)&"")(0) %>&nbsp;[<%= conn.execute("select jixiang_money from shejixiadan where id="&request("xiangmu_id")&"")(0) %>元]&nbsp;&nbsp;&nbsp;定金：<%= rs("money") %>&nbsp;元&nbsp;
	<%
if not isnull(rs3("hz_time")) then
response.Write "化妆:"&rs3("hz_time")&"&nbsp;&nbsp;"
end if
if not isnull(rs3("pz_time")) then
response.Write "拍照:"&rs3("pz_time")&"&nbsp;&nbsp;"
end if
if not isnull(rs3("pzlf_time")) then
response.Write "拍照礼服:"&rs3("pzlf_time")&"&nbsp;&nbsp;"
end if
if not isnull(rs3("kj_time")) then
response.Write "选片:"&rs3("kj_time")&"&nbsp;&nbsp;"
end if
if not isnull(rs3("qj_time")) then
response.Write "取件:"&rs3("qj_time")&"&nbsp;&nbsp;"
end if
%>
	<span class="style2"><span class="STYLE11">地址</span></span><strong><font size="2"><span class="style11">:
	<%
if conn.execute("select count(*) from xinfeng_sz ")(0)>0 then response.Write encode(conn.execute("select xinfeng_sz from xinfeng_sz")(0))
%></span></FONT></strong></div></td></tr><tr bgcolor="#FFFFFF"><td height="80" valign="top"><div align="left"><table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0">
<tr><td><%
If request("Dj")="1" Then
if isnull(rs3("yunyong")) then
response.Write "<span class='STYLE1'><b>服务内容:</b></span><br>没有套系应有!"
else
strPrintSetting = conn.execute("select orderPrintSetting from sysconfig")(0)
if strPrintSetting<>"" then
Arr_PrintSetting = split(strPrintSetting,"|")
else
dim Arr_PrintSetting(4)
end if
id=split(rs3("yunyong"),", ")
sl=split(rs3("sl"),", ")
if rs3("pagevol")<>"" and not isnull(rs3("pagevol")) then
pagevol=split(rs3("pagevol"),", ")
end if
if not isnull(rs3("wc")) then
wc=split(rs3("wc"),", ")
end if
if instr(cstr(Arr_PrintSetting(2)),"1")>0 then
%>
                ◇服务内容◇
              <div style="width:90%; padding:3px"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><%
count11=ubound(id)+1
counts=0
for yy=1 to count11
set rsflag = conn.execute("select [type] from yunyong where id="&id(yy-1))
if not rsflag.eof and rsflag("type")=0 then
counts=counts+1
%><td><%
if len(counts)=2 then
response.Write "<strong>"&counts&".</strong>"
else
response.Write "<strong>0"&counts&"</strong>"
response.Write "."
end if
dim flag,rs_yunyong
set rs_yunyong=conn.execute("select id,yunyong,isxc from yunyong where id="&id(yy-1)&"")
response.Write rs_yunyong("yunyong")&"&nbsp;"
if rs_yunyong("isxc")=1 then
if rs3("pagevol")<>"" and not isnull(rs3("pagevol")) then
response.write pagevol(yy-1)
else
response.write "0"
end if
response.write "P"
...
rs_yunyong.close()
%></td><%
if counts mod 3 =0 then response.write "</td><tr>"
end if
rsflag.close()
next
%></table></div><%
end if
if instr(cstr(Arr_PrintSetting(2)),"0")>0 then
%>
				   ◇取件内容◇
		      <div style="width:90%; padding:3px"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><%
count11=ubound(id)+1
...
elseIf request("Dj")="2" Then
%><span>◇套系内容◇
            <%
if isnull(rs3("yunyong")) then
response.Write "<br>没有套系内容!"
else
id=split(rs3("yunyong"),", ")
sl=split(rs3("sl"),", ")
if not isnull(rs3("wc")) then
...
%></span><div style="width:90%;"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><%
count11=ubound(id)+1
'
'
'
...
%>
		备注：<%
response.Write encode(rs("beizhu"))
finalmoney=FinalMoneySum(rs("xiangmu_id"),false)
if finalmoney>0 then
response.write "<br>总款未缴: "&finalmoney&" 元"
end if
%><br>
        经手人签名:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 打单:<%= session("username") %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;客户签名： &nbsp;&nbsp;&nbsp;</td></tr></table></div></td></tr>
        
<tr bgcolor="#efefef">
<td height="33" bgcolor="#FFFFFF"><p><strong>顾客须知:</strong><span class="STYLE11">①定金概不退还或转让及更换他物期②摄影拍摄前请把套系欠款付清③为加强公司服务品质，请在下班2个小时之前来选片④当您的新娘在拍摄当天需要皮肤保养，美甲服务时或在超出所订套系以外的服务，根据顾客需求，按本公司服务价格自费购买;  ⑤ 放大应由所预约的套系组数里挑选.⑥ 像片取件时，请当面点清，事后缺少不负赔偿之责，保管期限自拍照日起二个月 内过期概不负责。</span>
</p></td></tr>
<tr bgcolor="#efefef">
<td height="33" bgcolor="#FFFFFF"><p><span class="STYLE13">拍照配合</span><strong>:</strong>①  摄影前一天晚上先洗头，勿擦发膠及发油，若欲护肤者请于摄影前三天完成。 ② 新郞可自备一套西装、穿白祙子、深色皮鞋、刮胡子，如欲修剪、烫发，请于一星期前完成。 ③ 新娘拍照当天请穿着排扣的上衣及无肩带式的内衣，以免影响发型化妆。 ④ 拍照时如对彩妆、造型、摄影方式有意见，请当面与服务人员沟通。 ⑤ 摄影前一天请先修剪指甲及有充分睡眠，睡前勿喝太多开水，预防眼睛浮肿。拍照请勿带参观者。 </p>  </td>
</tr>
        </table>
            <%
rs3.close
