...
.STYLE7 {color: #999999}
.style3 {color: #FF0000}
--></style></head><body><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr bgcolor="#99FFFF" class="df"><form action="kanjian.asp" method="post" name="form1"><td height="24"><div align="left">&nbsp;时间查询:
...
%><input name="chaxun" type="submit" id="chaxun" value="查询"><input name="action" type="hidden" id="action" value="<%= request("action") %>">
      </div></td><td align="right"><a href="sy_hz.asp?action=ap">摄影</a>　<a href="kanjian.asp?action=ap"><font color="#FF0000">选片</font></a>&nbsp;&nbsp;<a href="qujian2.asp?action=ap" class="style19 STYLE22">取件</a><%
if CompanyType=0 then
%>
&nbsp;&nbsp;<a href="kan_qu.asp?action=hz&action2=ap"><span class="style19">结婚</span></a><%
end if
%>
&nbsp;&nbsp;<a href="xiaogao.asp?action=no_ap"><font color="#FF0000">看版管理</font></a>&nbsp;&nbsp;</td></form><form action="kan_qu.asp" method="post" name="form1"></form></tr></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><form action="xiugai2.asp" method="post" name="form1"><tr><td>&nbsp;高级查询
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;点击单号进入客户面版</td></tr></form></table><br>
&nbsp;<a href="kanjian.asp">未选片（日期未到）</a>&nbsp;&nbsp;&nbsp;<a href="kanjian.asp?action=now">逾期未选片</a>&nbsp; &nbsp;<a href="kanjian.asp?action=wc">已完成选片 </a>&nbsp;&nbsp;<a href="kanjian.asp?action=ap"><strong><span class="style3">未摄影未安排时间</span></strong></a>（全部）&nbsp; <a href="kanjian.asp?action=cp_ap"><strong><span class="style3">已摄影未安排时间</span></strong></a>（全部） &nbsp;<a href="kanjian.asp?action=ap_br"><strong><span class="style3">未安排时间</span></strong></a>（本人）<br><br><table width="100%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr><td colspan="5"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td bgcolor="#FFFFFF"><%
...
if fromtime<>"" then
sqlstr = sqlstr & " and s.kj_time>=#"&fromtime&"#"
end if
if totime<>"" then
sqlstr = sqlstr & " and s.kj_time<=#"&totime&"#"
end if
select case action
case "wc"
response.Write "&nbsp;&nbsp;已完成选片列表"
sqlstr = sqlstr & " and (not isnull(s.lc_ky) or not isnull(s.lc_wc))"
case "now"
response.Write "&nbsp;&nbsp;逾期未选片列表"
sqlstr = sqlstr & " and s.kj_time<#"&date()&"# and isnull(s.lc_ky)"
case "ap"
response.Write "&nbsp;&nbsp;未摄影未安排时间列表"
sqlstr = sqlstr & " and isnull(s.lc_cp) and isnull(s.lc_ky) and isnull(s.kj_time)"
case "cp_ap"
response.Write "&nbsp;&nbsp;已摄影未安排时间列表"
sqlstr = sqlstr & " and not isnull(s.lc_cp) and isnull(s.lc_ky) and isnull(s.kj_time)"
case "ap_br"
response.Write "&nbsp;&nbsp;当前状态-->未安排时间(本人)"
sqlstr = sqlstr & " and (s.userid='"&session("userid")&"' or s.userid2='"&session("userid")&"' or s.userid3='"&session("userid")&"')"
case else
response.Write "&nbsp;&nbsp;未选片记录列表"
sqlstr = sqlstr & " and s.kj_time>=#"&date()&"# and isnull(s.lc_ky)"
end select
%>&nbsp;<strong><span class="STYLE7">请点击<span class="style3"><strong>单号</strong></span>进入相关面版</span></strong><div align="center"><span class="style4 style2"></span></div></td></tr></table><div align="right"></div></td></tr><%
set rs = server.CreateObject("adodb.recordset")
if shopid<>"" then
...
sqlstr = sqlstr & " order by s.kj_time desc"
...
%><tr bgcolor="#FFFFFF"><td colspan="5"><div align="center"><span class="style1">暂时还没有未选片客户</span></div></td></tr><%
else
%><tr bgcolor="#99FFFF"><td width="105" align="center">单号</td><td width="160" align="center">客户名</td><td width="166" align="center">选片日期</td><td width="160" align="center">套系</td><td width="161"><div align="center"><%
if request("action")="cp_ap" then
response.write "摄影日期"
else
response.write "下单日期"
end if
...
rs4.close()
set rs4=nothing
%></div></td><td><div align="center" class="STYLE2"><%
response.Write rs("kj_time")&"&nbsp;&nbsp;"
response.Write rs("kj")
%></div></td><td align="center"><%= GetFieldDataBySQL("select jixiang from jixiang where id="&rs("jixiang"),"str","") %></td><td><div align="center"><%
if request("action")="cp_ap" then
response.write rs("lc_cp")
else
...
%></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td><div align="center">共有记录&nbsp;<font color="#FF0000"><%=  rs.recordcount %></font>&nbsp;个　当前页<span class="style1"><%=  page %>/<%=  pgnm %></span>：
        <%
If page>1 Then
%><a href="kanjian.asp?page=<%=  page-1 %>&action=<%= request("action") %>&shopid=<%= request("shopid") %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="kanjian.asp?page=<%= page+1 %>&action=<%= request("action") %>&shopid=<%= request("shopid") %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
%></div></td></tr></table><%
