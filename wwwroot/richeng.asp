...
session.Timeout=60
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title><%= request("times") %>&nbsp;日程列表</title><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script language="javascript" src="inc/func.js" type="text/javascript"></script></head><body><%
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from richeng where peplename='"&session("username")&"' and times=#"&request("times")&"# order by shijian asc",conn,1,1
while not rs.eof
%><table width="669" height="84" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#A4B6D7" bgcolor="#A4B6D7" class="df"><tr bgcolor="#ECF5FF"><td width="307" height="20" bgcolor="#FFFFCC">&nbsp;&nbsp;<%= rs("title") %></td><td width="45" bgcolor="#FFFFCC"><font color="#0000FF"><%
if rs("import")=1 then response.Write "重要"
%></font></td><td width="103" bgcolor="#FFFFCC"><div align="left"><%
if rs("shijian")<>""then
...
%></div></td><td width="214" bgcolor="#FFFFCC"><%= rs("company") %></td></tr><tr bgcolor="#ECF5FF"><td height="58" colspan="4" valign="top" bgcolor="#ECF5FF"><table width="96%" height="38" border="0" align="center" cellpadding="0" cellspacing="0" class="df"><tr><td height="18" valign="top"><%= encode(rs("shuoming")) %></td></tr><%
if rs("huifu")<>"" then
