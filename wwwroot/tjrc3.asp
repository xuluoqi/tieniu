...
End If
End Sub
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>客服记录添加</title><style type="text/css"><!--
.style2 {
	color: #993366;
	font-size: 18px;
}
...
{alert("请填写日程!");
return false;}
if(document.form1.times.value=="")
{alert("请填写日程时间!");
return false;}
}
</script><%
set rs=server.CreateObject("adodb.recordset")
sql="select * from richeng"
...
rs("shijian")=shijian
rs("kehu_id")=request("id")
rs("import")=import
rs("shuoming")=htmlencode2(request("shuoming"))
rs("times")=request("times")
rs.update
response.Write "<script>alert('添加客服记录成功!');location='admin/llrc3.asp?id="&request("id")&"';</script>"
end if
rs.close
set rs=nothing
%><p align="center">&nbsp;</p><table width="623" height="243" border="0" align="center" cellpadding="0" cellspacing="0" class="fsdfg"><form action="" method="post" name="form1" onSubmit="return check()"><tr><td height="20" colspan="3"><p align="left" class="fsdfg style3"> 　　　　　　　　　　　　　　　　　客服记录添加</p></td></tr><tr><td width="80" height="21"><div align="right"><span class="fsdfg style3 style4">标题：<span class="style2"></span></span><span class="style2"></span></div></td><td width="310" height="21"><span class="fsdfg style3 style4"><span class="style2"><input name="title" type="text" id="title" size="45" maxlength="50"></span></span></td><td width="233"><input name="times" type="text" id="times" size="15" value="<%= date() %>">
      &nbsp;<span class="font"><a onClick="return showCalendar('times', 'y-mm-dd');" href="#"><img src="Image/Button.gif" width="25" height="17" border="0" align="absMiddle" id="IMG2" /></a></span></td></tr><tr><td height="22" valign="middle">&nbsp;</td><td><input type="radio" name="shijian" value="1">
早上
  <input type="radio" name="shijian" value="2">
中午
<input type="radio" name="shijian" value="3">
下午</td><td>重要：
      <input name="import" type="checkbox" id="import" value="1"></td></tr><tr><td height="135" valign="top"><div align="right" class="style5">
      具体说明：</div><div align="center"></div></td><td colspan="2"><textarea name="shuoming" cols="68" rows="10" id="textarea"></textarea></td></tr><tr><td height="45" colspan="3">
      　　　　　 　　　　　　　　　　　　　　  
        <div align="center"><input name="tijiao" type="submit" id="tijiao" value="提交">　　　　　　　　　　　
          <input name="reset"  type="button" id="reset" value="返回" onClick="javascript:history.go(-1)"><input name="id" type="hidden" id="id" value="<%= request("id") %>">
        </div></td></tr></form></table><p>&nbsp;</p><p>&nbsp;</p><table width="623" border="0" align="center" cellpadding="0" cellspacing="0"><%
set rs4=server.CreateObject("adodb.recordset")
rs4.open "select * from yongyu where userid='"&session("userid")&"'",conn,1,1
%><form action="" method="post" name="form2"><tr><td width="99"><div align="right">常用语：</div></td><td width="524"><textarea name="yongyu" cols="75" rows="10" id="yongyu"><%
if rs4.recordcount>0 then
response.Write encode2(rs4("yongyu"))
else
if not isnull(conn.execute("select yongyu from yongyu where userid='"&"admin"&"'")(0)) then
response.Write encode2(conn.execute("select yongyu from yongyu where userid='"&"admin"&"'")(0))
end if
end if
%></textarea></td></tr><%
if session("level")=10 or session("zhuguan")=1 then
%><tr><td height="35">&nbsp;</td><td><div align="center"><input type="submit" name="Submit" value="修改"><input name="chang" type="hidden" id="chang" value="1"><input name="id" type="hidden" id="id" value="<%= request("id") %>">
</div></td></tr><%
end if
%></form><%
rs4.close
set rs4=nothing
if request("chang")=1 then
set rs5=server.CreateObject("adodb.recordset")
rs5.open "select * from yongyu where userid='"&session("userid")&"'",conn,1,3
if rs5.eof then
rs5.addnew
rs5("yongyu")=htmlencode2(request("yongyu"))
rs5("userid")=session("userid")
rs5.update
else
rs5("yongyu")=request("yongyu")
rs5.update
end if
response.Write "<script> alert('修改成功!');location='tjrc3.asp'</script>"
rs5.close
set rs5=nothing
end if
%></table><p>&nbsp;</p><div align="center"><br></div></body></html>