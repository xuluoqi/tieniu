...
response.Write "<script>alert('你的帐户已被暂停使用，请与系统管理人员联系。');history.back()</script>"
On Error GoTo 0
Err.Raise 9999
else
...
session("UserShopID")=rs("shopid")
session("IsShare")=rs("isshare")
session("CustSourEncryp")=conn.execute("select isEncryp from sysconfig")(0)
session.Timeout=60
set rs2=server.CreateObject("adodb.recordset")
rs2.open "select top 1 * from times",conn,1,3
rs2.addnew
rs2("ip_address")=Request.ServerVariables("remote_addr")
rs2("yuangong")=session("username")
rs2("times")=now
rs2.update
rs2.close
set rs2=nothing
...
if rs("level")=10 then
dim ExMaxNumDate
ExMaxNumDate = conn.execute("select ExMaxNumDate from sysconfig")(0)
if not isnull(ExMaxNumDate) then
dim exdates
exdates = datediff("d",date(),ExMaxNumDate)
if exdates<=3 then
if exdates=0 then exdates="今"
'
'
'
'
'
response.write "<script language='javascript'>alert('提示：软件将在"& exdates &"天之后（"& ExMaxNumDate &"）到期。');</script>"
'
end if
end if
end if
...
set rs=nothing
Call InitChatRoom()
conn.execute("update [count] set [now_count]=[now_count]+1")
if session("level")=10 or session("level")=7 then Call DatabaseBackup()
if session("level")=5 then
session("hz_wed_flag")=conn.execute("select hz_wed_flag from sysconfig")(0)
end if
response.write "<script language='javascript'>location.href='admin/admin.asp';</script>"
...
'
Sub InitChatRoom()
Session("CRNAME")="影楼管理系统聊天室"
'
useid=Request.ServerVariables("HTTP_X_FORWARDED_FOR")
if useid="" then
ip_1=request.ServerVariables("REMOTE_ADDR")
else
ip_1=useid
end if
session("ip")=ip_1
...
outip(1)=session("ip")
cur=1
sentences(cur)="<font color=#FF0000>欢迎进入 影楼管理系统 at "&Now&"</font>"
...
Application(SESSION("CRNAME")&"ip")=outip
