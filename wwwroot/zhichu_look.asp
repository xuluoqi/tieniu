...
.style3 {color: #FF0000}
--></style><script language="javascript" src="inc/func.js" type="text/javascript"></script><link href="admin/zxcss.css" rel="stylesheet" type="text/css"></head><body><p>&nbsp;</p><table width="672" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8"><tr><td><div align="center"><a href="zhichu.asp">添加修改记录</a></div></td></tr></table><%
set rs=server.CreateObject("adodb.recordset")
if request("fromtime")<>"" and request("totime")<>""then
rs.open "select * from zhichu where yewuyuan='"&session("username")&"' and times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# order by times desc" ,conn,1,1
else
rs.open "select * from zhichu where yewuyuan='"&session("username")&"' order by times desc" ,conn,1,1
end if
...
%><table width="672" height="42" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bgcolor="#00CCFF"><form action="" method="post" name="form1"><tr bgcolor="#FF9966"><td>&nbsp;时间查询：
...
　
&nbsp;
<input name="chaxun" type="submit" id="chaxun" value="查询"></td><%
if request("fromtime")<>""and request("totime")<>"" then
set cn=conn.execute("select sum(money) as [money] from zhichu  where times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# and yewuyuan='"&session("username")&"'")
else
set cn=conn.execute("select sum(money) as [money] from zhichu where yewuyuan='"&session("username")&"'")
end if
%><td><div align="center" class="style3">总费用支出:<font color="#0000FF"><%= cn("money") %></font>元</div></td></tr></form><%
DO While not rs.eof
%><tr><td width="466"><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td width="80%"><%= rs("times") %></td><td width="20%"><a href="javascript:openEditScript('richeng.asp?times=<%= rs("times") %>',750,470)">查看日程</a></td></tr></table></td><td width="203">&nbsp;支出费用:<font color="red"><%= rs("money") %></font>元</td></tr><tr><td height="22" colspan="2"><div align="left"></div><table width="671" border="0" cellspacing="0" cellpadding="0"><tr><td width="12" height="22" bgcolor="#FFCCCC"><div align="right"></div></td><td width="659" bgcolor="#FFCCCC"><%= encode(rs("shuoming")) %></td></tr></table><div align="left"></div></td></tr><%
rs.movenext
i=i+1
if i>=count then exit do
loop
%></table><table width="672" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td><div align="center">共有记录&nbsp;<span class="style3"><%=  rs.recordcount %>&nbsp;</span>个,当前页<span class="style3"><%=  page %>/<%=  pgnm %></span>：
          <%
If page>1 Then
%><a href="zhichu_look.asp?page=<%=  page-1 %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="zhichu_look.asp?page=<%= page+1 %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
