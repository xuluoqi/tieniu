...
End If
End Sub
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>日程打印</title><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script language="javascript" src="inc/func.js" type="text/javascript"></script><style type="text/css"><!--
.style3 {
	color: #0066FF;
	font-size: 10pt;
}
.style4 {color: #FFFFFF}
--></style></head><body><div align="center">[<a href="print.asp?times=<%= date-1 %>"><%= day(now-1) %></a>][<a href="print.asp?times=<%= date %>"><%= day(now) %></a>][<a href="print.asp?times=<%= date+1 %>"><%= day(now+1) %></a>]
    <%
set rs=server.CreateObject("adodb.recordset")
if request("times")<>""then
rs.open "select * from richeng where peplename='"&session("username")&"' and times=#"&cdate(request("times"))-1&"# order by times asc,shijian asc",conn,1,1
else
rs.open "select * from richeng where peplename='"&session("username")&"' and times=#"&date-1&"# order by times asc,shijian asc",conn,1,1
end if
count1=rs.recordcount
if not rs.eof then
%></div><table width="638" height="53" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#999999" bgcolor="#FFFFFF"><tr bgcolor="#FFCCCC"><%
if request("times")<>""then
timed=cdate(request("times"))-1
else
timed=date-1
end if
%><td height="20" colspan="2" bgcolor="#FFCCCC"><div align="center">日程标题-<%= day(timed ) %>&nbsp;
	<%
select case weekday(timed)
case 1
response.Write "星期日"
case 2
response.Write "星期一"
...
%></div></td><td width="137" bgcolor="#FFCCCC"><div align="center">公司名称</div></td><td width="55"><div align="center">联系人</div></td><td width="117"><div align="center">联系人手机</div></td><td width="114"><div align="center">董事长手机</div></td></tr><%
while not rs.eof
%><tr><td width="171" height="23"><div align="left">
            &nbsp;
            <%
if rs("title")="经理日程委派!" then
response.Write "<font color=red>"&rs("title")&"</font>"
else
response.Write  long_str(rs("title"),18)
end if
if rs("import")=1 then
response.Write "<font color=blue>[重要]</font>"
end if
%></div></td><td width="30"><div align="center"><%
select case rs("shijian")
case 1
response.Write "早"
case 2
response.Write "中"
...
%></div></td><td ><div align="left">
        &nbsp;<%= long_str(rs("company"),18) %></div></td><%
set rs4=server.CreateObject("adodb.recordset")
rs4.open "select * from kehu where companyname='"&rs("company")&"'and yewuyuan='"&session("username")&"'",conn,1,1
%><td><div align="center"><%= rs4("lxpeple") %></div></td><td><div align="center"><%= long_str(GetTelNo(rs4("telephone")),15) %></div></td><td><div align="center"><%= long_str(GetTelNo(rs4("dstel")),15) %></div></td><%
rs4.close
set rs4=nothing
%></tr><%
rs.movenext
...
set rs=server.CreateObject("adodb.recordset")
if request("times")<>""then
rs.open "select * from richeng where peplename='"&session("username")&"' and times=#"&cdate(request("times"))&"# order by times asc,shijian asc",conn,1,1
else
rs.open "select * from richeng where peplename='"&session("username")&"' and times=#"&date&"# order by times asc,shijian asc",conn,1,1
end if
count2=rs.recordcount
if not rs.eof then
%><table width="638" height="45" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#CCCCCC" bgcolor="#FFFFFF"><tr bgcolor="#FF0000"><%
if request("times")<>""then
timed=cdate(request("times"))
else
timed=date
end if
%><td height="20" colspan="2"><div align="center" class="style4">今日程标题-<%= day(timed) %>&nbsp;
      <%
select case weekday(timed)
case 1
response.Write "星期日"
...
%></div></td><td width="137"><div align="left" class="style4"><div align="center">公司名称</div></div></td><td width="55"><div align="center" class="style4">联系人</div></td><td width="117"><div align="center" class="style4">联系人手机</div></td><td width="114"><div align="center" class="style4">董事长手机</div></td></tr><%
while not rs.eof
%><tr><td width="171" height="23"><div align="left">
            &nbsp;
          <%
...
set rs4=server.CreateObject("adodb.recordset")
rs4.open "select * from kehu where companyname='"&rs("company")&"'and yewuyuan='"&session("username")&"'",conn,1,1
%><td><div align="center"><%= rs4("lxpeple") %></div></td><td><div align="center"><%= long_str(rs4("telephone"),15) %></div></td><td><div align="center"><%= long_str(rs4("dstel"),15) %></div></td><%
rs4.close
set rs4=nothing
%></tr><%
rs.movenext
...
count3=rs.recordcount
if not rs.eof then
%><table width="638" height="45" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#999999" bgcolor="#FFFFFF"><tr bgcolor="#FFCCCC"><%
if request("times")<>""then
timed=cdate(request("times"))+1
else
timed=date+1
...
%><table width="633" height="78" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td width="664" height="20"><div align="center">[
   <%
if request("times")<>"" then
response.Write day(cdate(request("times"))-1)
else
response.Write day(now-1)
end if
%>
   ]号日程共<font color=red><%= count1 %></font>条　[ <%
if request("times")<>"" then
response.Write day(cdate(request("times")))
else
response.Write day(now)
end if
%>]号日程共<font color=red><%= count2 %></font>条　[ <%
if request("times")<>"" then
response.Write day(cdate(request("times"))+1)
else
response.Write day(now+1)
end if
%>]号日程共<font color=red><%= count3 %></font>条</div></td></tr><tr><td height="57"><div align="center"><span class="style3"><a href="javascript:window.print()">打印</a></span></div></td></tr></table></body></html>