...
Response.Flush()
%><center><span class="STYLE4"><br><%= yeard %> 年 <%= monthd %> 月
	[ 工作月报表 ]</span><br><br></center><%
dim rscolor
set rscolor=server.createobject("adodb.recordset")
rscolor.open "select * from sys_datecolor order by px",conn,1,1
if not (rscolor.eof and rscolor.bof) then
%><div style="padding:10px 0 10px 15px">
图例：<%
do while not rscolor.eof
response.write "<span style='width:15px; height:10px; background-color:"&rscolor("color")&";'></span> - "&rscolor("desc") & "&nbsp;&nbsp;&nbsp;"
rscolor.movenext
loop
%></div><%
end if
...
%><table width="98%" border="1" align="center" cellspacing="0"><tr  bgcolor="#66FFFF" align="center"><td width="40" bgcolor="#66FFFF">日期</td><td bgcolor="#66FFFF">摄影</td><td bgcolor="#66FFFF">选片</td><td bgcolor="#66FFFF">看版</td><td bgcolor="#66FFFF">取件</td><%
if CompanyType=0 then
%><td bgcolor="#66FFFF">结婚礼服</td><td bgcolor="#66FFFF">结婚</td><%
end if
%></tr><%
dates = GetDatesInMonth(yeard,monthd)
all_pz = 0
all_kj = 0
all_xg = 0
all_qj = 0
...
all_hz = 0
for i = 1 to dates
tmp_date = yeard&"-"&monthd&"-"&i
if cdate(tmp_date)=date() then
color="#efefef"
else
color="#ffffff"
end if
...
%><tr bgcolor="<%= color %>" onMouseOver="this.bgColor='#FF9966'" onMouseOut="this.bgColor='<%= color %>'" onClick="parent.location.href='mxb.asp?fromtime=<%= tmp_date %>';" style="cursor:pointer" title="单击进入当天排单系统">
    <td align="center"><%= i %></td><td align="center"><%
tmpvol = GetWorkOrders("pz_time",tmp_date)
all_pz = all_pz + tmpvol
Call ShowDatecolorList("pz_time","tc_pz_time",tmp_date,tmp_date)
%></td><td align="center"><%
tmpvol = GetWorkOrders("kj_time",tmp_date)
all_kj = all_kj + tmpvol
Call ShowDatecolorList("kj_time","tc_kj_time",tmp_date,tmp_date)
%></td><td align="center"><%
tmpvol = GetWorkOrders("xg_time",tmp_date)
all_xg = all_xg + tmpvol
Call ShowDatecolorList("xg_time","tc_xg_time",tmp_date,tmp_date)
%></td><td align="center"><%
tmpvol = GetWorkOrders("qj_time",tmp_date)
all_qj = all_qj + tmpvol
Call ShowDatecolorList("qj_time","tc_qj_time",tmp_date,tmp_date)
%></td><%
if CompanyType=0 then
%><td align="center"><%
tmpvol = GetWorkOrders("jhlf_time",tmp_date)
all_jhlf = all_jhlf + tmpvol
Call ShowDatecolorList("jhlf_time","tc_jhlf_time",tmp_date,tmp_date)
%></td><td align="center"><%
tmpvol = GetWorkOrders("hz_time",tmp_date)
all_hz = all_hz + tmpvol
Call ShowDatecolorList("hz_time","tc_hz_time",tmp_date,tmp_date)
%></td><%
end if
%></tr><%
next
%><tr bgcolor="#ffffff" onMouseOver="this.bgColor='#FF9966'" onMouseOut="this.bgColor='#ffffff'"><td align="center">合计</td><td align="center"><%= all_pz %></td><td align="center"><%= all_kj %></td><td align="center"><%= all_xg %></td><td align="center"><%= all_qj %></td><%
if CompanyType=0 then
%><td align="center"><%= all_jhlf %></td><td align="center"><%= all_hz %></td><%
end if
%></tr></table><br><script language="javascript">loadingHidden();</script></body></html>