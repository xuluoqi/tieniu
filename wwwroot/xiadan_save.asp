...
if not isnumeric(savemoney) then
response.Write "<script>alert('请输入正确的预收金额.');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
...
sy_number=conn.execute("select count(*) from shejixiadan where (pz_time=#"&request("pz_time")&"# or pz_time2=#"&request("pz_time")&"#)")(0)
...
sy_number=conn.execute("select count(*) from shejixiadan where (pz_time=#"&request("pz_time2")&"# or pz_time2=#"&request("pz_time2")&"#)")(0)
if sy_number>=sys and sys<>0 then
...
danhao=conn.execute("select count(*) from shejixiadan where danhao='"&request("danhao")&"' and danhao<>''")(0)
if danhao>0 then
...
rs2.open "select * from shejixiadan ",conn,1,3
rs2.addnew
rs2("danhao")=request("danhao")
rs2("kehu_id")=request("id")
rs2("sl")=sl11
rs2("yunyong")=request("check")
rs2("jixiang")=request("jixiang")
if request("hz_time")<>"" and request("hz")<>"" then
rs2("hz_time")=request("hz_time")
rs2("hz")=request("hz")
end if
rs2("beizhu")=htmlencode2(request("beizhu"))
...
userids=conn.execute("select userid from kehu where id="&request("id")&"")(0)
groups=conn.execute("select [group] from yuangong where username='"&userids&"'")(0)
rs2("group")=groups
rs2("userid")=userids
userid2=conn.execute("select userid2 from kehu where id="&request("id")&"")(0)
if userid2<>"" and not isnull(userid2) then
rs2("userid2")=userid2
end if
userid3=conn.execute("select userid3 from kehu where id="&request("id")&"")(0)
if userid3<>"" and not isnull(userid3) then
rs2("userid3")=userid3
end if
rs2("kj_userid")=conn.execute("select userid from kehu where id="&request("id")&"")(0)
if request("times")="" then
rs2("times")=date()
else
rs2("times")=request("times")
end if
...
conn.execute("insert into cuenchu (xiangmu_id,sp_id,sl,type,type2,type3,beizhu,times) values ("&xiangmu_id&","&id3(ii)&","&ttt(ii)&",2,1,1,'"&htmlencode2(request("beizhu"))&"',#"&now&"#)")
end if
next
if savemoney>0 then
set rs=server.CreateObject("adodb.recordset")
...
conn.execute("update kehu set islost=0 where id="&request("id"))
response.Write "<script>alert('设计下单成功!');location='caiwu.asp'</script>"
On Error GoTo 0
Err.Raise 9999
end if
%>