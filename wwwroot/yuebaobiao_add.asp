...
End If
End Sub
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>员工管理</title><link href="images/hs.css" rel="stylesheet" type="text/css"><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script language="javascript" src="inc/func.js" type="text/javascript"></script><style type="text/css"><!--
.style4 {
	color: #0000FF;
	font-size: 10pt;
}
...
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from kehu where userid='"&session("userid")&"' order by times asc",conn,1,1
response.write"<script language = ""JavaScript"">"
...
count=1
while not rs.eof
response.write"subcat["&count&"] = new Array('"& trim(month(rs("times")))&"','["&rs("times")&"]','"&trim(rs("lxpeple"))&"');"
count = count + 1
rs.movenext
wend
rs.close
...
response.write"{"
response.write"if (subcat[i][0] == month)"
response.write"{"
response.write"kk+=subcat[i][2]+'　 '+subcat[i][1]+'\n';"
response.write"}"
response.write"}"
response.Write "document.form1.company.value='本月公司列表：\n'+kk ;"
response.write"}"
response.Write "</script>"
%><body><%
if request("tijiao")="提交" then
set rs=server.CreateObject("adodb.recordset")
sql="select * from yuebaobiao"
rs.open sql,connstr,2,3
rs.addnew
rs("kehu")=HTMLEncode2(request("kehu"))
rs("xinde")=HTMLEncode2(request("xinde"))
rs("fangxiang")=HTMLEncode2(request("fangxiang"))
if request("month")<>"" then
rs("title")=left(date,4)&"年"&request("month")&"月份报表"
else
rs("title")=left(date,4)&"年"&month(date)&"月份报表"
end if
rs("yuangong")=session("username")
rs("times")=now()
rs.update
response.Write"<script>alert('添加月报表成功！');location='yuebaobiao_look.asp'</script>"
rs.close
set rs=nothing
end if
%></p><table width="95%" height="594"  border="0" align="center" cellpadding="0" cellspacing="0"><form action=""  method="post" name="form1" onSubmit="return check()"><tr><td width="55%" height="536"><table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#D4D0C8"><tr><td width="17%" height="27">&nbsp;</td><td colspan="2"><div align="left"><span class="style4">　　　　　　　　　　　　　月报表</span></div></td></tr><tr><td width="17%" height="37">&nbsp;</td><td colspan="2"><select name="month" id="month" ONCHANGE="change(this.options[this.selectedIndex].value)"><%
months=month(date)
for i=1 to months
%><option value="<%= i %>" <%
if i=month(date) then response.Write "selected"
%>><%= left(date,4) %>年<%= i %>月份报表</option><%
next
%></select><span class="style5">(请选择月份)</span></td></tr><tr><td height="96" rowspan="3" valign="top"><div align="right"><p class="style5">&nbsp; </p></div></td><td width="63%" height="18" bgcolor="#FFFFFF"><span class="style5">本月主要客户</span></td><td width="20%">&nbsp;</td></tr><tr><td height="67"><textarea name="kehu" cols="74" rows="6" id="kehu"></textarea></td><td height="67">&nbsp;</td></tr><tr><td height="18">&nbsp;</td><td height="18">&nbsp;</td></tr><tr><td height="26" rowspan="3" valign="top"><span class="style5"></span></td><td height="13" bgcolor="#FFFFFF"><span class="style5">本月心得(不少80字) </span></td><td height="13">&nbsp;</td></tr><tr><td height="6" colspan="2"><span class="style5"><textarea name="xinde" cols="74" rows="9" id="xinde"></textarea></span></td></tr><tr><td height="7" colspan="2">&nbsp;</td></tr><tr><td height="47" rowspan="2" valign="top"><div align="right"></div></td><td height="23" bgcolor="#FFFFFF"><span class="style5">下月业绩目标(不少于100字)</span></td><td height="23">&nbsp;</td></tr><tr><td height="138"><textarea name="fangxiang" cols="74" rows="9" id="fangxiang"></textarea></td><td height="138">&nbsp;</td></tr><tr><td height="18" valign="top">&nbsp;</td></tr></table></td><td width="45%" rowspan="2"><table width="294" height="404" border="0" cellpadding="0" cellspacing="0"><tr><%
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from kehu where userid='"&session("userid")&"'and month(times)='"&month(date)&"' order by times",conn,1,1
%><td width="352" height="404" valign="top"><textarea name="company" cols="50" rows="30" id="company"><%
response.Write "本月公司列表："&chr(13)
while not rs.eof
response.Write "&nbsp;&nbsp;"&rs("lxpeple")&"&nbsp;&nbsp;&nbsp;["&rs("times")&"]"&chr(13)
rs.movenext
wend
rs.close
set rs=nothing
%></textarea></td></tr></table></td></tr><tr><td height="58"><div align="center"><input name="tijiao" type="submit" id="tijiao" value="提交">
      　　　　　
      <input name="quexiao" type="button" id="quexiao" value="返回" onClick="javascript:history.go(-1)"></div></td></tr></form></table><blockquote>&nbsp;</blockquote></body></html>