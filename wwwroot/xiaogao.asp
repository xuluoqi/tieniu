<%@Language="VBSCRIPT"%>
<%if StrComp(AspAppEncRegCodeO, AspAppEncRegCodeN, 0) <> 0 then
Response.write "  "
On Error GoTo 0
Err.Raise 9999
end if
...
.style1 {color: #FF0000}
--></style><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script src="Js/Calendar.js"></script><script language="javascript" src="inc/func.js" type="text/javascript"></script><link href="Css/calendar-blue.css" rel="stylesheet"><style type="text/css">
.STYLE3 {color: #999999}
.style31 {color: #FF0000}
</style></head><body><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr bgcolor="#99FFFF" class="df"><form action="xiaogao.asp" method="post" name="form1"><td height="24">&nbsp;时间查询:
...
%>">
        <span class="font"><A onClick="return showCalendar('fromtime', 'y-mm-dd');" href="#"><img src="Image/Button.gif" name="IMG2" border="0" align="absMiddle" id="IMG2" /></A></span>&nbsp;到：
...
%>">
        <span class="font"><A onClick="return showCalendar('totime', 'y-mm-dd');" href="#"><img src="Image/Button.gif" name="IMG2" border="0" align="absMiddle" id="IMG2" /></A></span>&nbsp;
        <%
...
%>
        门市：
        <select name="menshi" id="menshi"><option value="<%= request("menshi") %>">
            <%
if request("menshi")<>"" then
response.Write conn.execute("select peplename from yuangong where username='"&request("menshi")&"'")(0)
else
response.Write "请选择"
end if
%></option><%
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from yuangong where level=1 and isdisabled=0",conn,1,1
while not rs.eof
...
%><input name="menshi" type="hidden" id="menshi" value="<%= session("userid") %>">
        <%
end if
%><input name="action" type="hidden" id="action" value="<%= request("action") %>">
        <input name="chaxun" type="submit" id="chaxun" value="查询"></td><td align="right"><a href="sy_hz.asp?action=ap">摄影</a>　<a href="kanjian.asp?action=ap"><font color="#FF0000">选片</font></a>&nbsp;&nbsp;<a href="qujian2.asp?action=ap" class="style19 STYLE22">取件</a><%
if CompanyType=0 then
%>
&nbsp;&nbsp;<a href="kan_qu.asp?action=hz&action2=ap"><span class="style19">结婚</span></a><%
end if
%>
&nbsp;&nbsp;<a href="xiaogao.asp?action=no_ap"><font color="#FF0000">看版管理</font></a>&nbsp;&nbsp;</td></form><form action="kan_qu.asp" method="post" name="form1"></form></tr></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><form name="form1" method="post" action="ckkh.asp" ><tr><td>&nbsp;高级查询--&gt;查询条件
        <select name="tiaojian" id="tiaojian"><option value="">请选择</option><option value="联系人" selected>联系人</option><option value="卡号" <%
if request("tiaojian")="卡号" then response.write "selected"
%>>卡号</option><option value="系统单号" <%
if request("tiaojian")="系统单号" then response.write "selected"
%>>系统单号</option><option value="联系电话" <%
if request("tiaojian")="联系电话" then response.write "selected"
%>>联系电话</option><option value="预约单号" <%
if request("tiaojian")="预约单号" then response.write "selected"
%>>预约单号</option><option value="地址" <%
if request("tiaojian")="地址" then response.write "selected"
%>>地址</option><option value="备注" <%
if request("tiaojian")="备注" then response.write "selected"
%>>备注</option></select><input name="neirong" type="text" id="neirong2" size="12" value="<%
if request("neirong")<>"" then
response.Write request("neirong")
end if
%>">
        <input name="chaxun" type="submit" id="chaxun" value="查询"><input name="action" type="hidden" id="action" value="search1"></td></tr></form></table><table width="100%" border="0" cellspacing="0" cellpadding="0"><tr><td height="31">&nbsp;&nbsp;<a href="xiaogao.asp">未看版(日期未到)</a>&nbsp;&nbsp;&nbsp;&nbsp;
      <!--<a href="xiaogao.asp?action=now"><strong>逾期未看版（未签名）</strong></a>&nbsp;&nbsp;&nbsp;-->
      <a href="xiaogao.asp?action=wc">已看版（已签名）</a>&nbsp;&nbsp;&nbsp;<a href="xiaogao.asp?action=no_ap"><strong><span class="style31">未安排时间</span></strong></a>（全部）&nbsp; &nbsp;<a href="xiaogao.asp?action=no_ap_br"><strong><span class="style31">未安排时间</span></strong></a>（本人）</td></tr></table><table width="100%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr><td colspan="6"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="28%" bgcolor="#FFFFFF"> 当前位置&gt;&gt;
          <%
'
'
dim action,fromtime,totime,sqlstr,menshi,shopid
action = request("action")
fromtime = request("fromtime")
totime = request("totime")
menshi = request("menshi")
shopid = request("shopid")
sqlstr = "select s.* from shejixiadan s inner join kehu k on s.kehu_id=k.id where 1=1"
...
if fromtime<>"" then
sqlstr = sqlstr & " and s.xg_time>=#"&fromtime&"#"
end if
if totime<>"" then
sqlstr = sqlstr & " and s.xg_time<=#"&totime&"#"
end if
if shopid<>"" then
...
select case action
case "wc"
response.Write "&nbsp;&nbsp;已看版记录"
sqlstr = sqlstr & " and (not isnull(s.xg_sj) or not isnull(s.lc_wc)) order by s.xg_time desc"
case "now"
response.Write "&nbsp;&nbsp; 逾期未看版列表"
sqlstr = sqlstr & " and isnull(s.xg_sj) and not isnull(s.xg_time) and s.xg_time<=#"&date()&"# order by s.xg_time asc"
case "no_ap"
response.Write "&nbsp;&nbsp; 未安排时间(全部)"
sqlstr = sqlstr & " and isnull(s.xg_time) order by s.id asc"
case "no_ap_br"
response.Write "&nbsp;&nbsp; 未安排时间(本人)"
sqlstr = sqlstr & " and isnull(s.xg_time) and (s.userid='"&session("userid")&"' or s.userid2='"&session("userid")&"' or s.userid3='"&session("userid")&"') order by s.id asc"
case else
response.Write "&nbsp;&nbsp; 未看版记录列表"
sqlstr = sqlstr & " and not (isnull(s.xg_time) or s.xg_opt=0) and isnull(s.xg_sj) and s.xg_time>=#"&date()&"# order by s.id asc"
end select
set rs = server.CreateObject("adodb.recordset")
...
%></td><td width="72%" bgcolor="#FFFFFF"><span class="STYLE3">请点击<strong>单号</strong>进入相关本人所有操作面版&nbsp;</span>&nbsp;&nbsp;&nbsp;</td></tr></table><div align="right"></div></td></tr><%
if rs.eof then
%><tr bgcolor="#FFFFFF"><td colspan="6"><div align="center" class="style1">暂时还没有未选片客户</div></td></tr><%
else
%><tr bgcolor="#99FFFF"><td align="center"><font color="#FF0000"><strong>单号</strong></font></td><td align="center">客户名</td><td align="center">看版日期</td><td align="center">套系</td><td align="center">门市</td><td align="center">下单日期</td></tr><%
do while not rs.eof
%><tr bgcolor="#FFFFFF" onMouseOver="this.bgColor='#FF9966'" onMouseOut="this.bgColor='#FFFFFF'"><td align="center"><a href="javascript:openEditScript('kehu_mianban.asp?id=<%= rs("id") %>',450,500)"><%= rs("id") %></a></td><td align="center"><%
...
%></td><td align="center"><%= GetFieldDataBySQL("select jixiang from jixiang where id="&rs("jixiang"),"str","") %></td><td align="center"><div align="center"><%
...
if rs("userid3")<>"" and not isnull(rs("userid3")) then
response.write ". "&conn.execute("select peplename from yuangong where username='"&rs("userid3")&"'")(0)
end if
%></div></td><td align="center"><div align="center"><%= rs("times") %></div></td></tr><%
rs.movenext
i=i+1
if i>=count then exit do
loop
end if
%></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td><div align="center">共有记录&nbsp;<font color="#FF0000"><%=  rs.recordcount %></font>&nbsp;个　当前页<span class="style1"><%=  page %>/<%=  pgnm %></span>：
      <%
If page>1 Then
%><a href="xiaogao.asp?page=<%=  page-1 %>&action=<%= request("action") %>&shopid=<%= shopid %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="xiaogao.asp?page=<%= page+1 %>&action=<%= request("action") %>&shopid=<%= shopid %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
%></div></td></tr></table><%
