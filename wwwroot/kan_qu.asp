...
dim CompanyType
CompanyType = SystemConfig("CompanyType", 0)
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title></title><script src="Js/Calendar.js"></script><script language="javascript" src="inc/func.js" type="text/javascript"></script><link href="Css/calendar-blue.css" rel="stylesheet"><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><style type="text/css"><!--
.style3 {color: #FF0000}
.style4 {
	color: #0033FF;
	font-weight: bold;
...
--></style></head><body><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr bgcolor="#99FFFF" class="df"><form action="kan_qu.asp?action=<%= request("action") %>" method="post" name="form1">
      <td height="24">&nbsp;时间查询:
...
%><input name="action2" type="hidden" id="action2" value="<%= request("action2") %>">            <input name="chaxun" type="submit" id="chaxun" value="查询"></td></form><td align="right"><a href="sy_hz.asp?action=ap">摄影</a>　<a href="kanjian.asp?action=ap"><font color="#FF0000">选片</font></a>&nbsp;&nbsp;<a href="qujian2.asp?action=ap" class="style19 STYLE22">取件</a><%
if CompanyType=0 then
%>&nbsp;&nbsp;<a href="kan_qu.asp?action=hz&action2=ap"><span class="style19">结婚</span></a><%
end if
%>&nbsp;&nbsp;<a href="xiaogao.asp?action=no_ap"><font color="#FF0000">看版管理</font></a>&nbsp;&nbsp;</td></tr></table><%
dim action,fromtime,totime,sqlstr,shopid
...
if fromtime<>"" then
sqlstr = sqlstr & " and s.hz_time>=#"&fromtime&"#"
end if
if totime<>"" then
sqlstr = sqlstr & " and s.hz_time<=#"&totime&"#"
end if
select case request("action2")
case "wc"
sqlstr = sqlstr & " and (not isnull(s.hz_userid) or not isnull(s.lc_wc))"
case "now"
sqlstr = sqlstr & " and s.hz_time<=#"&date&"# and isnull(s.hz_userid)"
case "ap"
sqlstr = sqlstr & " and isnull(s.hz_time) and isnull(s.hz_userid) and (s.jhz_style<>'' and not isnull(s.jhz_style))"
case "ap_br"
sqlstr = sqlstr & " and (s.userid='"&session("userid")&"' or s.userid2='"&session("userid")&"' or s.userid3='"&session("userid")&"') and (s.jhz_style<>'' and not isnull(s.jhz_style))"
case else
sqlstr = sqlstr & " and s.hz_time>=#"&date()&"#"
end select
set rs = server.CreateObject("adodb.recordset")
if shopid<>"" then
...
if hzs<>"" then
sqlstr = sqlstr & " and s.hz_userid='"&hzs&"'"
end if
...
sqlstr = sqlstr & " order by s.hz_time desc"
...
%><table width="100%"  border="0" cellspacing="0" cellpadding="0"><form action="xiugai2.asp" method="post" name="form1"><tr><td>&nbsp;查询条件
        <select name="tiaojian" id="select"><option value="<%= request("tiaojian") %>">
          <%
...
%></option><option value="联系人">联系人</option><option value="卡号">卡号</option><option value="系统单号">系统单号</option><option value="预约单号">预约单号</option></select><input name="content" type="text" id="content" size="12"value="<%= request("content") %>">
        　　
        <input name="chaxun2" type="submit" id="chaxun22" value="查询"><br></td></tr></form></table><table width="100%" border="0" cellspacing="0" cellpadding="0"><tr><td height="31"><a href="kan_qu.asp?action=hz&action2=no">&nbsp;&nbsp;等待化妆（日期未到）</a>&nbsp;&nbsp;&nbsp;<a href="kan_qu.asp?action=hz&action2=now">逾期未完成结婚妆（未签名） </a>&nbsp;&nbsp;<a href="kan_qu.asp?action=hz&action2=wc">完成化妆（已签名）</a>&nbsp;&nbsp;<a href="kan_qu.asp?action=hz&action2=ap"><strong><span class="style31">未安排时间</span></strong></a>（全部）&nbsp; &nbsp;<a href="kan_qu.asp?action=hz&action2=ap_br"><strong><span class="style31">未安排时间</span></strong></a>（本人）</td></tr></table><table width="100%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr bgcolor="#99FFFF"><td height="23" colspan="6" bgcolor="#FFFFFF">&nbsp;&nbsp;当前位置&gt;&gt;
      <%
if request("action2")="no" then
response.Write "等待化妆(化妆日期未到)"
elseif request("action2")="wc" then
response.Write "已完成化妆(已确认化妆)"
elseif request("action2")="now" then
response.Write "未确认完成化妆(已过化妆日期)"
elseif request("action2")="ap" then
response.Write "未安排时间(全部)"
elseif request("action2")="ap_br" then
response.Write "未安排时间(本人)"
else
response.Write "等待化妆(化妆日期未到)"
end if
%>&nbsp;</td></tr><tr bgcolor="#99FFFF"><td width="10%"><div align="center">单号</div></td><td width="19%"><div align="center">顾客姓名</div></td><td width="16%"><div align="center"><%= GetDutyName(5) %></div></td><td width="26%"><div align="center">结婚化妆日期</div></td><td width="10%"><div align="center">门市</div></td><td width="19%"><div align="center">下单日期</div></td></tr><%
do while not rs.eof
%><tr bgcolor="#FFFFFF"><td><div align="center"><a href="javascript:openEditScript('kehu_mianban.asp?id=<%= rs("id") %>',450,500)"><%= rs("id") %></a></div></td><td><div align="center"><%
...
if not isnull(rs("hz_userid")) and rs("hz_userid")<>"" and conn.execute("select count(*) from yuangong where username='"&rs("hz_userid")&"'")(0)>0 then
response.Write conn.execute("select peplename from yuangong where username='"&rs("hz_userid")&"'")(0)
else
response.Write "无"
end if
%></div></td><td><div align="center"><div align="center" class="STYLE6"><%= rs("hz_time") %>&nbsp;
            <%
response.Write rs("hz")
%></div></div></td><td><div align="center"><%
...
%></div></td><td><div align="center"><%= rs("times") %></div></td></tr><%
rs.movenext
i=i+1
if i>=count then exit do
loop
%></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td><div align="center">共有记录<span class="style3"><%=  rs.recordcount %>&nbsp;</span>个,当前页<span class="style3"><%=  page %>/<%=  pgnm %></span>：
            <%
If page>1 Then
%><a href="kan_qu.asp?page=<%=  page-1 %>&shopid=<%= shopid %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>&action=<%= request("action") %>&action2=<%= request("action2") %>&menshi=<%= request("menshi") %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="kan_qu.asp?page=<%= page+1 %>&shopid=<%= shopid %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>&action=<%= request("action") %>&action2=<%= request("action2") %>&menshi=<%= request("menshi") %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
%></div></td></tr></table><%
