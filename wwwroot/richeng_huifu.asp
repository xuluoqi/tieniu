...
session.Timeout=60
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title><%= request("times") %>&nbsp;日程列表</title><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><style type="text/css"><!--
.style3 {	color: #0033FF;
	font-size: 10pt;
}
--></style><script language="javascript" src="inc/func.js" type="text/javascript"></script></head><body><%
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from richeng where userid='"&session("userid")&"' and times=#"&request("times")&"# order by shijian asc",conn,1,1
while not rs.eof
%><table width="669" height="76" border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" bgcolor="#FFFFFF" class="df"><tr bgcolor="#ECF5FF"><td width="307" height="20" bgcolor="#FFFFCC">&nbsp;&nbsp;&nbsp;<%= rs("title") %></td><td width="45" bgcolor="#FFFFCC"><font color="#0000FF"><%
if rs("import")=1 then response.Write "重要"
%></font></td><td width="256" bgcolor="#FFFFCC"><div align="left"><%
if rs("shijian")<>""then
...
%></div></td><td width="61" bgcolor="#FFFFCC"><a href="richeng_huifu2.asp?id=<%= rs("id") %>&timed=<%= request("times") %>">回复</a></td></tr><tr bgcolor="#ECF5FF"><td height="56" colspan="4" valign="top" bgcolor="#FFFFFF"><table width="100%" height="38" border="0" align="center" cellpadding="0" cellspacing="0" class="df"><tr><td height="18" valign="top" bgcolor="#ECF5FF"><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td><%= encode(rs("shuoming")) %></td></tr></table></td></tr><%
if rs("huifu")<>"" then
%><tr><td height="20" valign="top" bgcolor="#ECF5FF"><table width="96%" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td><font color="#0000FF">回复：</font><%= encode(rs("huifu")) %></td></tr></table></td></tr><%
end if
...
rs.close
set rs=nothing
%><span class="style3"><a href="javascript:window.print()">打印</a>　<a href="#" onClick="javascript:window.close()">关闭</a></span></div></td></tr></table></body></html>