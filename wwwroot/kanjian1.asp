...
.style1 {color: #FF0000}
--></style><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script language="javascript" src="inc/func.js" type="text/javascript"></script><script src="Js/Calendar.js"></script><link href="Css/calendar-blue.css" rel="stylesheet"></head><body><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr bgcolor="#99FFFF" class="df"><form action="kanjian.asp" method="post" name="form1"><td width="76%" height="21"><div align="left">&nbsp;时间查询:
...
%>">
          ...
          　
          <input name="chaxun" type="submit" id="chaxun" value="查询"><input name="action" type="hidden" id="action" value="<%= request("action") %>">
      </div></td><td width="24%">[<a href="kanjian.asp?action=<%= request("action") %>&fromtime=<%= date-2 %>&totime=<%= date-2 %>"><%= day(date-2) %></a>] [<a href="kanjian.asp?action=<%= request("action") %>&fromtime=<%= date-1 %>&totime=<%= date-1 %>"><%= day(date-1) %></a>] [<a href="kanjian.asp?action=<%= request("action") %>&fromtime=<%= date %>&totime=<%= date %>"><font color="#CC0033"><%= day(date) %></font></a>] [<a href="kanjian.asp?action=<%= request("action") %>&fromtime=<%= date+1 %>&totime=<%= date+1 %>"><%= day(date+1) %></a>] [<a href="kanjian.asp?action=<%= request("action") %>&fromtime=<%= date+2 %>&totime=<%= date+2 %>"><%= day(date+2) %></a>]</td></form><form action="kan_qu.asp" method="post" name="form1"></form></tr></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><form action="xiugai2.asp" method="post" name="form1"><tr><td>&nbsp;高级查询--&gt;查询条件
...
%></option><option value="联系人">联系人</option><option value="卡号">卡号</option><option value="系统单号">系统单号</option><option value="预约单号">预约单号</option></select><input name="content" type="text" id="content" size="12"value="<%= request("content") %>">
　　
<input name="chaxun2" type="submit" id="chaxun2" value="查询">
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="kanjian.asp?action=ap">未安排时间</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;点击单号进入客户面版</td></tr></form></table><%
set rs=server.CreateObject("adodb.recordset")
set rs=server.CreateObject("adodb.recordset")
if request("action")="wc" then
if request("fromtime")<>"" and request("menshi")="" then
rs.open "select * from  shejixiadan where not isnull(lc_ky) and kj_time>=#"&request("fromtime")&"# and kj_time<=#"&request("totime")&"# and  (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by times desc",conn,1,1
elseif  request("fromtime")="" and request("menshi")="" then
rs.open "select * from  shejixiadan where not isnull(lc_ky) and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by times desc",conn,1,1
end if
elseif request("action")="ap" then
rs.open "select * from  shejixiadan where isnull(lc_ky) and isnull(kj_time) and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by times desc",conn,1,1
elseif request("action")="now" then
if request("fromtime")<>"" and request("menshi")="" then
rs.open "select * from  shejixiadan where shejiwancheng=0 and kj_time<#"&date&"# and isnull(lc_ky) and kj_time>=#"&request("fromtime")&"# and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by kj_time desc",conn,1,1
elseif  request("fromtime")="" and request("menshi")="" then
rs.open "select * from  shejixiadan where shejiwancheng=0 and kj_time<#"&date&"# and isnull(lc_ky) and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by kj_time desc",conn,1,1
end if
else
if request("fromtime")<>"" and request("menshi")="" then
rs.open "select * from shejixiadan where isnull(lc_ky) and  kj_time>=#"&date&"# and kj_time<=#"&request("totime")&"# and shejiwancheng=0 and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by kj_time desc",conn,1,1
else
rs.open "select * from shejixiadan where isnull(lc_ky) and  kj_time>=#"&date&"# and shejiwancheng=0 and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by kj_time desc",conn,1,1
end if
end if
...
%><table width="100%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr><td colspan="6"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="29%" bgcolor="#FFFFFF"><div align="left"><%
if request("action")="wc" then
response.Write "&nbsp;&nbsp;当前状态-->已选片记录(已确认选片)"
elseif request("action")="now" then
response.Write "&nbsp;&nbsp;当前状态-->逾期未选片确认"
elseif request("action")="ap" then
response.Write "&nbsp;&nbsp;当前状态-->未安排时间"
else
response.Write "&nbsp;&nbsp;当前状态-->未选片记录(日期未到)"
end if
%></div></td><td width="21%" bgcolor="#FFFFFF">&nbsp;</td><td width="50%" bgcolor="#FFFFFF"><div align="center"><span class="style4 style2"></span><a href="kanjian.asp">未选片记录(日期未到)</a>&nbsp;&nbsp;&nbsp;<a href="kanjian.asp?action=now">逾期未选片确认</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="kanjian.asp?action=wc">已选片记录(已确认选片)</a></div></td></tr></table><div align="right"></div></td></tr><%
if rs.eof then
%><tr bgcolor="#FFFFFF"><td colspan="6"><div align="center"><span class="style1">暂时还没有未选片客户</span></div></td></tr><%
else
%><tr bgcolor="#99FFFF"><td width="77"><div align="center">单号</div></td><td width="100"><div align="center">客户名</div></td><td width="139"><div align="center">选片日期</div></td><td width="123"><div align="center">套系</div></td><td width="123"><div align="center">文件保存路径</div></td><td><div align="center"></div><div align="center"></div><div align="center">下单日期</div></td></tr><%
do while not rs.eof
%><tr bgcolor="#FFFFFF"><td><div align="center"><a href="javascript:openEditScript('kehu_mianban.asp?id=<%= rs("id") %>',450,500)"><%= rs("id") %></a></div></td><td><div align="center"><%= conn.execute("select lxpeple from kehu where id="&rs("kehu_id")&"")(0) %></div></td><td><div align="center"><%
response.Write rs("kj_time")&"&nbsp;&nbsp;"
response.Write rs("kj")
%></div></td><td><div align="center"><%= conn.execute("select jixiang from jixiang where id="&rs("jixiang")&"")(0) %></div></td><td><div align="center"><%
if not isnull(rs("path")) then
response.Write "<a href="&rs("path")&" target='_blank' title="&rs("path")&">打开文件</a>[<a href=javascript:openwindow('admin/path.asp?id="&rs("id")&"',600,200) title="&rs("path")&">修改</a>]"
else
response.Write "<a href=javascript:openwindow('admin/path.asp?id="&rs("id")&"',600,200)>添加文件路径</a>"
end if
%></div></td><td><div align="center"></div><div align="center"></div><div align="center"><%= rs("times") %></div></td></tr><%
rs.movenext
i=i+1
...
%></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td><div align="center">共有记录&nbsp;<font color="#FF0000"><%=  rs.recordcount %></font>&nbsp;个　当前页<span class="style1"><%=  page %>/<%=  pgnm %></span>：
        <%
If page>1 Then
%><a href="kanjian.asp?page=<%=  page-1 %>&action=<%= request("action") %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="kanjian.asp?page=<%= page+1 %>&action=<%= request("action") %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
%></div></td></tr></table><%
