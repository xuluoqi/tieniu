...
conn.execute("update shejixiadan set hz_time=#"&request("hz_time")&"#,hz="&request("hz")&", hz_beizhu='"&beizhu&"' where id="&request("id")&"")
response.Write "<script>alert('修改成功！');location='fujia.asp?id="&request("id")&"'</script>"
On Error GoTo 0
Err.Raise 9999
end if
set rs=server.CreateObject("adodb.recordset")
rs.open "select * from shejixiadan where id="&request("id")&"",conn,1,1
%><form action="hz_beizhu.asp?action=edit" method="post" name="form1"><table width="65%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#D4D0C8"><tr bgcolor="#FFFFFF"><td width="19%"><div align="right">结婚日期:<span class="font"></span></div></td><td width="81%"><div align="left"><span class="font"><input name="hz_time" type="text" maxlength="10"  size="13" value="<%= rs("hz_time") %>">
        <a onClick="return showCalendar('hz_time', 'y-mm-dd');" href="#">&nbsp;<img src="Image/Button.gif" width="25" height="17" border="0" align="absMiddle" id="IMG2" /></a><input name="hz" type="radio" value="1" <%
if rs("hz")=1 then response.Write "checked"
%>>
  早上
  <input type="radio" name="hz" value="2" <%
if rs("hz")=2 then response.Write "checked"
%>>
  下午</span></div></td></tr><tr bgcolor="#FFFFFF"><td><div align="right">备注:</div></td><td><div align="left"><textarea name="beizhu" cols="65" rows="8" id="beizhu"><%= rs("hz_beizhu") %></textarea></div></td></tr></table><table width="65%"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td height="47"><div align="center"><input type="submit" name="Submit" value="提交">
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;
      <input type="button" name="Submit2" value="返回" onClick="history.go(-1)"><input name="id" type="hidden" id="id" value="<%= request("id") %>">
