...
function check()
{if(document.form1.fromtime.value==""||document.form1.totime.value=="")
{alert("请填写时间！");
return false;
}
}
...
if ( confirm("你确定要删除吗？")) 
{
window.location.href = "dele3.asp?id="+id
}
}
</script><body leftmargin="0" topmargin="0"><table width="100%"   border="0" align="center" cellpadding="0" cellspacing="0" class="df"><tr bgcolor="#99FFFF"><form action="llrc.asp" method="post" name="form1"><td width="471"><div align="left">&nbsp;&nbsp;&nbsp;&nbsp;时间查询:
...
%>">
        ...
        <input name="chaxun" type="submit" id="chaxun" value="查询"></div></td></form><form action="llrc.asp" method="post" name="form1"><td width="326">&nbsp;[<a href="llrc.asp?chaxun=查询&fromtime=<%= date-2 %>&totime=<%= date-2 %>"><%= day(date-2) %></a>] [<a href="llrc.asp?chaxun=查询&fromtime=<%= date-1 %>&totime=<%= date-1 %>"><%= day(date-1) %></a>] [<a href="llrc.asp?chaxun=查询&fromtime=<%= date %>&totime=<%= date %>"><font color="#CC0033"><%= day(date) %></font></a>] [<a href="llrc.asp?chaxun=查询&fromtime=<%= date+1 %>&totime=<%= date+1 %>"><%= day(date+1) %></a>] [<a href="llrc.asp?chaxun=查询&fromtime=<%= date+2 %>&totime=<%= date+2 %>"><%= day(date+2) %></a>]	　<a href="xiugai2.asp" target="main2">客服添加</a></td></form></tr><tr><td  colspan="2"><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr bordercolor="#0000FF" bgcolor="#99FFFF"><td width="36%"><div align="left">　&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;客服标题</div></td><td width="12%"><div align="center">顾客姓名</div></td><td width="18%"><div align="center">手机号码</div></td><td width="12%" bgcolor="#99FFFF"><div align="center">门市</div></td><td width="12%" bgcolor="#99FFFF"><div align="center">日程时间</div></td><td width="10%" bgcolor="#99FFFF"><div align="center">客服添加</div></td></tr><%
set rs5=server.createobject("adodb.recordset")
if request("fromtime")<>"" and request("totime")<>""then
sql="select * from richeng where times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# and userid='"&session("userid")&"' order by times desc"
else
sql="select * from richeng where userid='"&session("userid")&"' order by times desc"
end if
...
if pgnm>0 then rs5.absolutepage=page
...
%>">
          <td ><div align="left">               &nbsp;
                <a href="admin/llrc3.asp?id=<%= rs5("kehu_id") %>" >
                <%
if rs5("title")="管理员日程委派!" then
response.Write "<font color=red>"&rs5("title")&"</font>"
else
if len(rs5("title"))>15 then
response.Write left(rs5("title"),15)&".."
else
response.Write rs5("title")
end if
end if
%></a><%
if rs5("import")=1 then
response.Write "<font color=blue>[重要]</font>"
end if
if rs5("times")>=date then
%><a href="richeng_xiugai.asp?id=<%= rs5("id") %>"><font color=red>修改</font></a><%
end if
%></div></td><%
set rs4=server.CreateObject("adodb.recordset")
rs4.open "select * from kehu where id="&rs5("kehu_id")&"",conn,1,1
%><td bgcolor="#FFFFFF"><div align="left">&nbsp;&nbsp;<a href="javascript:openEditScript('ckkh.asp?id=<%= rs4("id") %>',800,350)"><%= rs4("lxpeple") %></a></div></td><td bgcolor="#FFFFFF"><div align="left">&nbsp;&nbsp;&nbsp;<%= GetTelNo(rs4("telephone")) %></div></td><%
rs4.close
set rs4=nothing
%><td><div align="left">&nbsp;&nbsp;<%= conn.execute("select peplename from yuangong where username='"&session("userid")&"'")(0) %></div></td><td>&nbsp;&nbsp;<%= rs5("times") %></td><td><div align="center"><a href="tjrc3.asp?id=<%= rs5("kehu_id") %>">添加</a></div></td></tr><%
rs5.movenext
i=i+1
if i>=count then exit do
loop
%></table></td></tr><tr><td colspan="2"><div align="center"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><%
if request("chaxun")="" then
set cn=conn.execute("select count(*) as richeng_zong from richeng where userid='"&session("userid")&"'")
elseif  request("fromtime")<>""and request("totime")<>""then
set cn=conn.execute("select count(*) as richeng_zong from richeng where times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# and userid='"&session("userid")&"'")
end if
%><td colspan="2" bgcolor="#FFFFFF"><div align="center"></div></td></tr><tr><td width="75%"><div align="center"> 当前页<span class="style3"><%= page %>/<%= pgnm %></span>：
                <%
If page>1 Then
%><a href="llrc.asp?page=<%= page-1 %>&chaxun=<%= request("chaxun") %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="llrc.asp?page=<%= page+1 %>&chaxun=<%= request("chaxun") %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>"> 下一页</a><%
Else
%><%=  "下一页" %></div></td><td width="25%">合计：日程总共
                <%
response.Write "<font color=red>"&cn("richeng_zong")&"</font>"
%>
条 　</td></tr></table></div><div align="center"></div><%
end if
rs5.close
set rs5=nothing
conn.close
