...
End If
End Sub
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>收款打印</title><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script language="javascript" src="inc/func.js" type="text/javascript"></script></head><body><%
set rs=server.CreateObject("adodb.recordset")
...
%>
&nbsp;&nbsp;<%= datevalue(rs("times")) %>&nbsp;&nbsp;&nbsp;客户：<%= conn.execute("select lxpeple from kehu where id="&conn.execute("select kehu_id from shejixiadan where id="&rs("xiangmu_id")&"")(0)&"")(0) %>&nbsp;&nbsp;&nbsp;门市：<%
if rs3("ky_name")<>"" and not isnull(rs3("ky_name")) then
response.write rs3("ky_name")
if rs3("ky_name2")<>"" and not isnull(rs3("ky_name2")) then response.write rs3("ky_name2")
else
dim ms1,ms2,ms3
...
<table width="88%"  border="1" bordercolor="#dedede" cellpadding="0" cellspacing="0" bgcolor="#CCCCCC" style="margin-left:10px;border-collapse:collapse "><tr bgcolor="#efefef"><td height="32" bgcolor="#FFFFFF">&nbsp;&nbsp;
      <%
if rs("type")=1 then
response.Write "套系缴费"
...
response.Write "拍照化妆缴费"
elseif rs("type")=4 then
response.Write "结婚化妆缴费"
end if
%>
	金额：<%= rs("money") %>&nbsp;元&nbsp;&nbsp;
	<%
...
if finalmoney>0 then
response.write "总款未缴: "&finalmoney&" 元&nbsp;&nbsp;"
end if
if rs3("kj_time")<>"" and not isnull(rs3("ky_name")) then
response.Write "选片:"&rs3("ky_name")&"&nbsp;&nbsp;"
end if
if not isnull(rs3("qj_time")) then
response.Write "取件时间:"&rs3("qj_time")&"&nbsp;&nbsp;"
end if
if not isnull(rs3("hz_time")) then
response.Write "化妆时间:"&rs3("hz_time")
end if
%></td></tr><tr bgcolor="#FFFFFF"><td height="80" valign="top" style="padding:10px"><%
if rs("type")=2 then
...
if instr(cstr(Arr_PrintSetting(4)),"1")>0 then
%>
◇服务内容◇
...
if instr(cstr(Arr_PrintSetting(4)),"0")>0 then
%>
◇取件内容◇
<div style="width:90%; padding:3px"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><%
count11=ubound(id)+1
...
dim src_beizhu,beizhu
src_beizhu = rs("beizhu")
Set re = New RegExp
re.Pattern = "\/\d+元"
re.Global = True
re.IgnoreCase = True
re.MultiLine = True
beizhu = re.Replace(src_beizhu,"")
response.write "<div>"&encode2(beizhu)&"</div>"
%>
    公司经手人签名:&nbsp;                                       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;         &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <%
...
%>    &nbsp;&nbsp; &nbsp;打单:<%= session("username") %>&nbsp;&nbsp;&nbsp; 客户签名： </td></tr>
<tr bgcolor="#efefef">
<td height="33" bgcolor="#FFFFFF">&nbsp;&nbsp;<STRONG>顾客须知:</STRONG>① 新娘化妆时如对彩妆、造型、首饰有意见，请当面与服务人员沟通。②服务完成后，请清点须带物品③当您的新娘在服务当天需要皮肤保养，美甲服务时或在超出所订套系以外的服务，根据顾客需求，按本公司服务价格自费购买; ④ 放大应由所预约的套系组数里挑选.⑤   像片取件时，请当面点清，事后缺少不负赔偿之责，保管期限自拍照日起二个月 内过期概不负责。 </td>
</tr>
</table>
