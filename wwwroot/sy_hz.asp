...
End If
End Sub
dim action,fromtime,totime,shopid,customerlosttype,tiaojian,neirong,syflag,menshi
action = request("action")
fromtime = request("fromtime")
totime = request("totime")
shopid = request("shopid")
customerlosttype = request("customerlosttype")
tiaojian = request("tiaojian")
neirong = request("neirong")
syflag = request("syflag")
menshi = request("menshi")
dim CompanyType
CompanyType = SystemConfig("CompanyType", 0)
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>摄影单号</title><script src="Js/Calendar.js"></script><script language="javascript" src="inc/func.js" type="text/javascript"></script><script language="javascript">
...
function SubmitForm(tid){
	var aa = document.getElementsByName("numlist");
	var frm = document.all.frmlist;
	var flag = false;
	var tmp_phonenumber="";
	for (var i=0; i<aa.length; i++){
    	if(aa[i].checked){
			tmp_phonenumber = tmp_phonenumber + "," + aa[i].value;
			flag=true;
		}
	}
	if(!flag){
		...
		document.getElementById("inp_smsphonenumber").value = tmp_phonenumber;
		openSmsWindow('admin/sms_send.asp');
	}
}
</script><link href="Css/calendar-blue.css" rel="stylesheet"><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><style type="text/css"><!--
...
.STYLE7 {color: #999999}
--></style></head><body><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr bgcolor="#99FFFF" class="df"><td height="24">&nbsp;摄影流程信息</td><form action="kan_qu.asp" method="post" name="form1"><td align="right"><a href="sy_hz.asp?action=ap">摄影</a>　<a href="kanjian.asp?action=ap"><font color="#FF0000">选片</font></a>&nbsp;&nbsp;<a href="qujian2.asp?action=ap" class="style19 STYLE22">取件</a><%
if CompanyType=0 then
%>
        &nbsp;&nbsp;<a href="kan_qu.asp?action=hz&action2=ap"><span class="style19">结婚</span></a><%
end if
%>
      &nbsp;&nbsp;<a href="xiaogao.asp?action=no_ap"><font color="#FF0000">看版管理</font></a>&nbsp;&nbsp;</td></form></tr></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><form name="form1" method="post" action="sy_hz.asp" ><tr><td>&nbsp;摄影时间查询
      <input name="fromtime" type="text" id="fromtime" size="10" value="<%= fromtime %>">
      <span class="font"><A onClick="return showCalendar('fromtime', 'y-mm-dd');" href="#"><img src="Image/Button.gif" name="IMG" border="0" align="absMiddle" id="IMG" /></A></span>&nbsp;到
      <input name="totime" type="text" id="totime" size="10" value="<%= totime %>">
      <span class="font"><A onClick="return showCalendar('totime', 'y-mm-dd');" href="#"><img src="Image/Button.gif" name="IMG" border="0" align="absMiddle" id="IMG" /></A></span>&nbsp;
      <%
if isnull(GetMultipleShopListValue()) then
Call ShowMultipleShopSelect(2, request("shopid"), true)
end if
%>
拍照类型:<span class="font"><select name="customerlosttype" id="customerlosttype"><option value="">请选择...</option><%
dim rslosttype
set rslosttype=conn.execute("select * from customerlosttype order by px")
while not rslosttype.eof
%><option value="<%= rslosttype("id") %>" <%
if cstr(customerlosttype)=cstr(rslosttype("id")) then response.write "selected"
...
%></select>
摄影状态:
          <select name="syflag" id="syflag"><option value="">全部</option><option value="0" <%
if syflag="0" then response.write "selected"
%>>未摄影</option><option value="1" <%
if syflag="1" then response.write "selected"
%>>已摄影</option></select>
门市:
          <select name="menshi" id="menshi"><option value="">全部</option><%
dim rsmenshi
set rsmenshi=conn.execute("select * from yuangong where [level]=1 and isdisabled=0 order by username")
while not rsmenshi.eof
%><option value="<%= rsmenshi("username") %>" <%
if menshi=rsmenshi("username") then response.write "selected"
%>><%= rsmenshi("peplename") %></option><%
rsmenshi.movenext
wend
rsmenshi.close
set rsmenshi = nothing
%></select></span> 条件选择:
<select name="tiaojian" id="tiaojian"><option value="">请选择</option><option value="联系人" selected>联系人</option><option value="卡号" <%
if tiaojian="卡号" then response.write "selected"
%>>卡号</option><option value="系统单号" <%
if tiaojian="系统单号" then response.write "selected"
%>>系统单号</option><option value="联系电话" <%
if tiaojian="联系电话" then response.write "selected"
%>>联系电话</option><option value="预约单号" <%
if tiaojian="预约单号" then response.write "selected"
%>>预约单号</option><option value="地址" <%
if tiaojian="地址" then response.write "selected"
%>>地址</option><option value="备注" <%
if tiaojian="备注" then response.write "selected"
%>>备注</option></select><input name="neirong" type="text" id="neirong2" size="10" value="<%= neirong %>">
<input name="chaxun" type="submit" id="chaxun" value="查询"><input name="action" type="hidden" id="action" value="search"></td></tr></form></table><table width="100%"  border="0" cellpadding="0" cellspacing="0"><tr><td height="25">&nbsp;&nbsp;<a href="sy_hz.asp?action=no_sy">未摄影（日期未到）</a>&nbsp;&nbsp;&nbsp;<a href="sy_hz.asp?action=now_sy">逾期未摄影（未签名）</a>&nbsp;&nbsp;&nbsp;<a href="sy_hz.asp?action=wc_sy">已完成摄影（已签名）</a>&nbsp;&nbsp;&nbsp;<a href="sy_hz.asp?action=ap"><strong><span class="style3">未安排时间</span></strong></a>（全部）&nbsp;&nbsp; &nbsp;<a href="sy_hz.asp?action=ap_br"><strong><span class="style3">未安排时间</span></strong></a>（本人）</td></tr></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="47%">	&nbsp; 当前位置&gt;&gt;
	  <%
dim sqlstr
sqlstr = "select s.*,k.telephone,k.telephone2 from shejixiadan s inner join kehu k on s.kehu_id=k.id where 1=1"
if fromtime<>"" then
sqlstr = sqlstr & " and s.pz_time>=#"&fromtime&"#"
searchkey=searchkey&"&fromtime="&fromtime
end if
if totime<>"" then
sqlstr = sqlstr & " and s.pz_time<=#"&totime&"#"
searchkey=searchkey&"&totime="&totime
end if
if customerlosttype<>"" then
sqlstr = sqlstr & " and k.customerlosttype="&customerlosttype
searchkey=searchkey&"&customerlosttype="&customerlosttype
end if
if action<>"" then searchkey=searchkey&"&action="&action
select case action
case "no_sy"
response.Write "等待中摄影"
sqlstr = sqlstr & " and s.pz_time>=#"&date()&"# and isnull(s.lc_cp)"
case "now_sy"
response.Write "逾期未摄影"
sqlstr = sqlstr & " and s.pz_time<#"&date()&"# and isnull(s.lc_cp)"
case "wc_sy"
response.Write "已完成摄影"
sqlstr = sqlstr & " and (not isnull(s.lc_cp) or not isnull(s.lc_wc))"
case "ap_br"
response.Write "未安排时间（本人）"
sqlstr = sqlstr & " and isnull(s.pz_time) and isnull(s.lc_cp) and (s.userid='"&session("userid")&"' or s.userid2='"&session("userid")&"' or s.userid3='"&session("userid")&"')"
case "search"
response.Write "查询"
case else
response.Write "未安排时间"
sqlstr = sqlstr & " and isnull(s.pz_time) and isnull(s.lc_cp)"
end select
if shopid<>"" then
sqlstr = sqlstr & " and k.shopid="&shopid
searchkey=searchkey&"&shopid="&shopid
elseif not isnull(GetMultipleShopListValue()) then
sqlstr = sqlstr & " and k.shopid="&session("UserShopID")
searchkey=searchkey&"&shopid="&session("UserShopID")
end if
if action<>"wc_hz" and action<>"wc_sy" then
sqlstr = sqlstr & " and (s.wc_name='' or isnull(s.wc_name)) and isnull(s.lc_wc) and s.shejiwancheng=0"
end if
if tiaojian<>"" and neirong<>"" then
searchkey=searchkey&"&tiaojian="&tiaojian
searchkey=searchkey&"&neirong="&neirong
select case tiaojian
case "联系人"
sqlstr = sqlstr & " and k.lxpeple & k.lxpeple2 like '%"& neirong &"%'"
case "卡号"
sqlstr = sqlstr & " and k.number='"& neirong &"'"
case "系统单号"
sqlstr = sqlstr & " and s.id="& neirong
case "联系电话"
sqlstr = sqlstr & " and s.danhao='"& neirong &"'"
case "预约单号"
sqlstr = sqlstr & " and k.telephone & k.telephone2 like '%"& neirong &"%'"
case "地址"
sqlstr = sqlstr & " and s.address & s.address2 like '%"& neirong &"%'"
case "备注"
sqlstr = sqlstr & " and s.shuoming like '%"& neirong &"%'"
end select
end if
if syflag<>"" then
searchkey=searchkey&"&syflag="&syflag
if syflag=0 then
sqlstr = sqlstr & " and isnull(lc_cp)"
elseif syflag=1 then
sqlstr = sqlstr & " and not isnull(lc_cp)"
end if
end If
If menshi<>"" Then
searchkey=searchkey&"&menshi="&menshi
sqlstr = sqlstr & " and (s.userid='"&menshi&"' or s.userid2='"&menshi&"' or s.userid3='"&menshi&"')"
End If
if session("level")=1 and session("zhuguan")<>1 then
sqlstr = sqlstr& " and (s.userid='"&session("userid")&"' or s.userid2='"&session("userid")&"' or s.userid3='"&session("userid")&"')"
end if
sqlstr = sqlstr & " order by s.pz_time desc"
%>	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong><span class="STYLE7">请点击<span class="style3"><strong>单号</strong></span>进入相关面版</span></strong></td><td width="53%">&nbsp;</td></tr></table><%
set rs = server.CreateObject("adodb.recordset")
...
%><form name="frmlist" method="post" action="" style="display:inline"><table width="100%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr bgcolor="#99FFFF"><td width="40">&nbsp;</td><td><div align="center">单号</div></td><td><div align="center">顾客姓名</div></td><td align="center">联系电话</td><td width="70"><div align="center"><%= GetDutyName(5) %></div></td><%
if syflag<>"0" then
%><td width="70" align="center">摄影师</td><%
end if
%><td width="70"><div align="center">门市</div></td><%
if action<>"ap" and action<>"ap_br" then
%><td><div align="center">摄影时间</div></td><%
end if
if action="ap" or action="ap_br" or syflag="0" then
%><td align="center">接单时间</td><%
end if
%></tr><%
do while not rs.eof
%><tr bgcolor="#FFFFFF" align="center"><td><input type="checkbox" name="numlist" id="numlist" value="<%
dim cur_phone
cur_phone = ""
if rs("telephone")<>"" and not isnull(rs("telephone")) then cur_phone = cur_phone & "," & rs("telephone")
if rs("telephone2")<>"" and not isnull(rs("telephone2")) then cur_phone = cur_phone & "," & rs("telephone2")
if cur_phone<>"" then cur_phone = mid(cur_phone, 2)
response.write cur_phone
%>" ></td><td><a href="javascript:openEditScript('kehu_mianban.asp?id=<%= rs("id") %>',450,500)"><%= rs("id") %></a></td><td><%
...
rs4.close()
set rs4=nothing
%></td><td><%= GetTelNo(cur_phone) %></td><td><%= rs("hz_name") %></td><%
if syflag<>"0" then
%><td align="center"><%= rs("hz_name") %></td><%
end if
%><td align="center"><%= conn.execute("select peplename from yuangong where username='"&rs("userid")&"'")(0) %></td><%
if action<>"ap" and action<>"ap_br" then
%><td><%= rs("pz_time") %>&nbsp;
      <%
...
loop
%></table><table width="100%" border="0" cellspacing="1" cellpadding="0"><tr><td width="40" height="30" align="center"><input name="chk_all" type="checkbox" id="chk_all" value="all" onClick="checkAll(this, 'numlist')"></td><td> 全选
      <input type="button" name="btn_send" value="群发短信" onClick="javascript:SubmitForm()"></td></tr></table><textarea name="inp_smsphonenumber" id="inp_smsphonenumber" style="display:none"></textarea></form><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td><div align="center">共有记录<span class="style3"><%=  rs.recordcount %>&nbsp;</span>个,当前页<span class="style3"><%=  page %>/<%=  pgnm %></span>：
        <%
If page>1 Then
%><a href="sy_hz.asp?page=1<%= searchkey %>">首页</a>&nbsp; <a href="sy_hz.asp?page=<%=  page-1 %><%= searchkey %>">上一页</a>&nbsp;
        <%
Else
response.write "首页&nbsp;上一页&nbsp;"
End If
If page<>pgnm Then
%><a href="sy_hz.asp?page=<%= page+1 %><%= searchkey %>">下一页</a>&nbsp; <a href="sy_hz.asp?page=<%= pgnm %><%= searchkey %>">最后一页</a>&nbsp;
        <%
Else
response.write "下一页&nbsp;最后一页&nbsp;"
End If
%></div></td></tr></table><blockquote>&nbsp;</blockquote></body></html>