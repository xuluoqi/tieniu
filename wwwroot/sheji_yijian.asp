...
--></style><link href="admin/zxcss.css" rel="stylesheet" type="text/css"></head><body><p>&nbsp;</p><%
if request("all")<>"1" then
topeple=request("userid")
else
topeple="所有人"
end if
if request("tijiao")="提交" then
if request("baobiao")<>"" then
set rs2=server.CreateObject("adodb.recordset")
sql="select * from sjs_baobiao"
rs2.open sql,connstr,1,3
rs2.addnew
rs2("xiangmu_id")=request("id")
rs2("baobiao")=htmlencode2(request("baobiao"))
rs2("times")=now()
rs2("topeple")=topeple
rs2("userid")=session("userid")
rs2.update
%><script>alert("报表添加成功!");location="jinzhang_look.asp?id=<%= request("id") %>"
</script><%
rs2.close
set rs2=nothing
else
response.Write "<script>alert('请填写报表内容!');history.go(-1)</script>"
end if
end if
%><table width="483" height="229" border="0" align="center" cellpadding="0" cellspacing="0"><form action="" method="post" name="form1"><tr><td width="483" height="42"><div align="right"></div>        
         
		　　　　　　　　        
		  <div align="center"><span class="style1">意见留言</span><input name="hide" type="hidden" value="<%= request("xiangmu") %>">
            <input name="hide2" type="hidden" value="<%= request("shejishi") %>">
          </div></td></tr><tr><td><div align="center"><textarea name="baobiao" cols="65" rows="9" id="baobiao"></textarea></div></td></tr><tr><td height="54"><div align="left"><p align="center"><input name="tijiao" type="submit" id="tijiao" value="提交">　　　　　
              <input name="reset" type="button" id="reset" value="返回"  onClick="javascript:history.go(-1)"><input name="id" type="hidden" id="id" value="<%= request("xiangmu_id") %>">
          </p></div></td></tr></form></table></body></html>