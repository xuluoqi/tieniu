...
End If
End Sub
dim CompanyType,Customer3rdInvis
CompanyType=GetFieldDataBySQL("select CompanyType from sysconfig","int",0)
Customer3rdInvis=GetFieldDataBySQL("select Customer3rdInvis from sysconfig","int",0)
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title>无标题文档</title><style type="text/css"><!--
...
--></style><link href="Css/calendar-blue.css" rel="stylesheet"><script src="Js/Calendar.js"></script><script language="javascript" src="inc/func.js" type="text/javascript"></script><script language="javascript">
function chk(){
if(!CheckIsDate(document.form1.WeddingDay,"请输入正确的结婚日期,格式如:2006-10-1.")) return false;
var now=new Date();
var year=now.getYear();
if(!CheckIsShortDate(document.form1.chusheng,"请输入正确的生日日期,格式如:8-8或"+(year-25)+"-8-8.\n")) return false;
if(!CheckIsShortDate(document.form1.chusheng2,"请输入正确的生日日期,格式如:8-8或"+(year-25)+"-8-8.\n")) return false;
return true;
}
...
--></style><body><p><%
if request("xiugai")="修改" then
set rs=server.CreateObject("adodb.recordset")
rs.Open "select * from kehu where id="&Trim(Request.Form("id")),connstr,1,3
if request("WeddingDay")<>"" then
rs("WeddingDay")=request("WeddingDay")
else
rs("WeddingDay")=null
end if
if request("chusheng")<>"" then
rs("chusheng")=request("chusheng")
else
rs("chusheng")=null
end if
if request("lxpeple2")<>"" then
if request("chusheng2")<>"" then
...
if request("lxpeple3")<>"" then
if request("chusheng3")<>"" then
rs("chusheng3")=request("chusheng3")
else
rs("chusheng3")=null
end if
rs("ShengriType3")=request("ShengriType3")
end if
rs("CustomerRank")=request("CustomerRank")
...
rs("c_pic")=request("c_pic")
rs.update
rs.Close
set rs=nothing
if request("hid_oldpic")<>"" and request("c_pic")<>request("hid_oldpic") then
set FSO=server.createobject("scripting.filesystemobject")
pic=server.mappath("upload/"&request("hid_oldpic"))
if FSO.FileExists(pic) then FSO.DeleteFile pic
set FSO=nothing
end if
response.write "<script language='javascript'>alert('修改成功!');location.href='admin/ckkh.asp?page="&request("page")&"';</script>"
On Error GoTo 0
Err.Raise 9999
end if
set rs=server.CreateObject("adodb.recordset")
rs.Open "select * from kehu where id="&Trim(Request.QueryString("id")),conn,1,1
%></p><table width="811"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td><div align="center"><img src="img/1101.JPG" width="806" height="43" border="0" usemap="#Map"></div></td></tr><tr><td valign="top" style="background:url(img/1102.JPG) no-repeat"><table width="100%" height="535"  border="0" cellpadding="0" cellspacing="0"><tr><td width="3%" height="34">&nbsp;</td><td width="94%">&nbsp;</td><td width="3%">&nbsp;</td></tr><tr><td height="482">&nbsp;</td><td valign="top"><form name="form1" method="post" action='kehu_xiugai.asp?action=edit'><table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" bordercolor="#A4B6D7" bgcolor="#CCCCCC" class="gd"><tr bgcolor="#FFFFFF"><td height="20" colspan="3">&nbsp;&nbsp;<%
menshi1=conn.execute("select userid from kehu where id="&request("id")&"")(0)
menshi2=conn.execute("select userid2 from kehu where id="&request("id")&"")(0)
menshi3=conn.execute("select userid3 from kehu where id="&request("id")&"")(0)
if menshi1<>"" and not isnull(menshi1) and menshi1<>session("userid") then response.Write  "同事:"&conn.execute("select peplename from yuangong where username='"&menshi1&"'")(0)&"&nbsp;&nbsp;&nbsp;"
if menshi2<>"" and not isnull(menshi2) and menshi2<>session("userid") then response.Write  "同事:"&conn.execute("select peplename from yuangong where username='"&menshi2&"'")(0)&"&nbsp;&nbsp;&nbsp;"
if menshi3<>"" and not isnull(menshi3) and menshi3<>session("userid") then response.Write  "同事:"&conn.execute("select peplename from yuangong where username='"&menshi3&"'")(0)&"&nbsp;&nbsp;&nbsp;"
%></td></tr><tr bgcolor="#FFFFFF"><td height="20" colspan="3">&nbsp; 卡号:
                  <%
...
%>
&nbsp;&nbsp;<span class="font">&nbsp;&nbsp; 介绍人:
<input name="js_name" type="text" id="js_name" size="15" readonly value="<%
if not isnull(rs("js_id")) then
if rs("js_id")>0 then
set rskhx = conn.execute("select lxpeple,lxpeple2 from kehu where id="&rs("js_id"))
if not rskhx.eof then
response.write rskhx("lxpeple")
if rskhx("lxpeple2")<>"" then response.write ","&rskhx("lxpeple2")
end if
rskhx.close
set rskhx = nothing
end if
...
%>" />
<input type="hidden" name="js_id" id="js_id" value="<%= rs("js_id") %>">
</span><%
if CompanyType=0 then
%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 结婚纪念:<span class="font"><select name="JhDateType" id="JhDateType"><option value="0" <%
if rs("JhDateType")=0 then response.write "selected"
%>>农历</option><option value="1" <%
if rs("JhDateType")=1 then response.write "selected"
%>>公历</option></select></span><input name="WeddingDay" type="text" id="WeddingDay" value="<%= rs("WeddingDay") %>" size="13" />
<a onClick="return showCalendar('WeddingDay', 'y-mm-dd');" href="javascript:void(0)"><img src="Image/Button.gif" name="IMG" width="25" height="17" border="0" align="absmiddle" id="IMG" /></a><%
end if
%> &nbsp;<span class="font"><select name="CustomerRank" id="CustomerRank"><%
dim rsrank
soption=rs("CustomerRank")
set rsrank = server.createobject("adodb.recordset")
rsrank.open "select * from customerrank order by px,id",conn,1,1
do while not rsrank.eof
response.write "<option value="&rsrank("id")
if soption=rsrank("id") then response.write " selected"
response.write ">"&rsrank("title")&"</option>"
rsrank.movenext
loop
rsrank.close
set rsrank = nothing
%></select></span></td></tr><tr bgcolor="#FFFFFF"><td height="20" colspan="3"><span class="font">&nbsp;&nbsp;拍照类型:
<select name="customerlost" id="customerlost"><option value="0">请选择</option><%
dim rslosttype
set rslosttype=conn.execute("select * from customerlosttype order by px")
while not rslosttype.eof
%><option value="<%= rslosttype("id") %>" <%
if rs("customerlosttype")=rslosttype("id") then response.write "selected"
%>><%= rslosttype("title") %></option><%
rslosttype.movenext
wend
rslosttype.close
...
%></select>
&nbsp; &nbsp;图片
<input name="c_pic" type="text" id="c_pic" size="25" value="<%= rs("c_pic") %>" />
<a href="#" onClick="window.open('admin/upfile.asp?formname=form1&editname=c_pic&filelx=jpg','','status=no,scrollbars=no,top=20,left=110,width=420,height=165')">上传图片
<input type="hidden" name="hid_oldpic" id="hid_oldpic" value="<%= rs("c_pic") %>">
</a> &nbsp;&nbsp;
<input name="islost" type="checkbox" id="islost" value="1" <%
if rs("islost")=1 then response.write " checked"
%> disabled>
流失客户</span></td></tr><tr bgcolor="#FFFFFF"><td height="20" colspan="3" bgcolor="#E9F1FE">&nbsp;<span class="font">客人信息1</span></td></tr><tr bgcolor="#FFFFFF"><td width="273" height="20">&nbsp;&nbsp;&nbsp;<%= GetAppellation(1, rs("IsChild")) %>：
                  <input name="lxpeple" type="text" id="lxpeple" value="<%= rs("lxpeple") %>" size="10" readonly>                </td><td width="227" height="20">&nbsp;&nbsp;性别：
                  <input name="sex" type="radio" value="男"<%
if rs("sex")="男" then response.Write "checked"
%> disabled>
男
<input type="radio" name="sex" value="女" <%
if rs("sex")="女" then response.Write "checked"
%> disabled>
女&nbsp;</td><td width="258" height="20">&nbsp;&nbsp;&nbsp;出生：
                  <select name="ShengriType" id="ShengriType"><option value="0" <%
if rs("ShengriType")=0 then response.write "selected"
%>>农历</option><option value="1" <%
if rs("ShengriType")=1 then response.write "selected"
%>>公历</option></select><input name="chusheng" type="text" id="chusheng" value="<%= rs("chusheng") %>" size="12" onBlur="if (this.value!=''){CheckIsShortDate(this,'请输入正确的生日格式,如:8-8或<%= year(date())-25 %>-8-8.\t')}"></td></tr><tr bgcolor="#FFFFFF"><td height="19">&nbsp;&nbsp;&nbsp;手机：
                  <input name="telephone" type="<%= GetTelNoFlag(rs("telephone")) %>" id="telephone" size="15" value="<%= GetTelNo(rs("telephone")) %>"readonly></td><td height="19">&nbsp;&nbsp;电话：<span class="font"><input name="home_tel" type="<%= GetTelNoFlag(rs("home_tel")) %>" id="home_tel" size="15" value="<%= GetTelNo(rs("home_tel")) %>"readonly>
                </span></td><td height="19"><span class="font">&nbsp; 客人QQ:&nbsp;&nbsp;
                    <input name="qq" type="text" id="qq" size="15" disabled /></span></td></tr><tr bgcolor="#FFFFFF"><td height="19" colspan="2">&nbsp;&nbsp;<span class="style2">&nbsp;地址：
                    <select name="city" id="city" disabled><option value="<%= rs("city") %>"><%= rs("city") %></option><%
set rs5=server.CreateObject("adodb.recordset")
rs5.open "select * from address",conn,1,1
while not rs5.eof
%><option value="<%= rs5("address") %>"><%= rs5("address") %></option><%
rs5.movenext
wend
rs5.close
set rs5=nothing
%></select><input name="address" type="text" id="address" size="35" value="<%= rs("address") %>" readonly>
                </span></td><td height="19">&nbsp;&nbsp; 邮政编码:
                  <input name="post" type="text" id="post5" value="<%= rs("post") %>" size="15"readonly>
                  <input type="hidden" name="id" value="<%= request("id") %>" ></td></tr><tr bgcolor="#FFFFFF"><td height="20" colspan="3" bgcolor="#E9F1FE">&nbsp;<span class="font">客人信息2</span></td></tr><tr bgcolor="#FFFFFF"><td height="16">&nbsp;&nbsp;&nbsp;<span id="s_app2"><%= GetAppellation(2, rs("IsChild2")) %></span>：
                  <input name="lxpeple2" type="text" id="lxpeple23" value="<%= rs("lxpeple2") %>" size="10"readonly>&nbsp;&nbsp;<%
Call ShowElementForChildSystem("CheckIsChild", "chk_ischild2", "yes", rs("ischild2"), "s_app2", false, false)
%></td><td height="16">&nbsp;&nbsp;性别：
                  <input name="sex2" type="radio" value="男" <%
if rs("sex2")="男" then response.Write "checked"
%> disabled>
男
<input type="radio" name="sex2" value="女" <%
if rs("sex2")="女" then response.Write "checked"
%> disabled>
女</td><td height="16">&nbsp;&nbsp;出生：
                  <select name="ShengriType2" id="ShengriType2"><option value="0" <%
if rs("ShengriType2")=0 then response.write "selected"
%>>农历</option><option value="1" <%
if rs("ShengriType2")=1 then response.write "selected"
%>>公历</option></select><input name="chusheng2" type="text" id="chusheng2" value="<%= rs("chusheng2") %>" size="12" onBlur="if (this.value!=''){CheckIsShortDate(this,'请输入正确的生日格式,如:8-8或<%= year(date())-25 %>-8-8.\t')}"></td></tr><tr bgcolor="#FFFFFF"><td height="16">&nbsp;&nbsp;&nbsp;手机：
                  <input name="telephone2" type="<%= GetTelNoFlag(rs("telephone2")) %>" id="telephone2" size="15" value="<%= GetTelNo(rs("telephone2")) %>"readonly></td><td height="16">&nbsp;&nbsp;电话：<span class="font"><input name="home_tel2" type="<%= GetTelNoFlag(rs("home_tel2")) %>" id="home_tel2" size="15" value="<%= GetTelNo(rs("home_tel2")) %>"readonly>
                </span></td><td height="16"><span class="font">&nbsp; 客人QQ:&nbsp;&nbsp;
                    <input name="qq2" type="text" id="qq2" size="15" disabled /></span></td></tr><tr bgcolor="#FFFFFF"><td height="16" colspan="2">&nbsp;&nbsp;<span class="style2">&nbsp;地址：
                  
                    <select name="city2" id="city2" disabled><option value="<%= rs("city2") %>"><%= rs("city2") %></option><%
set rs5=server.CreateObject("adodb.recordset")
rs5.open "select * from address",conn,1,1
while not rs5.eof
%><option value="<%= rs5("address") %>"><%= rs5("address") %></option><%
...
rs5.close
set rs5=nothing
%></select><input name="address2" type="text" id="address23" size="35" value="<%= rs("address2") %>" readonly>
                </span><div align="right"></div></td><td height="16">&nbsp;&nbsp;邮政编码:
<input name="post2" type="text" id="post24" value="<%= rs("post2") %>" size="15"readonly></td></tr><%
if Customer3rdInvis=1 then
%><tr bgcolor="#FFFFFF"><td height="20" colspan="3" bgcolor="#E9F1FE">&nbsp;<span class="font">客人信息3</span></td></tr><tr bgcolor="#FFFFFF"><td height="16">&nbsp;&nbsp;&nbsp;<span id="s_app3"><%= GetAppellation(2, rs("IsChild3")) %></span>：
<input name="lxpeple3" type="text" id="lxpeple3" value="<%= rs("lxpeple3") %>" size="10" readonly>&nbsp;&nbsp;<%
Call ShowElementForChildSystem("CheckIsChild", "chk_ischild3", "yes", rs("ischild3"), "s_app3", false, false)
%></td><td height="16">&nbsp;&nbsp;性别：
                  <input name="sex3" type="radio" value="男" <%
if rs("sex3")="男" then response.Write "checked"
%> disabled>
男
<input type="radio" name="sex3" value="女" <%
if rs("sex3")="女" then response.Write "checked"
%> disabled>
女</td><td height="16">&nbsp;&nbsp;出生：
                  <select name="ShengriType3" id="ShengriType3"><option value="0" <%
if rs("ShengriType3")=0 then response.write "selected"
%>>农历</option><option value="1" <%
if rs("ShengriType3")=1 then response.write "selected"
%>>公历</option></select><input name="chusheng3" type="text" id="chusheng3" value="<%= rs("chusheng3") %>" size="12" onBlur="if (this.value!=''){CheckIsShortDate(this,'请输入正确的生日格式,如:8-8或<%= year(date())-25 %>-8-8.\t')}"></td></tr><tr bgcolor="#FFFFFF"><td height="16">&nbsp;&nbsp;&nbsp;手机：
                  <input name="telephone3" type="<%= GetTelNoFlag(rs("telephone3")) %>" id="telephone3" size="15" value="<%= GetTelNo(rs("telephone3")) %>"readonly></td><td height="16">&nbsp;&nbsp;电话：<span class="font"><input name="home_tel3" type="<%= GetTelNoFlag(rs("home_tel3")) %>" id="home_tel3" size="15" value="<%= GetTelNo(rs("home_tel3")) %>"readonly>
                </span></td><td height="16"><span class="font">&nbsp; 客人QQ:&nbsp;&nbsp;
                    <input name="qq3" type="text" id="qq3" size="15" disabled /></span></td></tr><tr bgcolor="#FFFFFF"><td height="16" colspan="2">&nbsp;&nbsp;<span class="style2">&nbsp;地址：
                  
                    <select name="city3" id="city3" disabled><option value="<%= rs("city3") %>"><%= rs("city3") %></option><%
set rs5=server.CreateObject("adodb.recordset")
rs5.open "select * from address",conn,1,1
while not rs5.eof
%><option value="<%= rs5("address") %>"><%= rs5("address") %></option><%
...
rs5.close
set rs5=nothing
%></select><input name="address3" type="text" id="address3" size="35" value="<%= rs("address3") %>" readonly>
                </span><div align="right"></div></td><td height="16">&nbsp;&nbsp;邮政编码:
<input name="post3" type="text" id="post3" value="<%= rs("post3") %>" size="15"readonly></td></tr><%
end if
%><tr bgcolor="#FFFFFF"><td height="20" colspan="3" bgcolor="#E9F1FE">&nbsp;<span class="font">客人偏好选择</span></td></tr><tr bgcolor="#FFFFFF"><td height="22" colspan="3"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="1%"></td><td width="98%" valign="top"><%
set rs3=server.CreateObject("adodb.recordset")
rs3.open "select * from pianhao where ishidden=0 order by px,id",conn,1,1
while not rs3.eof
response.Write "&nbsp;&nbsp;"&rs3("title")&":"
set rs2=server.CreateObject("adodb.recordset")
rs2.open "select * from pianhao_list where title_id="&rs3("id")&" and ishidden=0 order by px,id",conn,1,1
while not rs2.eof
%><input type="checkbox" name="check" value="<%= rs2("id") %>" <%
if instr(rs("pianhao"),rs2("id"))>0 then response.Write "checked"
%> disabled>
                          <%
...
rs3.close
set rs3=nothing
%></td></tr></table></td></tr><tr bgcolor="#FFFFFF"><td height="75" colspan="3">&nbsp;备注：&nbsp;&nbsp;&nbsp;&nbsp;<textarea name="beizhu" cols="65" rows="5" id="beizhu"><%= rs("shuoming") %></textarea></td></tr><%
rs.close
set rs=nothing
%></table><table width="96%" height="64"  border="0" align="center" cellpadding="0" cellspacing="0"><tr><td><div align="center"><input name="xiugai" type="submit" id="xiugai2" value="修改" onClick="return chk();">
          　　　　　　　
          <input type="button" name="reset" value="返回" onClick="location='xiugai2.asp'"><input name="page" type="hidden" id="page" value="<%= request("page") %>">
                </div></td></tr></table></form></td><td>&nbsp;</td></tr><tr><td height="18">&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td></tr></table></td></tr></table><map name="Map"><area shape="rect" coords="597,11,729,29" href="pianhao.asp"></map></body></html>