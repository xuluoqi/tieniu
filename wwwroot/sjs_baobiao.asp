...
--></style><script language="javascript" src="inc/func.js" type="text/javascript"></script><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><style type="text/css"><!--
.style3 {color: #FF0000}
.style4 {font-size: 12px}
--></style><meta name="GENERATOR" Content="Microsoft Visual Studio .NET 7.1"><meta name="CODE_LANGUAGE" Content="C#"><script src="Js/Calendar.js"></script><link href="Css/calendar-blue.css" rel="stylesheet"></head><script language="javascript">
function confirmDel(id){
if ( confirm("确定项目已经完成了吗？")) 
{
window.location.href = "wancheng.asp?id="+id
}
}
</script><body bottommargin="0" leftmargin="0" topmargin="0" rightmargin="0" onkeypress="keyEnter()"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0"><tr><td height="24"><div align="center"><table width="100%" height="44" border="0" cellpadding="0" cellspacing="0" bgcolor="#FF9966" class="jh"><tr bgcolor="#FFFFFF"><form action="" method="post" name="form1"><td width="473" height="22">&nbsp;时间查询：
              <input name="fromtime" type="text" id="fromtime" size="12" value="<%
...
%>">
              <span class="font"><a onClick="return showCalendar('fromtime', 'y-mm-dd');" href="#"><img src="Image/Button.gif" width="25" height="17" border="0" align="absMiddle" id="IMG2" /></a></span><span class="font"></span>&nbsp;到：
              <input name="totime" type="text" id="totime"size="12" value="<%
...
&nbsp;&nbsp;<span class="font"><A onClick="return showCalendar('totime', 'y-mm-dd');" href="#"><img src="Image/Button.gif" width="25" height="17" border="0" align="absMiddle" id="IMG2" /></A></span>&nbsp;&nbsp; &nbsp;
              <input name="chaxun" type="submit" id="chaxun" value="查询"><input name="action" type="hidden" id="action" value="<%= request("action") %>"></td></form><td width="167"></a></td><td width="161"><%
if request("action")="yes" then
response.Write "<a href='sjs_baobiao.asp?fromtime="&request("fromtime")&"&totime="&request("totime")&"'>未完成项目</a>"
else
response.Write "<a href='sjs_baobiao.asp?fromtime="&request("fromtime")&"&totime="&request("totime")&"&action=yes'>已完成项目</a>"
end if
%></td></tr><tr bgcolor="#FFFFFF"><form action="" method="post" name="form2"><td height="22">&nbsp;分类查询:
...
%></option><option value="单号">单号</option><option value="姓名">姓名</option></select><input name="neirong" type="text" id="neirong" size="12" value="<%
if request("neirong")<>"" then
response.Write request("neirong")
end if
%>">
&nbsp;
<input name="chaxun2" type="submit" id="chaxun2" value="查询"><input name="action2" type="hidden" id="action2" value="<%= request("action") %>"></td><td height="22" colspan="2">点击进入客户控制面版</td></form></tr></table></div></td></tr></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td><form action="sjs_baobiao.asp?action=edit" name="form1" method="post"><table width="100%"  border="0" align="center" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr><td colspan="3"><tr bgcolor="#99FFFF"><td width="43"><div align="center">单号</div></td><td width="72"><div align="center">客户</div></td><td width="114"><div align="center">套系</div></td><td width="89"><div align="center">数码师</div></td><td width="87"><div align="center">摄影师</div></td><td width="80"><div align="center"><%= GetDutyName(5) %></div></td><td width="116"><div align="center"></div><div align="center"></div><div align="center">下单时间</div></td><td width="191"><div align="center"></div><div align="center"></div><div align="center"></div><div align="center">管理&nbsp;</div></td></tr><%
set rs2=server.CreateObject("adodb.recordset")
if request("fenlei")="单号"then
if request("neirong")=""then
response.Write "<script>alert('请输入查询内容！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
if not isnumeric(request("neirong"))  then
response.Write "<script>alert('单号只能是数字！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
rs2.open "select * from shejixiadan where (userid='"&session("userid")&"' or kj_userid='"&session("userid")&"') and id="&request("neirong")&" order by id desc",conn,1,1
...
rs2.open "select * from shejixiadan where (userid='"&session("userid")&"' or kj_userid='"&session("userid")&"') and kehu_id in (select id from kehu where lxpeple like '%"&request("neirong")&"%') order by id desc",conn,1,1
elseif request("fromtime")<>"" and request("totime")<>"" and request("action")<>"yes" and request("menshi")="" then
rs2.open "select * from shejixiadan where shejiwancheng="&"0"&" and times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"#  and (userid='"&session("userid")&"' or kj_userid='"&session("menshi")&"') order by id desc",conn,1,1
elseif request("fromtime")<>"" and request("totime")<>"" and request("action")<>"yes" and request("menshi")<>"" then
rs2.open "select * from shejixiadan where shejiwancheng="&"0"&" and times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"#  and (userid='"&request("menshi")&"' or kj_userid='"&request("menshi")&"') order by id desc",conn,1,1
elseif request("fromtime")<>"" and request("totime")<>"" and request("action")="yes" and request("menshi")="" then
rs2.open "select * from shejixiadan where shejiwancheng=1 and times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"#  and (userid='"&session("menshi")&"' or kj_userid='"&session("menshi")&"') order by id desc",conn,1,1
elseif request("fromtime")<>"" and request("totime")<>"" and request("action")="yes" and request("menshi")<>"" then
rs2.open "select * from shejixiadan where shejiwancheng=1 and times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"#  and (userid='"&request("menshi")&"' or kj_userid='"&request("menshi")&"') order by id desc",conn,1,1
elseif request("action")="yes" then
rs2.open "select * from shejixiadan where shejiwancheng=1  and (userid='"&session("userid")&"' or kj_userid='"&session("userid")&"')  order by id desc",conn,1,1
else
rs2.open "select * from shejixiadan where shejiwancheng="&"0"&"  and (userid='"&session("userid")&"' or kj_userid='"&session("userid")&"')  order by id desc",conn,1,1
end if
...
if pgnm>0 then rs2.absolutepage=page
...
response.Write "bgcolor='#CddCFF'"
end if
%>>
          <td><div align="left"></div><div align="center"><a href="javascript:openEditScript('kehu_mianban.asp?id=<%= rs2("id") %>',450,500)"><%= rs2("id") %></a></div></td><td><div align="center"><a href="javascript:openEditScript('kehu_mianban.asp?id=<%= rs2("id") %>',450,500)"><%= conn.execute("select lxpeple from kehu where id="&rs2("kehu_id")&"")(0) %></a></div></td><td><div align="center"><a href="xiadan.asp?id=<%= rs2("id") %>&action=edit"><%= conn.execute("select jixiang from jixiang where id="&rs2("jixiang")&"")(0) %></a></div></td><td><div align="center"><%
set rs3=server.CreateObject("adodb.recordset")
rs3.open "select * from xiadan where xiangmu_id="&rs2("id")&" and type="&"2"&"",conn,1,1
if rs3.eof then
response.Write "未分派"
else
...
set rs3=server.CreateObject("adodb.recordset")
rs3.open "select * from xiadan where xiangmu_id="&rs2("id")&" and type="&"4"&"",conn,1,1
if rs3.eof then
response.Write "未分派"
else
response.Write conn.execute("select peplename from yuangong where username='"&rs3("userid")&"'")(0)&"&nbsp;&nbsp;"
...
%></div></td><td><div align="center"><%
set rs3=server.CreateObject("adodb.recordset")
rs3.open "select * from xiadan where xiangmu_id="&rs2("id")&" and type="&"5"&"",conn,1,1
if rs3.eof then
response.Write "未分派"
else
response.Write conn.execute("select peplename from yuangong where username='"&rs3("userid")&"'")(0)&"&nbsp;&nbsp;"
...
rs3.close
set rs3=nothing
%></div></td><td><div align="center"></div><div align="center"></div><div align="left">
                &nbsp;&nbsp;
                <%
...
set cn3=conn.execute("select sum(money) as [money] from save_money where xiangmu_id="&rs2("id")&"")
if isnull(cn3("money"))then
money5=0
else
money5=cn3("money")
end if
%><%= rs2("times") %></div></td><td><div align="center"></div><div align="center"></div><div align="center"></div><div align="center"><a href="jinzhang_look.asp?id=<%= rs2("id") %>">报表</a>&nbsp;                  <a href="admin/xiadan.asp?id=<%= rs2("id") %>">技术员安排</a>&nbsp;&nbsp;<a href="javascript:openEditScript('kehu_mianban.asp?id=<%= rs2("id") %>',450,500)">客户面版</a></div></td></tr><%
rs2.movenext
i=i+1
if i>=count then exit do
loop
%></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td><div align="center"><span class="style4">共有记录&nbsp;<font color="#FF0000"><%=  rs2.recordcount %></font>&nbsp;个当前页<span class="style3"><%=  page %>/<%=  pgnm %></span>：
                    <%
If page>1 Then
%><a href="sjs_baobiao.asp?page=<%=  page-1 %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>&action=<%= request("action") %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="sjs_baobiao.asp?page=<%= page+1 %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>&action=<%= request("action") %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
rs2.close
...
%>
&nbsp;&nbsp;
          </span></div></td></tr></table></form></td></tr></table></body></html>