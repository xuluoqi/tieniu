...
--></style><link href="zxcss.css" rel="stylesheet" type="text/css"><link href="admin/zxcss.css" rel="stylesheet" type="text/css"></head><script language="javascript" src="inc/func.js" type="text/javascript"></script><script language="javascript">
function check()
{if(!CheckIsNull(document.form1.title,"请填写留言标题!")) return false;
if(!CheckIsNull(document.form1.neirong,"请填写留言内容!")) return false;
if(!CheckIsNull(document.form1.yuangong,"请选择留言对象!")) return false;
}
</script><body><p><%
if request("fasong")= "发送"then
set rs=server.CreateObject("adodb.recordset")
sql="select * from admin_liuyan"
rs.open sql,connstr,2,3
rs.addnew
rs("title")=request("title")
rs("frompeple")=session("username")
rs("topeple")=request("yuangong")
...
alert("通告添加完成！！");location="liuyan_look.asp?yuangong=<%= request("yuangong") %>"
  </script><%
end if
%></p><p>&nbsp;</p><table width="590" height="285" border="0" align="center" cellpadding="0" cellspacing="0"><form action="" method="post" name="form1" onSubmit="return check()"><tr><td height="46" colspan="2"><div align="center" class="style2">信息留言</div></td></tr><tr><td width="104" height="29"><div align="right">标题:</div></td><td><input name="title" type="text" id="title" size="45">
      留言对象：
        <select name="yuangong" id="yuangong"><%
set rs2=server.CreateObject("adodb.recordset")
sql="select * from yuangong where peplename<>'"&session("username")&"'"
rs2.open sql,connstr,1,1
%><option value="">请选择</option><%
while not rs2.eof
%><option value="<%= rs2("peplename") %>"><%= rs2("peplename") %></option><%
rs2.movenext
wend
rs2.close
set rs2=nothing
%></select></td></tr><tr><td valign="middle"><div align="right">内容：</div></td><td valign="middle"><textarea name="neirong" cols="65" rows="10" id="neirong"></textarea></td></tr><tr><td height="61" colspan="2"><div align="center"><input name="fasong" type="submit" id="add2" value="发送">
        　　　　　　　
        <input name="reset" type="button" id="reset" value="返回" onClick="javascript:history.go(-1)"><input type="hidden" name="hide" value="<%= request("yuangong") %>">
