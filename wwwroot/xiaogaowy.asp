...
End If
End Sub
%><html><head><meta http-equiv="Content-Type" content="text/html; charset=gb2312"><title></title><style type="text/css"><!--
.style1 {color: #FF0000}
--></style><link href="admin/zxcss.css" rel="stylesheet" type="text/css"><script src="Js/Calendar.js"></script><script language="javascript" src="inc/func.js" type="text/javascript"></script><link href="Css/calendar-blue.css" rel="stylesheet"></head><body><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr bgcolor="#99FFFF" class="df"><form action="xiaogao.asp" method="post" name="form1"><td width="76%" height="21"><div align="left">&nbsp;时间查询:
...
%>">
      ...
      </div></td><td width="24%">[<a href="xiaogao.asp?action=<%= request("action") %>&fromtime=<%= date-2 %>&totime=<%= date-2 %>"><%= day(date-2) %></a>] [<a href="xiaogao.asp?action=<%= request("action") %>&fromtime=<%= date-1 %>&totime=<%= date-1 %>"><%= day(date-1) %></a>] [<a href="xiaogao.asp?action=<%= request("action") %>&fromtime=<%= date %>&totime=<%= date %>"><font color="#CC0033"><%= day(date) %></font></a>] [<a href="xiaogao.asp?action=<%= request("action") %>&fromtime=<%= date+1 %>&totime=<%= date+1 %>"><%= day(date+1) %></a>] [<a href="xiaogao.asp?action=<%= request("action") %>&fromtime=<%= date+2 %>&totime=<%= date+2 %>"><%= day(date+2) %></a>]</td></form><form action="kan_qu.asp" method="post" name="form1"></form></tr></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><form action="xiugai2.asp" method="post" name="form1"><tr><td>&nbsp;高级查询--&gt;查询条件
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="xiaogaowy.asp?action=ap">未安排时间</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;点击单号进入客户面版</td></tr></form></table><%
set rs=server.CreateObject("adodb.recordset")
if request("action")="wc" then
if request("fromtime")<>"" and request("menshi")="" then
rs.open "select * from  shejixiadan where not isnull(xg_sj) and xg_time>=#"&request("fromtime")&"# and xg_time<=#"&request("totime")&"# and  (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by times desc",conn,1,1
elseif  request("fromtime")="" and request("menshi")="" then
rs.open "select * from  shejixiadan where not isnull(xg_sj) and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by times desc",conn,1,1
end if
elseif request("action")="ap" then
rs.open "select * from  shejixiadan where isnull(xg_sj) and isnull(xg_time) and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by times desc",conn,1,1
elseif request("action")="now" then
if request("fromtime")<>"" and request("menshi")="" then
rs.open "select * from  shejixiadan where shejiwancheng=0 and xg_time<#"&date&"# and isnull(xg_sj) and xg_time>=#"&request("fromtime")&"# and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by xg_time desc",conn,1,1
elseif  request("fromtime")="" and request("menshi")="" then
rs.open "select * from  shejixiadan where shejiwancheng=0 and xg_time<#"&date&"# and isnull(xg_sj) and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by xg_time desc",conn,1,1
end if
else
if request("fromtime")<>"" and request("menshi")="" then
rs.open "select * from shejixiadan where isnull(xg_sj) and  xg_time>=#"&date&"# and xg_time<=#"&request("totime")&"# and shejiwancheng=0 and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by kj_time desc",conn,1,1
else
rs.open "select * from shejixiadan where isnull(xg_sj) and  xg_time>=#"&date&"# and shejiwancheng=0 and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"') order by kj_time desc",conn,1,1
end if
end if
...
%></div></td><td width="21%" bgcolor="#FFFFFF">&nbsp;</td><td width="50%" bgcolor="#FFFFFF"><div align="center"><span class="style4 style2"></span><a href="xiaogao.asp">未看版记录(日期未到)</a>&nbsp;&nbsp;&nbsp;<a href="xiaogao.asp?action=now">逾期未看版确认</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="xiaogao.asp?action=wc">已看版记录(已确认选片)</a></div></td></tr></table><div align="right"></div></td></tr><%
if rs.eof then
%><tr bgcolor="#FFFFFF"><td colspan="6"><div align="center"><span class="style1">暂时还没有未选片客户</span></div></td></tr><%
else
%><tr bgcolor="#99FFFF"><td width="77"><div align="center">单号</div></td><td width="100"><div align="center">客户名</div></td><td width="139"><div align="center">看版日期</div></td><td width="123"><div align="center">套系</div></td><td width="123"><div align="center">文件保存路径</div></td><td><div align="center"></div><div align="center"></div><div align="center">下单日期</div></td></tr><%
do while not rs.eof
...
%></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td><div align="center">共有记录&nbsp;<font color="#FF0000"><%=  rs.recordcount %></font>&nbsp;个　当前页<span class="style1"><%=  page %>/<%=  pgnm %></span>：
        <%
If page>1 Then
%><a href="xiaogao.asp?page=<%=  page-1 %>&action=<%= request("action") %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="xiaogao.asp?page=<%= page+1 %>&action=<%= request("action") %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
%></div></td></tr></table><%
