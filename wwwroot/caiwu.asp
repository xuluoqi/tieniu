...
money1=0
money2=0
%><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><form action="" method="post" name="form1"><td width="314">分类查询: 
      <select name="fenlei" id="fenlei"><option value="<%= request("fenlei") %>">
          <%
if request("fenlei")=""then
response.Write "请选择"
else
response.Write request("fenlei")
end if
%></option><option value="序号">序号</option><option value="姓名">姓名</option></select><input name="neirong" type="text" id="neirong" size="12" value="<%
if request("neirong")<>"" then
response.Write request("neirong")
end if
%>">
&nbsp;
  <input name="chaxun" type="submit" id="chaxun" value="查询"></td></form><form action="" method="post" name="form2"><td width="487">时间查询
...
%>">
  ...
  <input name="chaxun" type="submit" id="chaxun" value="查询"><input name="action" type="hidden" id="action4" value="<%= request("action") %>">
  &nbsp;<a href="rc_caiwu.asp" target="main2">财务开票记录</a></td></form></tr></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="21%"><div align="left">&nbsp;&nbsp;
        <%
if request("action")="wc_no" then
response.Write "当前状态-->未结清"
elseif request("action")="wc_jq" then
response.Write "当前状态-->已结清[未完成]"
elseif request("action")="wc_yj" then
response.Write "当前状态-->已结清[已完成]"
end if
%></div></td><td width="44%">请点击单号进入面版</td><td width="35%"><div align="center"><a href="caiwu.asp?action=wc_no">未结清</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="caiwu.asp?action=wc_jq">已结清[未完成]&nbsp;</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="caiwu.asp?action=wc_yj">已结清[已完成]</a></div></td></tr></table><div align="center"><%
set rs=server.CreateObject("adodb.recordset")
...
if request("fenlei")="序号"then
if request("neirong")=""then
response.Write "<script>alert('请输入查询内容！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
rs.open "select * from shejixiadan where userid='"&session("userid")&"' and id="&request("neirong")&" order by id desc",conn,1,1
elseif request("fenlei")="姓名" then
if request("neirong")=""then
response.Write "<script>alert('请输入查询内容！');history.go(-1)</script>"
On Error GoTo 0
Err.Raise 9999
end if
rs.open "select * from shejixiadan where userid='"&session("userid")&"' and kehu_id in (select id from kehu where lxpeple like '%"&request("neirong")&"%') order by id desc",conn,1,1
elseif request("fromtime")<>""and request("totime")<>"" and request("menshi")="" then
rs.open "select * from shejixiadan where ReceivablesFlag="&tmp&" and (userid='"&session("userid")&"'  or userid2='"&session("userid")&"' or userid3='"&session("userid")&"' or kj_userid='"&session("userid")&"') and times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# order by id desc",conn,1,1
elseif request("fromtime")<>""and request("totime")<>"" and request("menshi")<>"" then
rs.open "select * from shejixiadan where ReceivablesFlag="&tmp&" and (userid='"&request("menshi")&"'  or userid2='"&session("userid")&"' or userid3='"&session("userid")&"' or kj_userid='"&request("menshi")&"') and times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# order by id desc",conn,1,1
else
rs.open "select * from shejixiadan where ReceivablesFlag="&tmp&" and (userid='"&session("userid")&"' or userid2='"&session("userid")&"' or userid3='"&session("userid")&"' or kj_userid='"&session("userid")&"') order by id desc",conn,1,1
'
'
end if
if rs.eof then
response.Write "<br><br><font color=red>暂时没有数据！</font>"
else
...
m4=0
m5=0
%></div><table width="100%" height="46" border="0" cellspacing="1" bgcolor="#CCCCCC"><tr bgcolor="#99FFFF"><td width="102" height="21" align="center">序号</td><td width="108" align="center">客户</td><td width="186" align="center">套系</td><td width="167" align="center">套系金额/已缴</td><td width="172" align="center">二次后期/已缴</td><td width="109" align="center">总金额</td><td width="129" align="center">状态</td></tr><%
do while not rs.eof
%><tr bgcolor="#FFFFFF"><td height="22" align="center"><a href="javascript:openEditScript('kehu_mianban.asp?id=<%= rs("id") %>',450,500)"><%= rs("id") %></a></td><td height="22" align="center"><a href="javascript:openEditScript('kehu_mianban.asp?id=<%= rs("id") %>',450,500)"><%= conn.execute("select lxpeple from kehu where id="&rs("kehu_id")&"")(0) %></a></td><td align="center"><%
str=conn.execute("select jixiang from jixiang where id="&rs("jixiang")&"")(0)
response.Write str
%></td><td align="center"><font color="#FF0000"><%
save1=conn.execute("select sum(money) from save_money where xiangmu_id="&rs("id")&" and type=1")(0)
save2=conn.execute("select sum(money) from save_money where xiangmu_id="&rs("id")&" and type=2")(0)
save3=conn.execute("select sum(money) from save_money where xiangmu_id="&rs("id")&" and type=3")(0)
save4=conn.execute("select sum(money) from save_money where xiangmu_id="&rs("id")&" and type=4")(0)
if isnull(save1) then save1=0
if isnull(save2) then save2=0
if isnull(save3) then save3=0
if isnull(save4) then save4=0
if rs("kj_userid")=session("userid") and rs("userid")<>session("userid") and rs("userid2")<>session("userid") and rs("userid3")<>session("userid") then
response.Write 0
else
response.Write rs("jixiang_money")
end if
%>
      /<%= save1 %></font></td><td align="center"><font color="#FF0000"><%
if rs("kj_userid")<>session("userid") then
money1=0
else
...
%>
      /<%= save2 %></font><div align="center"></div></td><td align="center"><font color="#FF0000"><%
if rs("kj_userid")=session("userid") and rs("userid")<>session("userid") then
money4=0
else
...
money_no=rs("jixiang_money")+money1+money2-save1-save2-save3
if money_no>0 then
response.Write "未结清"
elseif money_no<=0 and rs("shejiwancheng")=0 then
response.Write "已结清[未完成]"
elseif money_no<=0 and rs("shejiwancheng")=1 then
response.Write "已结清[已完成]"
end if
%></td></tr><%
m1=m1+money1
m2=m2+money2
...
loop
if request("fromtime")<>"" and request("totime")<>"" and request("menshi")="" then
jixiang_money =conn.execute("select sum(jixiang_money) from shejixiadan where times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"#")(0)
fujia_money=conn.execute("select sum(money) from fujia where xiangmu_id in (select id from shejixiadan where  times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# and userid='"&session("userid")&"')")(0)
fujia2_money=conn.execute("select sum(money) from fujia2 where xiangmu_id in (select id from shejixiadan where  times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# and userid='"&session("userid")&"')")(0)
goumai_money=conn.execute("select sum(money) from goumai where xiangmu_id in (select id from shejixiadan where  times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# and userid='"&session("userid")&"')")(0)
choucheng1=conn.execute("select sum(yewuchoucheng) from shejixiadan  where  times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# and userid='"&session("userid")&"'")(0)
choucheng2=conn.execute("select sum(choucheng) from fujia  where xiangmu_id in (select id from shejixiadan where  times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# and kj_userid='"&session("userid")&"')")(0)
elseif request("fromtime")<>"" and request("totime")<>"" and request("menshi")<>"" then
jixiang_money =conn.execute("select sum(jixiang_money) from shejixiadan where times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# and userid='"&request("menshi")&"'")(0)
fujia_money=conn.execute("select sum(money) from fujia where xiangmu_id in (select id from shejixiadan where  times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"#  and kj_userid='"&request("menshi")&"')")(0)
fujia2_money=conn.execute("select sum(money) from fujia2 where xiangmu_id in (select id from shejixiadan where  times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"#  and userid='"&request("menshi")&"')")(0)
goumai_money=conn.execute("select sum(money) from goumai where xiangmu_id in (select id from shejixiadan where  times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"#  and userid='"&request("menshi")&"')")(0)
choucheng1=conn.execute("select sum(yewuchoucheng) from shejixiadan where times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# and userid='"&request("menshi")&"'")(0)
choucheng2=conn.execute("select sum(choucheng) from fujia  where xiangmu_id in (select id from shejixiadan where  times>=#"&request("fromtime")&"# and times<=#"&request("totime")&"# and kj_userid='"&request("menshi")&"')")(0)
else
jixiang_money =conn.execute("select sum(jixiang_money) from shejixiadan where userid='"&session("userid")&"'")(0)
fujia_money=conn.execute("select sum(money) from fujia where xiangmu_id in (select id from shejixiadan where  kj_userid='"&session("userid")&"')")(0)
fujia2_money=conn.execute("select sum(money) from fujia2 where xiangmu_id in (select id from shejixiadan where userid='"&session("userid")&"')")(0)
goumai_money=conn.execute("select sum(money) from goumai where xiangmu_id in (select id from shejixiadan where userid='"&session("userid")&"')")(0)
choucheng1=conn.execute("select sum(yewuchoucheng) from shejixiadan where  userid='"&session("userid")&"'")(0)
choucheng2=conn.execute("select sum(choucheng) from fujia  where xiangmu_id in (select id from shejixiadan where kj_userid='"&session("userid")&"')")(0)
end if
if isnull(jixiang_money) then  jixiang_money=0
...
if isnull(choucheng1) then  choucheng1=0
if isnull(choucheng2) then  choucheng2=0
money11=jixiang_money+fujia_money+fujia2_money+goumai_money
choucheng11=choucheng1+choucheng2
%></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td><div align="center"><span class="style4">&nbsp;总业绩:<font color="#FF0000"><%= money11 %></font>元&nbsp;总抽成：<font color="#FF0000"><%= Round(choucheng11,2) %></font>元</span></div></td></tr></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td><div align="center">共有记录&nbsp;<span class="style3"><%=  rs.recordcount %>&nbsp;</span>个,当前页<span class="style3"><%=  page %>/<%=  pgnm %></span>：
        <%
If page>1 Then
%><a href="caiwu.asp?page=<%=  page-1 %>&fenlei=<%= request("fenlei") %>&neirong=<%= request("neirong") %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>&menshi=<%= request("menshi") %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="caiwu.asp?page=<%= page+1 %>&fenlei=<%= request("fenlei") %>&neirong=<%= request("neirong") %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>&menshi=<%= request("menshi") %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
%></div></td></tr></table><%
