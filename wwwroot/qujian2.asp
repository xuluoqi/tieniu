...
--></style><script language="javascript" src="inc/func.js" type="text/javascript"></script></head><body><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr bgcolor="#99FFFF" class="df"><form action="qujian2.asp" method="post" name="form1"><td height="24"><div align="left">&nbsp;时间查询:
...
%>
&nbsp;&nbsp;<a href="xiaogao.asp?action=no_ap"><font color="#FF0000">看版管理</font></a>&nbsp;&nbsp;</td></form><form action="kan_qu.asp" method="post" name="form1"></form></tr></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><form action="xiugai2.asp" method="post" name="form1"><tr><td>&nbsp;高级查询--&gt;查询条件
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="qujian2.asp?action=ap"></a></td></tr></form></table><table width="100%" border="0" cellspacing="0" cellpadding="0"><tr><td height="32"><%
...
select case action
case "wc"
sqlstr = sqlstr & " and not isnull(s.lc_wc)"
case "no"
sqlstr = sqlstr & " and isnull(s.lc_wc) and s.qj_time<=#"&date()&"#"
case "ap"
sqlstr = sqlstr & " and isnull(s.lc_wc) and isnull(s.qj_time)"
case "ap_br"
...
sqlstr = sqlstr & " order by s.qj_time desc"
set rs = server.CreateObject("adodb.recordset")
...
%>
&nbsp;&nbsp;<span class="style4 style2"><a href="qujian2.asp">未取件（日期未到）&nbsp;</a>&nbsp; <a href="qujian2.asp?action=no">逾期未取件（未签名）</a>&nbsp;&nbsp;&nbsp; <a href="qujian2.asp?action=wc">已取件（已签名）</a></span>&nbsp; &nbsp;<a href="qujian2.asp?action=ap"><strong><span class="style31">未安排时间</span></strong></a>（全部）&nbsp;&nbsp; &nbsp;<a href="qujian2.asp?action=ap_br"><strong><span class="style31">未安排时间</span></strong></a>（本人）</td></tr></table><table width="100%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#CCCCCC"><tr><td colspan="5"><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td width="25%" bgcolor="#FFFFFF"><div align="left"><%
if request("action")="wc" then
response.Write "&nbsp;&nbsp;当前状态-->已取件列表"
elseif request("action")="no" then
response.Write "&nbsp;&nbsp;当前状态-->逾期未取件"
elseif request("action")="ap" then
response.Write "&nbsp;&nbsp;当前状态-->未安排时间(全部)"
elseif request("action")="ap_br" then
response.Write "&nbsp;&nbsp;当前状态-->未安排时间(本人)"
else
response.Write "&nbsp;&nbsp;当前状态-->未取件列表"
end if
%></div></td><td width="41%" bgcolor="#FFFFFF"><span class="STYLE3">请点击单号进入客户控制面版</span></td><td width="34%" bgcolor="#FFFFFF"></td></tr></table></td></tr><%
if rs.eof then
%><tr bgcolor="#FFFFFF"><td colspan="5"><div align="center"><span class="style1">暂时还没有未取件客户</span></div></td></tr><%
else
%><tr bgcolor="#99FFFF"><td width="99" align="center">单号</td><td width="151" align="center">客户名</td><td width="144" align="center">取件日期</td><td width="247" align="center">套系</td><td width="111" align="center">下单日期</td></tr><%
do while not rs.eof
%><tr bgcolor="#FFFFFF"><td><div align="center"><a href="javascript:openEditScript('kehu_mianban.asp?id=<%= rs("id") %>',450,500)"><%= rs("id") %></a></div></td><td align="center"><%
...
rs4.close()
set rs4=nothing
%></td><td><div align="center" class="STYLE2"><%
response.Write rs("qj_time")&"&nbsp;&nbsp;"
response.Write rs("qj")
%></div></td><td><div align="center"><%= GetFieldDataBySQL("select jixiang from jixiang where id="&rs("jixiang"),"str","") %></div></td><td><div align="center"><%= rs("times") %></div></td></tr><%
rs.movenext
i=i+1
...
%></table><table width="100%"  border="0" cellspacing="0" cellpadding="0"><tr><td><div align="center">共有记录&nbsp;<font color="#FF0000"><%=  rs.recordcount %></font>&nbsp;个　当前页<span class="style1"><%=  page %>/<%=  pgnm %></span>：
        <%
If page>1 Then
%><a href="qujian2.asp?page=<%=  page-1 %>&action=<%= request("action") %>&shopid=<%= shopid %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>">上一页</a><%
Else
%><%=  "上一页" %><%
End If
If page<>pgnm Then
%><a href="qujian2.asp?page=<%= page+1 %>&action=<%= request("action") %>&shopid=<%= shopid %>&fromtime=<%= request("fromtime") %>&totime=<%= request("totime") %>"> 下一页</a><%
Else
%><%=  "下一页" %><%
End If
%></div></td></tr></table><%
