...
public class UserOper extends Controller {
	...
	public static void types(){
		List<ProductTypes> list= ProductTypes.find("type=null").fetch();
		for(ProductTypes type : list){
			type.types = ProductTypes.find("type.id=?", type.id).fetch();
		}
		render(list);
	}
	...
	public static void saveType(Long typeId,String typeName,Integer type,String typeEname){
        ...
        if(typeName==null||type==null||typeName.equals("")){
			...
			if(type==0){
				types = ProductTypes.findById(typeId);
				new ProductTypes(typeName, types,typeEname).save();
			}else if(type==1){
				types = ProductTypes.findById(typeId);
				types.name = typeName;
	...
	public static void typeByTid(Long id){
		List<ProductTypes> types = ProductTypes.find("type.id=?", id).fetch();
	...
	public static void delType(Long tid , Integer type){
        if(type == 0) {
            List<ProductTypes> types = ProductTypes.find("type.id = ? " ,tid).fetch();
            for (ProductTypes ts : types) {
                Product.delete("type.id = ?", ts.id);  // 删除商品
            }
            ProductTypes.delete("type.id=?", tid);  //删除子类
            ProductTypes.delete("id=?", tid); //删除自己
        } else {
	...
	public static void saveProduct(Product p ,File img,Long childType,File openImg) throws IOException{
		...
		if(p.id==0l)
			p.id=null;
		ProductTypes type = ProductTypes.findById(childType);
		p.productType = type;
		p.save();
